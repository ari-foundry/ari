...
static const CapabilityRow kCapabilityRows[] = {
    ...
    {"structural-capability-parameters", "implemented", "parser/type/trait-diagnostics", "make check-traits",
     "parameter-local, generic-bound, alias-bound, generic-alias-bound, and aggregate generic-bound single/grouped has-method capability syntax for functions, impl methods, trait method contracts, structs, and enums, method-only shape diagnostics, duplicate method-name rejection, String-returning capability methods, forwarded aggregate-bound use inside generic impl bodies, static call-site and aggregate type-application method checks, trait-impl structural-bound matching, and trait-quality diagnostics without an interface keyword"},
    {"union-by-fields", "implemented", "parser/type/ownership/backend", "make check-structs",
     "discriminant-linked union fields with stable selector validation, enum and bool selector construction via field: arm(payload) and compatibility field: arm => payload, struct-payload shorthand field: arm { ... }, bool false/true arm coverage, non-enum selector rejection, direct and nested selector inference, same-literal selector checks, direct field, selector-path, wrapper selector-path, and local alias match payload narrowing, selector/payload mutation rejection, direct owned-word active payload move/drop, whole-value active-payload cleanup, and owner-bearing aggregate payload diagnostics"},
