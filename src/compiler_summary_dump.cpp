...
static const CapabilityRow kCapabilityRows[] = {
    ...
    {"union-by-fields", "partial", "parser/type/ownership/backend", "make check-structs",
     "discriminant-linked union fields with stable selector validation, enum and bool selector construction via field: arm(payload) and compatibility field: arm => payload, struct-payload shorthand field: arm { ... }, bool false/true arm coverage, non-enum selector rejection, direct and nested selector inference, same-literal selector checks, direct field, selector-path, wrapper selector-path, and local alias match payload narrowing, and selector/payload mutation rejection"},
