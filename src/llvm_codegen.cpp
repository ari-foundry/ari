...
private:
    ...
    static IrType builtin_result_type(const std::string& symbol, SourceLocation loc) {
        ...
        if (symbol == "ari_builtin_context_arg" ||
            ...
            symbol == "ari_builtin_fs_canonicalize_bytes" ||
        ...
        if (symbol == "ari_builtin_env_has" ||
            ...
            symbol == "ari_builtin_fs_exists_bytes" ||
            ...
            symbol == "ari_builtin_fs_rename_bytes" ||
            symbol == "ari_builtin_fs_hard_link_bytes" ||
            symbol == "ari_builtin_fs_symbolic_link_bytes" ||
            symbol == "ari_builtin_fs_create_dir" ||
            symbol == "ari_builtin_fs_create_dir_bytes" ||
            symbol == "ari_builtin_fs_create_dir_all" ||
            symbol == "ari_builtin_fs_create_dir_all_bytes" ||
            symbol == "ari_builtin_fs_remove_dir" ||
            symbol == "ari_builtin_fs_set_mode" ||
            symbol == "ari_builtin_fs_set_mode_bytes" ||
            ...
            symbol == "ari_builtin_net_set_reuse_port" ||
            symbol == "ari_builtin_net_set_keepalive" ||
            ...
            symbol == "ari_builtin_net_unix_datagram_connect_bytes" ||
    ...
    void emit_runtime() {
        ...
        line("define private ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer) {");
        line("entry:");
        line("  %negative = icmp slt i64 %len, 0");
        line("  %too_long = icmp sge i64 %len, 4095");
        line("  %bad = or i1 %negative, %too_long");
        line("  br i1 %bad, label %fail, label %copy");
        line("copy:");
        line("  call void @llvm.memcpy.p0.p0.i64(ptr %buffer, ptr %data, i64 %len, i1 false)");
        line("  %end = getelementptr inbounds i8, ptr %buffer, i64 %len");
        line("  store i8 0, ptr %end");
        line("  ret ptr %buffer");
        line("fail:");
        line("  ret ptr null");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_exists_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_exists(ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_can_read_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_can_read(ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_can_write_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_can_write(ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_can_execute_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_can_execute(ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %size = call i64 @ari_builtin_fs_metadata_size(ptr %path)");
        line("  ret i64 %size");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_fs_metadata_kind_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_fs_metadata_mode_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %mode = call i64 @ari_builtin_fs_metadata_mode(ptr %path)");
        line("  ret i64 %mode");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %size = call i64 @ari_builtin_fs_symlink_metadata_size(ptr %path)");
        line("  ret i64 %size");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_fs_symlink_metadata_kind_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("define " + runtime_visibility + "ptr @ari_builtin_fs_canonicalize_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        line("  %missing = icmp eq ptr %path, null");
        line("  br i1 %missing, label %empty, label %check");
        line("check:");
        line("  %resolved = call ptr @ari_builtin_fs_canonicalize(ptr %path)");
        line("  ret ptr %resolved");
        line("empty:");
        line("  ret ptr " + empty);
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "ptr @ari_builtin_fs_read_link_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %empty, label %check");
        line("check:");
        line("  %target = call ptr @ari_builtin_fs_read_link(ptr %path)");
        line("  ret ptr %target");
        line("empty:");
        line("  ret ptr " + empty);
        line("}");
        ...
        line("define private i1 @ari_runtime_fs_two_paths_from_bytes(ptr %left_data, i64 %left_len, ptr %right_data, i64 %right_len, ptr %left_buffer, ptr %right_buffer, ptr %operation) {");
        line("entry:");
        line("  %left = call ptr @ari_runtime_fs_path_from_bytes(ptr %left_data, i64 %left_len, ptr %left_buffer)");
        line("  %left.missing = icmp eq ptr %left, null");
        line("  br i1 %left.missing, label %fail, label %right.check");
        line("right.check:");
        line("  %right = call ptr @ari_runtime_fs_path_from_bytes(ptr %right_data, i64 %right_len, ptr %right_buffer)");
        line("  %right.missing = icmp eq ptr %right, null");
        line("  br i1 %right.missing, label %fail, label %call");
        line("call:");
        line("  %operation.fn = bitcast ptr %operation to ptr");
        line("  %code = call i32 %operation.fn(ptr %left, ptr %right)");
        line("  %ok = icmp eq i32 %code, 0");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_rename_bytes(ptr %source_data, i64 %source_len, ptr %target_data, i64 %target_len) {");
        line("entry:");
        line("  %source.storage = alloca [4096 x i8], align 16");
        line("  %source.buffer = getelementptr inbounds [4096 x i8], ptr %source.storage, i64 0, i64 0");
        line("  %target.storage = alloca [4096 x i8], align 16");
        line("  %target.buffer = getelementptr inbounds [4096 x i8], ptr %target.storage, i64 0, i64 0");
        line("  %ok = call i1 @ari_runtime_fs_two_paths_from_bytes(ptr %source_data, i64 %source_len, ptr %target_data, i64 %target_len, ptr %source.buffer, ptr %target.buffer, ptr @rename)");
        line("  ret i1 %ok");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_hard_link_bytes(ptr %existing_data, i64 %existing_len, ptr %link_data, i64 %link_len) {");
        line("entry:");
        line("  %existing.storage = alloca [4096 x i8], align 16");
        line("  %existing.buffer = getelementptr inbounds [4096 x i8], ptr %existing.storage, i64 0, i64 0");
        line("  %link.storage = alloca [4096 x i8], align 16");
        line("  %link.buffer = getelementptr inbounds [4096 x i8], ptr %link.storage, i64 0, i64 0");
        line("  %ok = call i1 @ari_runtime_fs_two_paths_from_bytes(ptr %existing_data, i64 %existing_len, ptr %link_data, i64 %link_len, ptr %existing.buffer, ptr %link.buffer, ptr @link)");
        line("  ret i1 %ok");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_symbolic_link_bytes(ptr %target_data, i64 %target_len, ptr %link_data, i64 %link_len) {");
        line("entry:");
        line("  %target.storage = alloca [4096 x i8], align 16");
        line("  %target.buffer = getelementptr inbounds [4096 x i8], ptr %target.storage, i64 0, i64 0");
        line("  %link.storage = alloca [4096 x i8], align 16");
        line("  %link.buffer = getelementptr inbounds [4096 x i8], ptr %link.storage, i64 0, i64 0");
        line("  %ok = call i1 @ari_runtime_fs_two_paths_from_bytes(ptr %target_data, i64 %target_len, ptr %link_data, i64 %link_len, ptr %target.buffer, ptr %link.buffer, ptr @symlink)");
        line("  ret i1 %ok");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_create_dir_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_create_dir(ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_create_dir_all_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_create_dir_all(ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_fs_set_mode_bytes(ptr %data, i64 %len, i64 %mode) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %check");
        line("check:");
        line("  %ok = call i1 @ari_builtin_fs_set_mode(ptr %path, i64 %mode)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "{ i64 } @ari_builtin_fs_open_options(ptr %path, i1 %read, i1 %write, i1 %append, i1 %truncate, i1 %create, i1 %create_new) {");
        line("entry:");
        line("  %write.or.append = or i1 %write, %append");
        line("  %has.access = or i1 %read, %write.or.append");
        line("  %no.access = icmp eq i1 %has.access, false");
        line("  %create.any = or i1 %create, %create_new");
        line("  %not.writable = icmp eq i1 %write.or.append, false");
        line("  %bad.create = and i1 %create.any, %not.writable");
        line("  %bad.truncate = and i1 %truncate, %not.writable");
        line("  %bad.append.truncate = and i1 %append, %truncate");
        line("  %bad.0 = or i1 %no.access, %bad.create");
        line("  %bad.1 = or i1 %bad.0, %bad.truncate");
        line("  %bad = or i1 %bad.1, %bad.append.truncate");
        line("  br i1 %bad, label %invalid, label %open");
        line("open:");
        line("  %base.writable = select i1 %read, i32 2, i32 1");
        line("  %base = select i1 %write.or.append, i32 %base.writable, i32 0");
        line("  %create.flag = select i1 %create.any, i32 64, i32 0");
        line("  %excl.flag = select i1 %create_new, i32 128, i32 0");
        line("  %truncate.flag = select i1 %truncate, i32 512, i32 0");
        line("  %append.flag = select i1 %append, i32 1024, i32 0");
        line("  %flags.0 = or i32 %base, %create.flag");
        line("  %flags.1 = or i32 %flags.0, %excl.flag");
        line("  %flags.2 = or i32 %flags.1, %truncate.flag");
        line("  %flags = or i32 %flags.2, %append.flag");
        line("  %fd32 = call i32 @open(ptr %path, i32 %flags, i32 420)");
        line("  %fd = sext i32 %fd32 to i64");
        line("  %file = insertvalue { i64 } undef, i64 %fd, 0");
        line("  ret { i64 } %file");
        ...
        line("define " + runtime_visibility + "{ i64 } @ari_builtin_fs_open_options_bytes(ptr %data, i64 %len, i1 %read, i1 %write, i1 %append, i1 %truncate, i1 %create, i1 %create_new) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        line("  %missing = icmp eq ptr %path, null");
        line("  br i1 %missing, label %invalid, label %open");
        line("open:");
        line("  %file = call { i64 } @ari_builtin_fs_open_options(ptr %path, i1 %read, i1 %write, i1 %append, i1 %truncate, i1 %create, i1 %create_new)");
        line("  ret { i64 } %file");
        line("invalid:");
        line("  %invalid.file = insertvalue { i64 } undef, i64 -1, 0");
        line("  ret { i64 } %invalid.file");
        ...
        line("  %family.ptr = getelementptr inbounds i8, ptr %addr, i64 0");
        line("  store i16 10, ptr %family.ptr, align 2");
        line("  %port16 = trunc i64 %port to i16");
        line("  %net.port = call i16 @htons(i16 %port16)");
        line("  %port.ptr = getelementptr inbounds i8, ptr %addr, i64 2");
        line("  store i16 %net.port, ptr %port.ptr, align 2");
        line("  %s0.16 = trunc i64 %s0 to i16");
        line("  %s0.net = call i16 @htons(i16 %s0.16)");
        line("  %s0.ptr = getelementptr inbounds i8, ptr %addr, i64 8");
        line("  store i16 %s0.net, ptr %s0.ptr, align 2");
        line("  %s1.16 = trunc i64 %s1 to i16");
        line("  %s1.net = call i16 @htons(i16 %s1.16)");
        line("  %s1.ptr = getelementptr inbounds i8, ptr %addr, i64 10");
        line("  store i16 %s1.net, ptr %s1.ptr, align 2");
        line("  %s2.16 = trunc i64 %s2 to i16");
        line("  %s2.net = call i16 @htons(i16 %s2.16)");
        line("  %s2.ptr = getelementptr inbounds i8, ptr %addr, i64 12");
        line("  store i16 %s2.net, ptr %s2.ptr, align 2");
        line("  %s3.16 = trunc i64 %s3 to i16");
        line("  %s3.net = call i16 @htons(i16 %s3.16)");
        line("  %s3.ptr = getelementptr inbounds i8, ptr %addr, i64 14");
        line("  store i16 %s3.net, ptr %s3.ptr, align 2");
        line("  %s4.16 = trunc i64 %s4 to i16");
        line("  %s4.net = call i16 @htons(i16 %s4.16)");
        line("  %s4.ptr = getelementptr inbounds i8, ptr %addr, i64 16");
        line("  store i16 %s4.net, ptr %s4.ptr, align 2");
        line("  %s5.16 = trunc i64 %s5 to i16");
        line("  %s5.net = call i16 @htons(i16 %s5.16)");
        line("  %s5.ptr = getelementptr inbounds i8, ptr %addr, i64 18");
        line("  store i16 %s5.net, ptr %s5.ptr, align 2");
        line("  %s6.16 = trunc i64 %s6 to i16");
        line("  %s6.net = call i16 @htons(i16 %s6.16)");
        line("  %s6.ptr = getelementptr inbounds i8, ptr %addr, i64 20");
        line("  store i16 %s6.net, ptr %s6.ptr, align 2");
        line("  %s7.16 = trunc i64 %s7 to i16");
        line("  %s7.net = call i16 @htons(i16 %s7.16)");
        line("  %s7.ptr = getelementptr inbounds i8, ptr %addr, i64 22");
        line("  store i16 %s7.net, ptr %s7.ptr, align 2");
        line("  ret void");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_tcp_listen_v6(i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port) {");
        line("entry:");
        line("  %fd32 = call i32 @socket(i32 10, i32 1, i32 0)");
        line("  %socket.bad = icmp slt i32 %fd32, 0");
        line("  br i1 %socket.bad, label %fail, label %bind");
        line("bind:");
        line("  %storage = alloca [28 x i8], align 4");
        line("  %addr = getelementptr inbounds [28 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v6(ptr %addr, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port)");
        line("  %bind.code = call i32 @bind(i32 %fd32, ptr %addr, i32 28)");
        line("  %bind.ok = icmp eq i32 %bind.code, 0");
        line("  br i1 %bind.ok, label %listen, label %close_fail");
        line("listen:");
        line("  %listen.code = call i32 @listen(i32 %fd32, i32 128)");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_tcp_connect_v6(i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port) {");
        line("entry:");
        line("  %fd32 = call i32 @socket(i32 10, i32 1, i32 0)");
        line("  %socket.bad = icmp slt i32 %fd32, 0");
        line("  br i1 %socket.bad, label %fail, label %connect");
        ...
        line("  %addr = getelementptr inbounds [28 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v6(ptr %addr, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port)");
        line("  %connect.code = call i32 @connect(i32 %fd32, ptr %addr, i32 28)");
        line("  %connect.ok = icmp eq i32 %connect.code, 0");
        line("  br i1 %connect.ok, label %ok, label %close_fail");
        line("ok:");
        line("  %fd = sext i32 %fd32 to i64");
        ...
        line("query:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %storage = alloca [128 x i8], align 8");
        line("  %addr = getelementptr inbounds [128 x i8], ptr %storage, i64 0, i64 0");
        line("  %len.ptr = alloca i32, align 4");
        line("  store i32 128, ptr %len.ptr, align 4");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_local_addr_family(i64 %fd) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %query");
        line("query:");
        ...
        line("  br i1 %is.v4, label %v4, label %check_v6");
        line("check_v6:");
        line("  %is.v6 = icmp eq i16 %family, 10");
        line("  br i1 %is.v6, label %v6, label %fail");
        line("v4:");
        line("  ret i64 4");
        line("v6:");
        line("  ret i64 6");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_peer_addr_family(i64 %fd) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %query");
        line("query:");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_local_addr_v4(i64 %fd) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %query");
        line("query:");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_local_addr_v6(i64 %fd, ptr %out) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.out = icmp eq ptr %out, null");
        line("  %invalid = or i1 %invalid.fd, %invalid.out");
        line("  br i1 %invalid, label %fail, label %query");
        line("query:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %storage = alloca [128 x i8], align 8");
        ...
        line("  %family = load i16, ptr %addr, align 2");
        line("  %is.v6 = icmp eq i16 %family, 10");
        line("  br i1 %is.v6, label %pack, label %fail");
        line("pack:");
        line("  %port.ptr = getelementptr inbounds i8, ptr %addr, i64 2");
        line("  %net.port = load i16, ptr %port.ptr, align 2");
        line("  %host.port = call i16 @ntohs(i16 %net.port)");
        line("  %s0.ptr = getelementptr inbounds i8, ptr %addr, i64 8");
        line("  %s0.net = load i16, ptr %s0.ptr, align 2");
        line("  %s0.host = call i16 @ntohs(i16 %s0.net)");
        line("  %out0 = getelementptr inbounds i16, ptr %out, i64 0");
        line("  store i16 %s0.host, ptr %out0, align 2");
        line("  %s1.ptr = getelementptr inbounds i8, ptr %addr, i64 10");
        line("  %s1.net = load i16, ptr %s1.ptr, align 2");
        line("  %s1.host = call i16 @ntohs(i16 %s1.net)");
        line("  %out1 = getelementptr inbounds i16, ptr %out, i64 1");
        line("  store i16 %s1.host, ptr %out1, align 2");
        line("  %s2.ptr = getelementptr inbounds i8, ptr %addr, i64 12");
        line("  %s2.net = load i16, ptr %s2.ptr, align 2");
        line("  %s2.host = call i16 @ntohs(i16 %s2.net)");
        line("  %out2 = getelementptr inbounds i16, ptr %out, i64 2");
        line("  store i16 %s2.host, ptr %out2, align 2");
        line("  %s3.ptr = getelementptr inbounds i8, ptr %addr, i64 14");
        line("  %s3.net = load i16, ptr %s3.ptr, align 2");
        line("  %s3.host = call i16 @ntohs(i16 %s3.net)");
        line("  %out3 = getelementptr inbounds i16, ptr %out, i64 3");
        line("  store i16 %s3.host, ptr %out3, align 2");
        line("  %s4.ptr = getelementptr inbounds i8, ptr %addr, i64 16");
        line("  %s4.net = load i16, ptr %s4.ptr, align 2");
        line("  %s4.host = call i16 @ntohs(i16 %s4.net)");
        line("  %out4 = getelementptr inbounds i16, ptr %out, i64 4");
        line("  store i16 %s4.host, ptr %out4, align 2");
        line("  %s5.ptr = getelementptr inbounds i8, ptr %addr, i64 18");
        line("  %s5.net = load i16, ptr %s5.ptr, align 2");
        line("  %s5.host = call i16 @ntohs(i16 %s5.net)");
        line("  %out5 = getelementptr inbounds i16, ptr %out, i64 5");
        line("  store i16 %s5.host, ptr %out5, align 2");
        line("  %s6.ptr = getelementptr inbounds i8, ptr %addr, i64 20");
        line("  %s6.net = load i16, ptr %s6.ptr, align 2");
        line("  %s6.host = call i16 @ntohs(i16 %s6.net)");
        line("  %out6 = getelementptr inbounds i16, ptr %out, i64 6");
        line("  store i16 %s6.host, ptr %out6, align 2");
        line("  %s7.ptr = getelementptr inbounds i8, ptr %addr, i64 22");
        line("  %s7.net = load i16, ptr %s7.ptr, align 2");
        line("  %s7.host = call i16 @ntohs(i16 %s7.net)");
        line("  %out7 = getelementptr inbounds i16, ptr %out, i64 7");
        line("  store i16 %s7.host, ptr %out7, align 2");
        line("  %wide = zext i16 %host.port to i64");
        line("  ret i64 %wide");
        line("fail:");
        line("  ret i64 -1");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_peer_addr_v6(i64 %fd, ptr %out) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.out = icmp eq ptr %out, null");
        line("  %invalid = or i1 %invalid.fd, %invalid.out");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_udp_bind_v6(i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port) {");
        line("entry:");
        line("  %fd32 = call i32 @socket(i32 10, i32 2, i32 0)");
        line("  %socket.bad = icmp slt i32 %fd32, 0");
        line("  br i1 %socket.bad, label %fail, label %bind");
        line("bind:");
        line("  %storage = alloca [28 x i8], align 4");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_net_udp_send_byte_to_v6(i64 %fd, i8 %value, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %send");
        line("send:");
        ...
        line("  %addr = getelementptr inbounds [28 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v6(ptr %addr, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port)");
        line("  %count = call i64 @sendto(i32 %fd32, ptr %byte.ptr, i64 1, i32 0, ptr %addr, i32 28)");
        line("  %ok = icmp eq i64 %count, 1");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_udp_send_to_v4(i64 %fd, ptr %data, i64 %len, i64 %a, i64 %b, i64 %c, i64 %d, i64 %port) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.len = icmp slt i64 %len, 0");
        line("  %invalid = or i1 %invalid.fd, %invalid.len");
        line("  br i1 %invalid, label %fail, label %send");
        line("send:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %storage = alloca [16 x i8], align 4");
        line("  %addr = getelementptr inbounds [16 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v4(ptr %addr, i64 %a, i64 %b, i64 %c, i64 %d, i64 %port)");
        line("  %count = call i64 @sendto(i32 %fd32, ptr %data, i64 %len, i32 0, ptr %addr, i32 16)");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_udp_send_to_v6(i64 %fd, ptr %data, i64 %len, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.len = icmp slt i64 %len, 0");
        line("  %invalid = or i1 %invalid.fd, %invalid.len");
        ...
        line("  %addr = getelementptr inbounds [28 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v6(ptr %addr, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port)");
        line("  %count = call i64 @sendto(i32 %fd32, ptr %data, i64 %len, i32 0, ptr %addr, i32 28)");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_net_udp_connect_v4(i64 %fd, i64 %a, i64 %b, i64 %c, i64 %d, i64 %port) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %connect");
        line("connect:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %storage = alloca [16 x i8], align 4");
        line("  %addr = getelementptr inbounds [16 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v4(ptr %addr, i64 %a, i64 %b, i64 %c, i64 %d, i64 %port)");
        line("  %code = call i32 @connect(i32 %fd32, ptr %addr, i32 16)");
        line("  %ok = icmp eq i32 %code, 0");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_net_udp_connect_v6(i64 %fd, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %connect");
        line("connect:");
        ...
        line("  %addr = getelementptr inbounds [28 x i8], ptr %storage, i64 0, i64 0");
        line("  call void @ari_runtime_net_sockaddr_v6(ptr %addr, i64 %s0, i64 %s1, i64 %s2, i64 %s3, i64 %s4, i64 %s5, i64 %s6, i64 %s7, i64 %port)");
        line("  %code = call i32 @connect(i32 %fd32, ptr %addr, i32 28)");
        line("  %ok = icmp eq i32 %code, 0");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_udp_send(i64 %fd, ptr %data, i64 %len) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.len = icmp slt i64 %len, 0");
        line("  %invalid = or i1 %invalid.fd, %invalid.len");
        ...
        line("send:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %count = call i64 @sendto(i32 %fd32, ptr %data, i64 %len, i32 0, ptr null, i32 0)");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_udp_recv(i64 %fd, ptr %data, i64 %len) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.len = icmp slt i64 %len, 0");
        line("  %invalid = or i1 %invalid.fd, %invalid.len");
        ...
        line("recv:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %count = call i64 @recvfrom(i32 %fd32, ptr %data, i64 %len, i32 0, ptr null, ptr null)");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_udp_recv_from(i64 %fd, ptr %data, i64 %len, i64 %flags, ptr %family.out, ptr %port.out, ptr %segments.out) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.len = icmp slt i64 %len, 0");
        line("  %family.missing = icmp eq ptr %family.out, null");
        line("  %port.missing = icmp eq ptr %port.out, null");
        line("  %segments.missing = icmp eq ptr %segments.out, null");
        line("  %invalid.a = or i1 %invalid.fd, %invalid.len");
        line("  %invalid.b = or i1 %family.missing, %port.missing");
        line("  %invalid.c = or i1 %segments.missing, %invalid.a");
        line("  %invalid = or i1 %invalid.b, %invalid.c");
        line("  br i1 %invalid, label %fail, label %recv");
        line("recv:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %flags32 = trunc i64 %flags to i32");
        line("  %storage = alloca [28 x i8], align 4");
        line("  %addr = getelementptr inbounds [28 x i8], ptr %storage, i64 0, i64 0");
        line("  %len.ptr = alloca i32, align 4");
        line("  store i32 28, ptr %len.ptr, align 4");
        line("  %count = call i64 @recvfrom(i32 %fd32, ptr %data, i64 %len, i32 %flags32, ptr %addr, ptr %len.ptr)");
        line("  %bad.count = icmp slt i64 %count, 0");
        line("  br i1 %bad.count, label %fail, label %load_family");
        line("load_family:");
        line("  %family.ptr = getelementptr inbounds i8, ptr %addr, i64 0");
        line("  %family16 = load i16, ptr %family.ptr, align 2");
        line("  %is.v4 = icmp eq i16 %family16, 2");
        line("  br i1 %is.v4, label %store_v4, label %check_v6");
        line("check_v6:");
        line("  %is.v6 = icmp eq i16 %family16, 10");
        line("  br i1 %is.v6, label %store_v6, label %fail");
        line("store_v4:");
        line("  store i64 4, ptr %family.out, align 8");
        line("  %v4.port.ptr = getelementptr inbounds i8, ptr %addr, i64 2");
        line("  %v4.port.net = load i16, ptr %v4.port.ptr, align 2");
        line("  %v4.port.host = call i16 @ntohs(i16 %v4.port.net)");
        line("  %v4.port64 = zext i16 %v4.port.host to i64");
        line("  store i64 %v4.port64, ptr %port.out, align 8");
        line("  %v4.addr.ptr = getelementptr inbounds i8, ptr %addr, i64 4");
        line("  %v4.addr.net = load i32, ptr %v4.addr.ptr, align 4");
        line("  %v4.addr.host = call i32 @ntohl(i32 %v4.addr.net)");
        line("  %v4.a32 = lshr i32 %v4.addr.host, 24");
        line("  %v4.bshift = lshr i32 %v4.addr.host, 16");
        line("  %v4.cshift = lshr i32 %v4.addr.host, 8");
        line("  %v4.a16 = trunc i32 %v4.a32 to i16");
        line("  %v4.b32 = and i32 %v4.bshift, 255");
        line("  %v4.b16 = trunc i32 %v4.b32 to i16");
        line("  %v4.c32 = and i32 %v4.cshift, 255");
        line("  %v4.c16 = trunc i32 %v4.c32 to i16");
        line("  %v4.d32 = and i32 %v4.addr.host, 255");
        line("  %v4.d16 = trunc i32 %v4.d32 to i16");
        line("  %out0 = getelementptr inbounds i16, ptr %segments.out, i64 0");
        line("  store i16 %v4.a16, ptr %out0, align 2");
        line("  %out1 = getelementptr inbounds i16, ptr %segments.out, i64 1");
        line("  store i16 %v4.b16, ptr %out1, align 2");
        line("  %out2 = getelementptr inbounds i16, ptr %segments.out, i64 2");
        line("  store i16 %v4.c16, ptr %out2, align 2");
        line("  %out3 = getelementptr inbounds i16, ptr %segments.out, i64 3");
        line("  store i16 %v4.d16, ptr %out3, align 2");
        line("  ret i64 %count");
        line("store_v6:");
        line("  store i64 6, ptr %family.out, align 8");
        line("  %v6.port.ptr = getelementptr inbounds i8, ptr %addr, i64 2");
        line("  %v6.port.net = load i16, ptr %v6.port.ptr, align 2");
        line("  %v6.port.host = call i16 @ntohs(i16 %v6.port.net)");
        line("  %v6.port64 = zext i16 %v6.port.host to i64");
        line("  store i64 %v6.port64, ptr %port.out, align 8");
        line("  %s0.ptr = getelementptr inbounds i8, ptr %addr, i64 8");
        line("  %s0.net = load i16, ptr %s0.ptr, align 2");
        line("  %s0.host = call i16 @ntohs(i16 %s0.net)");
        line("  %v6.out0 = getelementptr inbounds i16, ptr %segments.out, i64 0");
        line("  store i16 %s0.host, ptr %v6.out0, align 2");
        line("  %s1.ptr = getelementptr inbounds i8, ptr %addr, i64 10");
        line("  %s1.net = load i16, ptr %s1.ptr, align 2");
        line("  %s1.host = call i16 @ntohs(i16 %s1.net)");
        line("  %v6.out1 = getelementptr inbounds i16, ptr %segments.out, i64 1");
        line("  store i16 %s1.host, ptr %v6.out1, align 2");
        line("  %s2.ptr = getelementptr inbounds i8, ptr %addr, i64 12");
        line("  %s2.net = load i16, ptr %s2.ptr, align 2");
        line("  %s2.host = call i16 @ntohs(i16 %s2.net)");
        line("  %v6.out2 = getelementptr inbounds i16, ptr %segments.out, i64 2");
        line("  store i16 %s2.host, ptr %v6.out2, align 2");
        line("  %s3.ptr = getelementptr inbounds i8, ptr %addr, i64 14");
        line("  %s3.net = load i16, ptr %s3.ptr, align 2");
        line("  %s3.host = call i16 @ntohs(i16 %s3.net)");
        line("  %v6.out3 = getelementptr inbounds i16, ptr %segments.out, i64 3");
        line("  store i16 %s3.host, ptr %v6.out3, align 2");
        line("  %s4.ptr = getelementptr inbounds i8, ptr %addr, i64 16");
        line("  %s4.net = load i16, ptr %s4.ptr, align 2");
        line("  %s4.host = call i16 @ntohs(i16 %s4.net)");
        line("  %v6.out4 = getelementptr inbounds i16, ptr %segments.out, i64 4");
        line("  store i16 %s4.host, ptr %v6.out4, align 2");
        line("  %s5.ptr = getelementptr inbounds i8, ptr %addr, i64 18");
        line("  %s5.net = load i16, ptr %s5.ptr, align 2");
        line("  %s5.host = call i16 @ntohs(i16 %s5.net)");
        line("  %v6.out5 = getelementptr inbounds i16, ptr %segments.out, i64 5");
        line("  store i16 %s5.host, ptr %v6.out5, align 2");
        line("  %s6.ptr = getelementptr inbounds i8, ptr %addr, i64 20");
        line("  %s6.net = load i16, ptr %s6.ptr, align 2");
        line("  %s6.host = call i16 @ntohs(i16 %s6.net)");
        line("  %v6.out6 = getelementptr inbounds i16, ptr %segments.out, i64 6");
        line("  store i16 %s6.host, ptr %v6.out6, align 2");
        line("  %s7.ptr = getelementptr inbounds i8, ptr %addr, i64 22");
        line("  %s7.net = load i16, ptr %s7.ptr, align 2");
        line("  %s7.host = call i16 @ntohs(i16 %s7.net)");
        line("  %v6.out7 = getelementptr inbounds i16, ptr %segments.out, i64 7");
        line("  store i16 %s7.host, ptr %v6.out7, align 2");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define private i64 @ari_runtime_net_int_option(i64 %fd, i32 %level, i32 %option) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %get");
        line("get:");
        ...
        line("load:");
        line("  %value = load i32, ptr %value.ptr, align 4");
        line("  %wide = sext i32 %value to i64");
        line("  ret i64 %wide");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define private i1 @ari_runtime_net_set_int_option(i64 %fd, i32 %level, i32 %option, i64 %value) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.value = icmp slt i64 %value, 0");
        line("  %invalid = or i1 %invalid.fd, %invalid.value");
        line("  br i1 %invalid, label %fail, label %set");
        line("set:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %value.ptr = alloca i32, align 4");
        line("  %value32 = trunc i64 %value to i32");
        line("  store i32 %value32, ptr %value.ptr, align 4");
        line("  %code = call i32 @setsockopt(i32 %fd32, i32 %level, i32 %option, ptr %value.ptr, i32 4)");
        line("  %ok = icmp eq i32 %code, 0");
        line("  ret i1 %ok");
        line("fail:");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_reuse_port(i64 %fd) {");
        line("entry:");
        line("  %value = call i64 @ari_runtime_net_bool_option(i64 %fd, i32 1, i32 15)");
        line("  ret i64 %value");
        line("}");
        line();

        line("define " + runtime_visibility + "i1 @ari_builtin_net_set_reuse_port(i64 %fd, i1 %enabled) {");
        line("entry:");
        line("  %ok = call i1 @ari_runtime_net_set_bool_option(i64 %fd, i32 1, i32 15, i1 %enabled)");
        line("  ret i1 %ok");
        line("}");
        line();

        line("define " + runtime_visibility + "i64 @ari_builtin_net_keepalive(i64 %fd) {");
        line("entry:");
        line("  %value = call i64 @ari_runtime_net_bool_option(i64 %fd, i32 1, i32 9)");
        line("  ret i64 %value");
        line("}");
        line();

        line("define " + runtime_visibility + "i1 @ari_builtin_net_set_keepalive(i64 %fd, i1 %enabled) {");
        line("entry:");
        line("  %ok = call i1 @ari_runtime_net_set_bool_option(i64 %fd, i32 1, i32 9, i1 %enabled)");
        line("  ret i1 %ok");
        line("}");
        line();

        line("define " + runtime_visibility + "i64 @ari_builtin_net_broadcast(i64 %fd) {");
        line("entry:");
        line("  %value = call i64 @ari_runtime_net_bool_option(i64 %fd, i32 1, i32 6)");
        line("  ret i64 %value");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_send_buffer_size(i64 %fd) {");
        line("entry:");
        line("  %value = call i64 @ari_runtime_net_int_option(i64 %fd, i32 1, i32 7)");
        line("  ret i64 %value");
        line("}");
        line();

        line("define " + runtime_visibility + "i1 @ari_builtin_net_set_send_buffer_size(i64 %fd, i64 %value) {");
        line("entry:");
        line("  %ok = call i1 @ari_runtime_net_set_int_option(i64 %fd, i32 1, i32 7, i64 %value)");
        line("  ret i1 %ok");
        line("}");
        line();

        line("define " + runtime_visibility + "i64 @ari_builtin_net_recv_buffer_size(i64 %fd) {");
        line("entry:");
        line("  %value = call i64 @ari_runtime_net_int_option(i64 %fd, i32 1, i32 8)");
        line("  ret i64 %value");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_unix_datagram_bind(ptr %path) {");
        line("entry:");
        line("  %storage = alloca [110 x i8], align 2");
        line("  %addr = getelementptr inbounds [110 x i8], ptr %storage, i64 0, i64 0");
        line("  %addr.len = call i64 @ari_runtime_net_sockaddr_un(ptr %addr, ptr %path)");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_net_unix_datagram_connect(i64 %fd, ptr %path) {");
        line("entry:");
        line("  %invalid = icmp slt i64 %fd, 0");
        line("  br i1 %invalid, label %fail, label %build");
        line("build:");
        line("  %storage = alloca [110 x i8], align 2");
        line("  %addr = getelementptr inbounds [110 x i8], ptr %storage, i64 0, i64 0");
        line("  %addr.len = call i64 @ari_runtime_net_sockaddr_un(ptr %addr, ptr %path)");
        line("  %bad.addr = icmp slt i64 %addr.len, 0");
        line("  br i1 %bad.addr, label %fail, label %connect");
        line("connect:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %addr.len32 = trunc i64 %addr.len to i32");
        line("  %connect.code = call i32 @connect(i32 %fd32, ptr %addr, i32 %addr.len32)");
        line("  %ok = icmp eq i32 %connect.code, 0");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_unix_datagram_send_to(i64 %fd, ptr %data, i64 %len, ptr %path) {");
        line("entry:");
        line("  %invalid.fd = icmp slt i64 %fd, 0");
        line("  %invalid.len = icmp slt i64 %len, 0");
        line("  %invalid = or i1 %invalid.fd, %invalid.len");
        ...
        line("  %addr.len = call i64 @ari_runtime_net_sockaddr_un(ptr %addr, ptr %path)");
        line("  %bad.addr = icmp slt i64 %addr.len, 0");
        line("  br i1 %bad.addr, label %fail, label %send");
        line("send:");
        line("  %fd32 = trunc i64 %fd to i32");
        line("  %addr.len32 = trunc i64 %addr.len to i32");
        line("  %count = call i64 @sendto(i32 %fd32, ptr %data, i64 %len, i32 0, ptr %addr, i32 %addr.len32)");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_unix_listen_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        line("  %missing = icmp eq ptr %path, null");
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %fd = call i64 @ari_builtin_net_unix_listen(ptr %path)");
        line("  ret i64 %fd");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_unix_connect_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %fd = call i64 @ari_builtin_net_unix_connect(ptr %path)");
        line("  ret i64 %fd");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_unix_datagram_bind_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %fd = call i64 @ari_builtin_net_unix_datagram_bind(ptr %path)");
        line("  ret i64 %fd");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i1 @ari_builtin_net_unix_datagram_connect_bytes(i64 %fd, ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %ok = call i1 @ari_builtin_net_unix_datagram_connect(i64 %fd, ptr %path)");
        line("  ret i1 %ok");
        line("fail:");
        line("  ret i1 false");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_unix_datagram_send_to_bytes(i64 %fd, ptr %values, i64 %values_len, ptr %path_data, i64 %path_len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %path = call ptr @ari_runtime_fs_path_from_bytes(ptr %path_data, i64 %path_len, ptr %buffer)");
        line("  %missing = icmp eq ptr %path, null");
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %count = call i64 @ari_builtin_net_unix_datagram_send_to(i64 %fd, ptr %values, i64 %values_len, ptr %path)");
        line("  ret i64 %count");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v6(ptr %host, i64 %port, ptr %out) {");
        line("entry:");
        line("  %out.missing = icmp eq ptr %out, null");
        line("  br i1 %out.missing, label %fail, label %lookup");
        line("lookup:");
        line("  %hints.storage = alloca [48 x i8], align 8");
        line("  %hints = getelementptr inbounds [48 x i8], ptr %hints.storage, i64 0, i64 0");
        line("  call void @llvm.memset.p0.i64(ptr %hints, i8 0, i64 48, i1 false)");
        line("  %family.ptr = getelementptr inbounds i8, ptr %hints, i64 4");
        line("  store i32 10, ptr %family.ptr, align 4");
        line("  %result.ptr = alloca ptr, align 8");
        line("  store ptr null, ptr %result.ptr, align 8");
        line("  %code = call i32 @getaddrinfo(ptr %host, ptr null, ptr %hints, ptr %result.ptr)");
        line("  %ok = icmp eq i32 %code, 0");
        ...
        line("  %res.family.ptr = getelementptr inbounds i8, ptr %result, i64 4");
        line("  %family = load i32, ptr %res.family.ptr, align 4");
        line("  %is.v6 = icmp eq i32 %family, 10");
        line("  br i1 %is.v6, label %load_addr, label %free_fail");
        line("load_addr:");
        line("  %sockaddr.slot = getelementptr inbounds i8, ptr %result, i64 24");
        line("  %sockaddr = load ptr, ptr %sockaddr.slot, align 8");
        line("  %addr.missing = icmp eq ptr %sockaddr, null");
        line("  br i1 %addr.missing, label %free_fail, label %pack");
        line("pack:");
        line("  %s0.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 8");
        line("  %s0.net = load i16, ptr %s0.ptr, align 2");
        line("  %s0.host = call i16 @ntohs(i16 %s0.net)");
        line("  %out0 = getelementptr inbounds i16, ptr %out, i64 0");
        line("  store i16 %s0.host, ptr %out0, align 2");
        line("  %s1.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 10");
        line("  %s1.net = load i16, ptr %s1.ptr, align 2");
        line("  %s1.host = call i16 @ntohs(i16 %s1.net)");
        line("  %out1 = getelementptr inbounds i16, ptr %out, i64 1");
        line("  store i16 %s1.host, ptr %out1, align 2");
        line("  %s2.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 12");
        line("  %s2.net = load i16, ptr %s2.ptr, align 2");
        line("  %s2.host = call i16 @ntohs(i16 %s2.net)");
        line("  %out2 = getelementptr inbounds i16, ptr %out, i64 2");
        line("  store i16 %s2.host, ptr %out2, align 2");
        line("  %s3.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 14");
        line("  %s3.net = load i16, ptr %s3.ptr, align 2");
        line("  %s3.host = call i16 @ntohs(i16 %s3.net)");
        line("  %out3 = getelementptr inbounds i16, ptr %out, i64 3");
        line("  store i16 %s3.host, ptr %out3, align 2");
        line("  %s4.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 16");
        line("  %s4.net = load i16, ptr %s4.ptr, align 2");
        line("  %s4.host = call i16 @ntohs(i16 %s4.net)");
        line("  %out4 = getelementptr inbounds i16, ptr %out, i64 4");
        line("  store i16 %s4.host, ptr %out4, align 2");
        line("  %s5.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 18");
        line("  %s5.net = load i16, ptr %s5.ptr, align 2");
        line("  %s5.host = call i16 @ntohs(i16 %s5.net)");
        line("  %out5 = getelementptr inbounds i16, ptr %out, i64 5");
        line("  store i16 %s5.host, ptr %out5, align 2");
        line("  %s6.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 20");
        line("  %s6.net = load i16, ptr %s6.ptr, align 2");
        line("  %s6.host = call i16 @ntohs(i16 %s6.net)");
        line("  %out6 = getelementptr inbounds i16, ptr %out, i64 6");
        line("  store i16 %s6.host, ptr %out6, align 2");
        line("  %s7.ptr = getelementptr inbounds i8, ptr %sockaddr, i64 22");
        line("  %s7.net = load i16, ptr %s7.ptr, align 2");
        line("  %s7.host = call i16 @ntohs(i16 %s7.net)");
        line("  %out7 = getelementptr inbounds i16, ptr %out, i64 7");
        line("  store i16 %s7.host, ptr %out7, align 2");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v4_endpoint(ptr %endpoint) {");
        line("entry:");
        line("  %len = call i64 @strlen(ptr %endpoint)");
        line("  %empty = icmp eq i64 %len, 0");
        line("  br i1 %empty, label %fail, label %scan");
        line("scan:");
        line("  %scan.index = phi i64 [ 0, %entry ], [ %scan.next, %scan_continue ]");
        line("  %scan.done = icmp eq i64 %scan.index, %len");
        line("  br i1 %scan.done, label %fail, label %scan_load");
        line("scan_load:");
        line("  %scan.ptr = getelementptr inbounds i8, ptr %endpoint, i64 %scan.index");
        line("  %scan.ch = load i8, ptr %scan.ptr, align 1");
        line("  %is.colon = icmp eq i8 %scan.ch, 58");
        line("  br i1 %is.colon, label %check_colon, label %scan_continue");
        line("scan_continue:");
        line("  %scan.next = add i64 %scan.index, 1");
        line("  br label %scan");
        line("check_colon:");
        line("  %host.empty = icmp eq i64 %scan.index, 0");
        line("  %port.start = add i64 %scan.index, 1");
        line("  %port.empty = icmp eq i64 %port.start, %len");
        line("  %bad.edge = or i1 %host.empty, %port.empty");
        line("  br i1 %bad.edge, label %fail, label %parse_port");
        line("parse_port:");
        line("  %port.index = phi i64 [ %port.start, %check_colon ], [ %port.next.index, %port_continue ]");
        line("  %port.value = phi i64 [ 0, %check_colon ], [ %port.next.value, %port_continue ]");
        line("  %port.done = icmp eq i64 %port.index, %len");
        line("  br i1 %port.done, label %copy_host, label %parse_digit");
        line("parse_digit:");
        line("  %port.ptr = getelementptr inbounds i8, ptr %endpoint, i64 %port.index");
        line("  %port.ch = load i8, ptr %port.ptr, align 1");
        line("  %digit.low = icmp uge i8 %port.ch, 48");
        line("  %digit.high = icmp ule i8 %port.ch, 57");
        line("  %is.digit = and i1 %digit.low, %digit.high");
        line("  br i1 %is.digit, label %accumulate_port, label %fail");
        line("accumulate_port:");
        line("  %digit.i8 = sub i8 %port.ch, 48");
        line("  %digit = zext i8 %digit.i8 to i64");
        line("  %port.mul = mul i64 %port.value, 10");
        line("  %port.next.value = add i64 %port.mul, %digit");
        line("  %port.too.large = icmp ugt i64 %port.next.value, 65535");
        line("  br i1 %port.too.large, label %fail, label %port_continue");
        line("port_continue:");
        line("  %port.next.index = add i64 %port.index, 1");
        line("  br label %parse_port");
        line("copy_host:");
        line("  %host.alloc.len = add i64 %scan.index, 1");
        line("  %host.copy = call ptr @malloc(i64 %host.alloc.len)");
        line("  %host.missing = icmp eq ptr %host.copy, null");
        line("  br i1 %host.missing, label %fail, label %copy_host_bytes");
        line("copy_host_bytes:");
        line("  call void @llvm.memcpy.p0.p0.i64(ptr %host.copy, ptr %endpoint, i64 %scan.index, i1 false)");
        line("  %nul.ptr = getelementptr inbounds i8, ptr %host.copy, i64 %scan.index");
        line("  store i8 0, ptr %nul.ptr, align 1");
        line("  %packed = call i64 @ari_builtin_net_lookup_v4(ptr %host.copy, i64 %port.value)");
        line("  call void @free(ptr %host.copy)");
        line("  ret i64 %packed");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v6_endpoint(ptr %endpoint, ptr %out) {");
        line("entry:");
        line("  %out.missing = icmp eq ptr %out, null");
        line("  br i1 %out.missing, label %fail, label %check_len");
        line("check_len:");
        line("  %len = call i64 @strlen(ptr %endpoint)");
        line("  %too.short = icmp ult i64 %len, 5");
        line("  br i1 %too.short, label %fail, label %check_open");
        line("check_open:");
        line("  %first = load i8, ptr %endpoint, align 1");
        line("  %is.open = icmp eq i8 %first, 91");
        line("  br i1 %is.open, label %scan, label %fail");
        line("scan:");
        line("  %scan.index = phi i64 [ 1, %check_open ], [ %scan.next, %scan_continue ]");
        line("  %scan.done = icmp eq i64 %scan.index, %len");
        line("  br i1 %scan.done, label %fail, label %scan_load");
        line("scan_load:");
        line("  %scan.ptr = getelementptr inbounds i8, ptr %endpoint, i64 %scan.index");
        line("  %scan.ch = load i8, ptr %scan.ptr, align 1");
        line("  %is.close = icmp eq i8 %scan.ch, 93");
        line("  br i1 %is.close, label %check_close, label %scan_continue");
        line("scan_continue:");
        line("  %scan.next = add i64 %scan.index, 1");
        line("  br label %scan");
        line("check_close:");
        line("  %host.empty = icmp eq i64 %scan.index, 1");
        line("  %colon.index = add i64 %scan.index, 1");
        line("  %colon.missing = icmp uge i64 %colon.index, %len");
        line("  %bad.close = or i1 %host.empty, %colon.missing");
        line("  br i1 %bad.close, label %fail, label %check_colon");
        line("check_colon:");
        line("  %colon.ptr = getelementptr inbounds i8, ptr %endpoint, i64 %colon.index");
        line("  %colon.ch = load i8, ptr %colon.ptr, align 1");
        line("  %is.colon = icmp eq i8 %colon.ch, 58");
        line("  br i1 %is.colon, label %parse_port_entry, label %fail");
        line("parse_port_entry:");
        line("  %port.start = add i64 %colon.index, 1");
        line("  %port.empty = icmp eq i64 %port.start, %len");
        line("  br i1 %port.empty, label %fail, label %parse_port");
        line("parse_port:");
        line("  %port.index = phi i64 [ %port.start, %parse_port_entry ], [ %port.next.index, %port_continue ]");
        line("  %port.value = phi i64 [ 0, %parse_port_entry ], [ %port.next.value, %port_continue ]");
        line("  %port.done = icmp eq i64 %port.index, %len");
        line("  br i1 %port.done, label %copy_host, label %parse_digit");
        line("parse_digit:");
        line("  %port.ptr = getelementptr inbounds i8, ptr %endpoint, i64 %port.index");
        ...
        line("  br label %parse_port");
        line("copy_host:");
        line("  %host.len = sub i64 %scan.index, 1");
        line("  %host.alloc.len = add i64 %host.len, 1");
        line("  %host.copy = call ptr @malloc(i64 %host.alloc.len)");
        line("  %host.missing = icmp eq ptr %host.copy, null");
        line("  br i1 %host.missing, label %fail, label %copy_host_bytes");
        line("copy_host_bytes:");
        line("  %host.source = getelementptr inbounds i8, ptr %endpoint, i64 1");
        line("  call void @llvm.memcpy.p0.p0.i64(ptr %host.copy, ptr %host.source, i64 %host.len, i1 false)");
        line("  %nul.ptr = getelementptr inbounds i8, ptr %host.copy, i64 %host.len");
        line("  store i8 0, ptr %nul.ptr, align 1");
        line("  %resolved.port = call i64 @ari_builtin_net_lookup_v6(ptr %host.copy, i64 %port.value, ptr %out)");
        line("  call void @free(ptr %host.copy)");
        line("  ret i64 %resolved.port");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        line();

        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v4_bytes(ptr %data, i64 %len, i64 %port) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %host = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        line("  %missing = icmp eq ptr %host, null");
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %packed = call i64 @ari_builtin_net_lookup_v4(ptr %host, i64 %port)");
        line("  ret i64 %packed");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v6_bytes(ptr %data, i64 %len, i64 %port, ptr %out) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %host = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %resolved = call i64 @ari_builtin_net_lookup_v6(ptr %host, i64 %port, ptr %out)");
        line("  ret i64 %resolved");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        line();

        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v4_endpoint_bytes(ptr %data, i64 %len) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %endpoint = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        line("  %missing = icmp eq ptr %endpoint, null");
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %packed = call i64 @ari_builtin_net_lookup_v4_endpoint(ptr %endpoint)");
        line("  ret i64 %packed");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define " + runtime_visibility + "i64 @ari_builtin_net_lookup_v6_endpoint_bytes(ptr %data, i64 %len, ptr %out) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %endpoint = call ptr @ari_runtime_fs_path_from_bytes(ptr %data, i64 %len, ptr %buffer)");
        ...
        line("  br i1 %missing, label %fail, label %call");
        line("call:");
        line("  %resolved = call i64 @ari_builtin_net_lookup_v6_endpoint(ptr %endpoint, ptr %out)");
        line("  ret i64 %resolved");
        line("fail:");
        line("  ret i64 -1");
        line("}");
        ...
        line("define private ptr @ari_thread_trampoline_raw(ptr %packet) {");
        line("entry:");
        line("  %thread.entry = load ptr, ptr %packet, align 8");
        line("  %data.ptr = getelementptr i8, ptr %packet, i64 8");
        line("  %data = load ptr, ptr %data.ptr, align 8");
        line("  %id.ptr = getelementptr i8, ptr %packet, i64 16");
        line("  %id = load i64, ptr %id.ptr, align 8");
        line("  store i64 %id, ptr @ari_thread_id");
        line("  call void @free(ptr %packet)");
        line("  %result = call i64 %thread.entry(ptr %data)");
        line("  %as.ptr = inttoptr i64 %result to ptr");
        line("  ret ptr %as.ptr");
        line("}");
        line();
        ...
        line("define " + runtime_visibility + "{ i64, i64 } @ari_builtin_thread_spawn_raw(ptr %start, ptr %data) {");
        line("entry:");
        line("  %thread = call { i64, i64 } @ari_builtin_thread_spawn_raw_configured(ptr %start, ptr %data, ptr null, i64 0)");
        line("  ret { i64, i64 } %thread");
        line("}");
        line();

        line("define " + runtime_visibility + "{ i64, i64 } @ari_builtin_thread_spawn_raw_configured(ptr %start, ptr %data, ptr %name, i64 %stack_size) {");
        line("entry:");
        line("  %native.ptr = alloca i64, align 8");
        line("  %attr.storage = alloca [128 x i8], align 8");
        line("  %attr.ptr = getelementptr [128 x i8], ptr %attr.storage, i64 0, i64 0");
        line("  %packet = call ptr @malloc(i64 24)");
        line("  %alloc.failed = icmp eq ptr %packet, null");
        line("  br i1 %alloc.failed, label %fail, label %prepare");
        line("prepare:");
        line("  store ptr %start, ptr %packet, align 8");
        line("  %data.ptr = getelementptr i8, ptr %packet, i64 8");
        line("  store ptr %data, ptr %data.ptr, align 8");
        line("  %id.ptr = getelementptr i8, ptr %packet, i64 16");
        line("  %id = atomicrmw add ptr @ari_next_thread_id, i64 1 monotonic");
        line("  store i64 %id, ptr %id.ptr, align 8");
        line("  %use.attr = icmp sgt i64 %stack_size, 0");
        ...
        line("  br i1 %attr.stack.failed, label %attr_fail, label %create_attr");
        line("create_attr:");
        line("  %code.attr = call i32 @pthread_create(ptr %native.ptr, ptr %attr.ptr, ptr @ari_thread_trampoline_raw, ptr %packet)");
        line("  %attr.destroy.after = call i32 @pthread_attr_destroy(ptr %attr.ptr)");
        line("  %create.attr.failed = icmp ne i32 %code.attr, 0");
        line("  br i1 %create.attr.failed, label %create_fail, label %ok");
        line("attr_fail:");
        ...
        line("  br label %create_fail");
        line("create_plain:");
        line("  %code = call i32 @pthread_create(ptr %native.ptr, ptr null, ptr @ari_thread_trampoline_raw, ptr %packet)");
        line("  %create.failed = icmp ne i32 %code, 0");
        line("  br i1 %create.failed, label %create_fail, label %ok");
        line("ok:");
        line("  %native = load i64, ptr %native.ptr, align 8");
        ...
        line("define " + runtime_visibility + "{ i64, i64 } @ari_builtin_thread_spawn_configured_bytes(ptr %start, ptr %name_data, i64 %name_len, i64 %stack_size) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %name = call ptr @ari_runtime_fs_path_from_bytes(ptr %name_data, i64 %name_len, ptr %buffer)");
        line("  %thread = call { i64, i64 } @ari_builtin_thread_spawn_configured(ptr %start, ptr %name, i64 %stack_size)");
        line("  ret { i64, i64 } %thread");
        line("}");
        line();

        line("define " + runtime_visibility + "{ i64, i64 } @ari_builtin_thread_spawn_raw_configured_bytes(ptr %start, ptr %data, ptr %name_data, i64 %name_len, i64 %stack_size) {");
        line("entry:");
        line("  %storage = alloca [4096 x i8], align 16");
        line("  %buffer = getelementptr inbounds [4096 x i8], ptr %storage, i64 0, i64 0");
        line("  %name = call ptr @ari_runtime_fs_path_from_bytes(ptr %name_data, i64 %name_len, ptr %buffer)");
        line("  %thread = call { i64, i64 } @ari_builtin_thread_spawn_raw_configured(ptr %start, ptr %data, ptr %name, i64 %stack_size)");
        line("  ret { i64, i64 } %thread");
        line("}");
        line();
