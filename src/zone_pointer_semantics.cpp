...
bool is_zone_pointer_trackable_type(const IrType& type) {
    ...
    return type.qualifier == TypeQualifier::Ptr ||
           ...
           is_std_sync_zone_handle_type(value_type) ||
...
bool zone_pointer_source_name_from_expr(const IrExpr& value,
    ...
    if (source.kind == IrExprKind::Tuple && is_std_process_output_zone_handle_type(source.type)) {
        bool found_any = false;
        for (const auto& arg : source.args) {
            merge_source(arg, found_any);
        }
    ...
    if (source.kind == IrExprKind::Tuple && is_std_sync_zone_handle_type(source.type)) {
        std::optional<std::size_t> source_index = std_sync_zone_handle_source_field_index(source.type);
        if (!source_index || *source_index >= source.args.size()) return false;
        return zone_pointer_source_name_from_expr(*source.args[*source_index], resolver, out);
    }
    if (source.kind == IrExprKind::Tuple && is_std_thread_zone_handle_type(source.type)) {
        std::optional<std::size_t> source_index = std_thread_zone_handle_source_field_index(source.type);
        if (!source_index || *source_index >= source.args.size()) return false;
        return zone_pointer_source_name_from_expr(*source.args[*source_index], resolver, out);
    }
    ...
    if (source.kind == IrExprKind::TupleIndex && ir_expr_operand(source)) {
        ...
        std::optional<std::size_t> sync_source_index =
            std_sync_zone_handle_source_field_index(operand.type);
        if (sync_source_index && source.tuple_index == *sync_source_index) {
            return zone_pointer_source_name_from_expr(operand, resolver, out);
        }
        std::optional<std::size_t> thread_source_index =
            std_thread_zone_handle_source_field_index(operand.type);
        if (thread_source_index && source.tuple_index == *thread_source_index) {
            return zone_pointer_source_name_from_expr(operand, resolver, out);
        }
