...
def main() -> int:
    ...
    for needle in [
        ...
        "empty files",
        "CRLF",
        "UTF-8 byte-column policy",
        "syntax-dump-basic.syntax",
        "diagnostic-type-assignment.diagnostic",
        "Source table",
        "Span validation",
        "Line table",
        "Snippet extraction",
    ...
    for needle in [
        ...
        "ari --explain-diagnostic P0001",
        "python3 tests/check_compiler_diagnostic_cli.py",
    ...
    for needle in [
        ...
        "`--list-passes` prints compiler pass owners",
        "`--explain-pass sema` prints the sema pass contract",
        "`--list-test-buckets` prints compiler fixture buckets",
        "`--explain-test-bucket compiler-artifact-ok` prints when to use",
        "`--list-work-items` prints compiler implementation slices",
        "`--explain-work-item generic-aggregate-stress` prints the next compiler work",
        "`--list-capabilities` prints the same compiler feature surface",
        "`--explain-capability trait-resolution` prints the owner",
        "`--list-artifacts` prints the available artifact producers",
        "`--explain-artifact --emit-tokens` prints the owner",
        "`--list-fixtures` writes a deterministic inventory",
        ...
        "`--emit-syntax` writes deterministic parser output with AST `source_id=`",
    ...
    for needle in [
        ...
        "double-move",
        "live-owner",
        "invalid enum payload-slot moves",
        "compact enum payload ref-pattern rejection",
        "ownership-carrying aggregate enum payloads",
        "borrow-carrying aggregate enum payloads",
        "moving owning aggregate elements through dynamic indexes",
        "temporary aggregate element owner moves",
