...
`KwImpl`, `KwFor`, `KwIn`, `KwLet`, `KwVar`, `KwOwn`, `KwRef`, `KwMut`, and `KwPtr`, plus longer `structure`, `external`,
`enumerate`, `traitor`, `dynamic`, `matches`, `module`, `public`, `user`,
`implicit`, `forest`, `inside`, `letter`, `variant`, `owner`, `reference`, `mutable`, and `ptrace`, and the source-text parser/driver
