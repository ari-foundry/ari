...
`KwImpl`, `KwFor`, `KwIn`, `KwLet`, `KwVar`, `KwOwn`, `KwRef`, `KwMut`, `KwPtr`, and `KwReturn`, plus longer `structure`, `external`,
`enumerate`, `traitor`, `dynamic`, `matches`, `module`, `public`, `user`,
`implicit`, `forest`, `inside`, `letter`, `variant`, `owner`, `reference`, `mutable`, `ptrace`, and `returning`, and the source-text parser/driver
