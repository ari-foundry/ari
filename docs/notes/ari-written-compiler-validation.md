...
`KwContinue`, `KwBreak`, `KwDrop`, `KwForget`, and `KwNull`, plus longer
...
`forget1`, and `null1`, and the source-text parser/driver keyword path without
...
smoke checked successful statement spans without requiring a hosted compiler
fix. The AST node value query and
parser payload-value smoke checked successful statement
