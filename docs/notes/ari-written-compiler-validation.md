...
The lexer one-character result path review removed the duplicated full-token
construction match from `scan_one_result`. That function now reuses `scan_one`
...
path for public queries, and required no hosted compiler fix.
...
hosted compiler fix.
The parser-facing literal AST payload smoke checked that numeric statement
nodes preserve literal base, suffix rank, checked integer/float values, and
literal/suffix source spans through parser result accessors without requiring a
hosted compiler fix.
...
confirmed hosted compiler bug.
The parser-facing literal payload slice re-hit the same pressure with
`lexer::token::LiteralPayload` versus `ast::token::LiteralPayload`. The accepted
implementation stores an AST-owned primitive snapshot instead of passing nested
module payload types across phase boundaries. This is the same cross-module
type identity pressure, not a confirmed hosted compiler bug.
...
- Lexer diagnostic wrappers should stay thin over the token scanner when they
...
- String and byte-character scan loops should keep the byte currently under
  ...
  scanning now follow this shape, and the shared string/byte-character escape
  helper loops now use local current bytes for hex/octal digit and braced
  Unicode checks. This is not a hosted compiler bug.
- Diagnostic metadata should avoid separate full-kind matches for rank, public
  ...
  names the newer diagnostic identity.
