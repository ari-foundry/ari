...
- `compiler/lexer.ari` classifies shift compound-assignment operators `<<=`
  ...
  `>>`, `<=`, and `>=`.
- `compiler/lexer.ari` classifies exact source-text `fn` and `let` as keywords
  while preserving longer identifier runs such as `fn1` and `letter` as
  identifiers.
...
- Add text-backed keyword classification for stage0 `var`, preserving longer
  identifiers such as `variant` as identifiers.
