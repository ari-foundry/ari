...
- Backfilled reusable keyword-table smoke coverage for existing `KwLet`, with
  ...
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwVar`, with
  focused smokes for exact `var`, longer `variant`, and the table-backed
  parser/driver keyword path.
...
- Backfilled reusable keyword-table smoke coverage for existing `KwReturn`,
  ...
  table-backed parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwIf`, with
  focused smokes for exact `if`, longer `iffy`, and the table-backed
  parser/driver keyword path.
...
- Backfilled reusable keyword-table smoke coverage for existing `KwWhile`, with
  ...
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwInit`, with
  focused smokes for exact `init`, longer `initial`, and the table-backed
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwNext`, with
  focused smokes for exact `next`, longer `next1`, and the table-backed
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwContinue`,
  with focused smokes for exact `continue`, longer `continue1`, and the
  table-backed parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwBreak`, with
  focused smokes for exact `break`, longer `break1`, and the table-backed
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwDrop`, with
  focused smokes for exact `drop`, longer `drop1`, and the table-backed
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwForget`,
  with focused smokes for exact `forget`, longer `forget1`, and the
  table-backed parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwNull`, with
  focused smokes for exact `null`, longer `null1`, and the table-backed
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwTrue`, with
  focused smokes for exact `true`, longer `true1`, and the table-backed
  parser/driver keyword path.
- Backfilled reusable keyword-table smoke coverage for existing `KwFalse`, with
  focused smokes for exact `false`, longer `false1`, and the table-backed
  parser/driver keyword path.
...
- Added source-text unsupported byte character escape diagnostics for spellings
  ...
  source-text paths.
- Added source-text byte character escape digit-shape diagnostics for spellings
...
- Added source-text post-comment extra-token diagnostics after a statement
  token, checking line and block comments followed by another identifier
  through lexer handoff, parser, and driver paths.
- Added source-text post-comment lexer-failure diagnostics after a statement
  token, checking line and block comments followed by an unterminated string
  through lexer handoff, parser, and driver paths.
- Added source-text leading-comment parser and driver handoff coverage before a
  statement token, checking line and block comments followed by a valid
  identifier through parser and driver paths.
- Added source-text leading-comment lexer-failure diagnostics before a
  statement token, checking line and block comments followed by an unterminated
  string through lexer handoff, parser, and driver paths.
- Added source-text leading-comment extra-token diagnostics after a statement
  token, checking line and block comments before a statement followed by
  another identifier through lexer handoff, parser, and driver paths, and
  making parser missing-EOF diagnostics point at the extra token span.
...
- Added source-text leading-comment punctuation diagnostics before a statement
...
- Added source-text leading-comment numeric missing-base-digit diagnostics
  before a statement token, checking line and block comments followed by `0x`
  through lexer handoff, parser, keyword-table parser, and driver paths.
- Added source-text leading-comment invalid base-digit diagnostics before a
  statement token, checking line and block comments followed by `0b102`
  through lexer handoff, parser, keyword-table parser, and driver paths.
- Added source-text leading-comment unsupported number-suffix diagnostics
  before a statement token, checking line and block comments followed by
  `42abc` through lexer handoff, parser, keyword-table parser, and driver
  paths.
...
- Added source-text leading-comment byte-character escape spans before a
  ...
  lexer handoff, parser, keyword-table parser, and driver paths.
- Added source-text leading-comment byte-character numeric escape spans before
...
- Added CRLF post-comment source-text extra-token parser and driver coverage,
  checking `name// skip\r\nother` preserves parser missing-EOF diagnostic code
  `2003` and extra-token span `13..18` through parser and keyword-table parser
  paths, and `p// skip\r\nq` preserves driver diagnostic code `2003`.
...
- Added a source span start location summary helper in `compiler/source.ari`,
  checking source-id ownership and byte bounds before deriving the span start's
  line and byte-column metadata.
- Added a source span end location summary helper in `compiler/source.ari`,
  sharing the span/file validity check with the start helper and preserving EOF
  insertion-point behavior.
- Added a source-table entry span start location summary helper in
  `compiler/source.ari`, delegating through the entry's `SourceFile` so callers
  do not duplicate entry unwrapping or span ownership checks.
...
- Added a loaded-source span start location summary helper in
  `compiler/source.ari`, delegating through the loaded source's `SourceFile`
  so source ownership and byte-bound validation stay shared.

## Small Task Queue

- Keep `compiler/main.ari` thin; grow real entry behavior in `driver.ari` only
  when the underlying phases have checked handoff data.
- Add a loaded-source span end location summary helper in `compiler/source.ari`.
