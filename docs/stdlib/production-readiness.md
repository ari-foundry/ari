...
| `io`, `input`, `env`, `fs`, `process`, `thread`, `sync`, `time`, IPv4 `net` DNS/host-port/TCP/UDP and Unix stream handles | seed to usable | Stronger owned handle policy, richer `Result` error values, blocking sync internals, socket capability notes, IPv6 handles, multi-address DNS iteration, buffer-oriented net IO, and platform notes. |
