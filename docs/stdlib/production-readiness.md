...
| `io`, `input`, `env`, `fs`, `process`, `thread`, `sync`, `time`, IPv4/IPv6 `net` DNS/host-port/TCP/UDP and Unix stream handles | usable | Stronger owned handle policy, richer structured error values, blocking sync internals, socket readiness notes, full DNS iteration, platform-specific path/process/socket behavior, and platform notes. |
