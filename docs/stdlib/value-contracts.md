...
| `algo::fill(values, value)` / `slice.fill(value)` / `vec.fill(value)` | Stores the same `value` into every live slot. This is not a clone factory. It is valid for copyable/plain values. | Not relevant. | Overwritten owners are not a supported use case. Future APIs should separate `fill_clone` or `fill_with`. |
...
| `algo::swap`, `reverse`, `rotate_left`, `rotate_right`, `sort`, `sort_by`, `stable_sort`, `stable_sort_by`, `partition`, `stable_partition` | Reorders in place using raw element swaps or shifts. Requires copyable/plain `T` today. `*_by` comparators currently receive values by copy, so they are also copy-oriented. | Reordering is in-place within one slice. | Does not create or destroy logical elements. Future move-aware versions must use explicit take/store place operations. |
...
| `vec.drain()` | Immediately sets the vector length to zero and returns a cursor over the old live range. | Cursor owns the removed logical range. | Consumed items are returned by value. Unconsumed items are dropped when the drain cursor is dropped, so early stop is covered in normal control flow. No unwind guarantee exists yet. |
...
Avoid adding a helper whose name hides the contract. For example, a future
resource-safe grow operation should be named like `resize_with(make_value)` or
`extend_move(source)` rather than making today's `resize(length, value)` silently
consume one value many times.
