...
| `Set.retain(keep)`, `HashSet.retain(keep)`, `HashMap.retain(keep)` | Mutates an owning collection by keeping only values or key-value pairs whose predicate returns `true`. `HashMap` predicates receive `ref K` and `ref mut V`, so retained values may be updated in place. | In-place within one collection. Hash retain scans live buckets and leaves tombstones for rejected entries. | Drops rejected set values or rejected map keys and values before reducing the live length. Tree retain remains future work because ordered deletion/range mutation needs a separate red-black tree policy. |
