...
for sorting, binary search, lower/upper/equal-range bounds, partition-point
  ...
  parsing, integer `ParseError` diagnostics, hex/binary/octal signed wrappers,
  bool and decimal float parsing, plus trait-backed `parse<T>`,
...
base64 codecs with fallible owned decoders and structured codec diagnostics
for untrusted input.
