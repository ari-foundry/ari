...
| `std::process` | Represent the current process and child processes explicitly. | Current `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, typed `ExitCode`, status/root predicates, POSIX `fork`/`wait_status`/`wait` direct `Error` helpers, raw compatibility `fork_raw`/`wait_raw`, child/error predicates, typed `Signal` helpers, `Arg`/`EnvVar` wrappers, `Command` builder with `arg`/`arg_bytes`/`args`/`env`/`env_bytes`/`env_var`/`clear_env`/`inherit_env`/`current_dir`/`current_dir_path`, by-value chainable `with_arg`/`with_env`/`with_clear_env`/`with_inherit_env`/`with_current_dir`, module-level and method `spawn`/`status`/`status_with_stdin`/`status_with_stdin_string`/`exit_status`/`output`/`output_in`/`exec`, `Child` handle, `ChildStdin`/`ChildStdout`/`ChildStderr` aliases, typed `ExitStatus`, `Output` capture handle, `kill`, `kill_signal`, `terminate`, Result-returning current/executable path wrappers with optional/default compatibility forms, temp file/temp dir constructors, child environment inheritance/clearing policy plus selected assignments, child working-directory setup, parent-visible child setup and `execvp` errors through close-on-exec setup pipes, bounded pipe-backed stdin plus file-backed or `/dev/null` stdin redirection at execution time, normal/signal status inspection, and readiness-drained stdout/stderr capture; future interactive streaming stdin/stdout/stderr handles, portable Windows mapping, richer platform status fields, and daemon helpers as optional policy. |
