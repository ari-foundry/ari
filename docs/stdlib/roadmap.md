...
single-slot MPSC channel helpers with Result errors,
`time`
...
| `std::sync` | Share state between threads deliberately. | Current `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr[T]`, explicit `Ordering`, Result-returning compare-exchange methods with `_bool` compatibility, source primitive no-poison `Mutex`/`RwLock` unlock guards, `Once`, `OnceLock` with value-preserving `set` and fallible initializer status, generation `Condvar` with spin/yield timeout waits, `Barrier`, and capacity-1 MPSC channels with Result errors; future target-native generic atomics, value-protecting `Mutex[T]`/`RwLock[T]`, optional poison-aware lock types, optional `Semaphore`, blocking wait/wake, configurable channel capacity, sender cloning, receive timeouts, and Linux futex-backed internals after ownership rules are stable. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  `Once`, `OnceLock`, `Condvar`, `Barrier`, and a capacity-1 MPSC channel.
