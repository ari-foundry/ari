...
| `std::process` | Represent the current process and child processes explicitly. | Current `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, status/root predicates, POSIX `fork_result`/`wait_result` direct `Error` helpers, raw compatibility `fork`/`wait`, and child/error predicates; future portable `spawn`, `exec`, `kill`, richer status/result values, daemon helpers as optional policy, and process handles. |
