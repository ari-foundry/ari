...
| `std::sync` | Share state between threads deliberately. | Current `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr[T]`, explicit `Ordering`, Result-returning compare-exchange methods with `_bool` compatibility, source primitive no-poison `Mutex`/`RwLock` unlock guards, `Once`, `OnceLock` with value-preserving `set` and fallible initializer status, generation `Condvar` with spin/yield timeout waits, `Barrier`, and capacity-1 MPSC channels with Result errors, timeout receives, and cloned sender handles; future target-native generic atomics, value-protecting `Mutex[T]`/`RwLock[T]`, optional poison-aware lock types, optional `Semaphore`, blocking wait/wake, configurable channel capacity, sender-counted close semantics, and Linux futex-backed internals after ownership rules are stable. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  metadata/platform symlink policy, Windows locking policy, and exact
  mid-read filesystem errors after EOF/read failure is split below
  `read_byte`.
  ...
  `Once`, `OnceLock`, `Condvar`, `Barrier`, and a capacity-1 MPSC channel with
  cloned sender handles.
  Grow them toward explicit ownership transfer, user-facing thread-local storage, richer
  statuses, target-native atomics, value-protecting locks, optional
