...
| `std::process` | Represent the current process and child processes explicitly. | Current `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, status/root predicates, POSIX `fork_result`/`wait_result` direct `Error` helpers, raw compatibility `fork`/`wait`, child/error predicates, `Arg`/`EnvVar` wrappers, `Command` builder, `Child` handle, `Output` capture handle, `spawn`, `status`, `output_in`, `exec`, `kill`, `terminate`, child environment setup, child working-directory setup, and small stdout/stderr capture; future large-output readiness draining, stdin redirection, portable Windows mapping, richer status/result values, daemon helpers as optional policy, and environment inheritance/clearing policy. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait, and the
  first `Command`/`Child`/`Output` builder into fully portable child-process handles.
  The next process slices should add readiness-based large output draining,
  explicit stdin redirection, richer `ExitStatus` values, and Windows mapping.
