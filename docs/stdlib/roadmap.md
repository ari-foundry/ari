...
| `std::fs` | Work with files and directories through explicit handles. | Current `File`, `Dir`, `DirEntry`, `FileKind`, `Metadata`, `Permissions`, `exists`, access-style `can_read`/`can_write`/`can_execute` and `permissions`, target-following `metadata`/`try_metadata`, no-follow `symlink_metadata`/`try_symlink_metadata`, `try_file_type` plus `is_file`/`is_dir`/`is_symlink`/`is_other`, POSIX permission `mode`/`try_mode`/`set_mode`/`set_permissions`, `canonicalize`/`try_canonicalize`, `remove`, `rename`, `hard_link`, `symbolic_link`, `read_link`/`try_read_link`, non-truncating `ensure_file`, single-directory `create_dir`/`ensure_dir`/`remove_dir`, recursive `create_dir_all`/`ensure_dir_all`, directory-name reads with one-shot `try_read_dir`/`read_dir`, entry reads with `try_read_dir_entries`/`read_dir_entries`, lazy `DirEntry` metadata/file-kind predicates, and streaming `try_open_dir`/`Dir::next`/`Dir::close`, mode-string `open`/`try_open` with `"r"`, `"w"`, `"a"`, `"rw"`, `"r+"`, `"w+"`, and `"a+"`, `create`/`try_create`, compatibility `open_*`/`try_open_*` wrappers, byte `read_byte`/`write_byte`/`write_bytes`, cursor `position`/`seek`, whole-file `read`/`try_read`/`write`/`try_write`/`append`/`try_append`, `truncate`, source streaming `copy`/`try_copy`, `read_to_string`, `try_read_to_string`, `descriptor`, and `close`; future owned resource policy, richer per-entry errors, richer owner/group/ACL/timestamps, recursive directory removal, owned path values, temporary files, richer link metadata/platform symlink policy, optional file locking, and an options-style open builder. |
...
- Grow `std::process` from current-process helpers and the first POSIX
  ...
  timestamps, owner/group/ACL permission policy,
  recursive directory removal, owned path values, temporary files, path helpers,
