...
| `std::net` | Represent network addresses, DNS results, and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4/IPv6 `lookup_v4`/`lookup_v6` plus `_optional`/`try_*` absence-only compatibility and raw compatibility lookup bridges, deterministic `service_port`/`service_port_bytes` helpers for common service-name ports, `"host:port"`/`"[host]:port"` `resolve` plus `_optional`/`try_*` and `resolve_raw`, zone-backed `resolve_all`/`resolve_service`/`to_socket_addrs`/`to_socket_addrs_service`, `ToSocketAddrs` returning `Vec[SocketAddr]`, host-port `connect_host`/`tcp_connect_host`, module-level `listen`/`connect`/`tcp_listen`/`tcp_connect`/`tcp_listen_v6`/`tcp_connect_v6`/`udp_bind`/`udp_bind_v6`/`unix_listen`/`unix_connect`, hosted IPv4/IPv6 `TcpListener`/`TcpStream` bind/connect/accept/local-port/local-address/peer-address helpers with natural direct `Error` names, IPv6-specific local/peer address helpers, optional compatibility helpers, and raw compatibility forms, hosted IPv4/IPv6 `UdpSocket` bind/local-address/send-byte/receive-byte helpers, datagram `send_to`/`recv_from`/`peek_from` source-address helpers, connected UDP `connect`/`send`/`recv`, optional compatibility bind helpers, and raw compatibility forms, hosted Unix stream listener/connect/accept helpers with direct `Error` names, optional compatibility helpers, and raw compatibility forms, descriptor views, explicit close, close-on-exec, nonblocking flags, single-descriptor readiness probes, TCP listener/UDP reuse-address and reuse-port helpers, TCP nodelay/keepalive helpers, UDP broadcast helpers, TCP/UDP send/receive buffer-size helpers, IPv4 TTL and IPv6 hop-limit helpers, `std::time::Duration` read/write/accept timeouts plus raw millisecond compatibility helpers, `Shutdown`, TCP/Unix `std::io::Reader`/`Writer` byte adapters, TCP/Unix `read`/`write`/`read_exact`/`write_all`/`read_to_end`/`read_to_string` stream buffer helpers, and restricted-host `PermissionDenied` error bridge tests; future fuller DNS iteration and host service-database resolution, timeout-specific error results, linger/multicast options, multi-descriptor poll/event-loop APIs, Unix datagram/peer-credential helpers, and richer owned socket policy. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  zone-backed multi-address resolver lists, TCP, UDP, and Unix stream handles
  ...
  timeout-specific error results, multi-descriptor poll/event-loop APIs, linger,
  multicast controls, and Unix datagram/credential helpers. TLS
