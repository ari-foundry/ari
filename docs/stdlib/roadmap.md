...
| `std::thread` | Start and join OS threads with clear ownership transfer. | Current `ThreadId`, owning `JoinHandle`, `JoinError`, raw `Thread` info handles, `Builder`, explicit `ThreadLocal[T]`, Result-returning `spawn`, `spawn_configured`, `join`, `available_parallelism`, explicit `detach`, advisory `is_finished`, `yield_now`, `sleep`, `id`, `is_main`, raw/compatibility join and parallelism helpers, and `is_join_error` for plain `fn() -> i64` entries; future captured/capability entries, generic `JoinHandle[T]`, scoped threads, compiler-level `thread_local` declaration sugar, richer status/result values, and deeper `std::sync` integration. |
