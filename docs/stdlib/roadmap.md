...
`Utf8`/`OsStr` views, a `StringBuilder` alias for append-oriented CLI/parser
buffers, and `std::c::CStr` convenience construction, `ascii`
...
| `std::process` | Represent the current process and child processes explicitly. | Current `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, typed `ExitCode`, status/root predicates, POSIX `fork`/`wait_status`/`wait` direct `Error` helpers, raw compatibility `fork_raw`/`wait_raw`, child/error predicates, typed `Signal` helpers, `Arg`/`EnvVar` wrappers, `Command` builder with `arg`/`arg_bytes`/`args`/`env`/`env_bytes`/`env_var`/`current_dir`/`current_dir_path`, by-value chainable `with_arg`/`with_env`/`with_current_dir`, module-level and method `spawn`/`status`/`exit_status`/`output`/`output_in`/`exec`, `Child` handle, `ChildStdin`/`ChildStdout`/`ChildStderr` aliases, typed `ExitStatus`, `Output` capture handle, `kill`, `kill_signal`, `terminate`, Result-returning current/executable path wrappers with optional/default compatibility forms, temp file/temp dir constructors, child environment setup, child working-directory setup, normal/signal status inspection, and small stdout/stderr capture; future large-output readiness draining, stdin redirection, portable Windows mapping, richer platform status fields, daemon helpers as optional policy, parent-visible child setup errors, and environment clearing policy. |
...
| `std::parse` | Parse whole byte-slice values with names that read naturally at call sites. | Current ASCII-trimmed overflow-checked signed `integer`, `is_integer`, `integer_or`, base-2-through-36 signed `integer_radix`/`is_integer_radix`/`integer_radix_or`, unsigned `unsigned`/`is_unsigned`/`unsigned_or` and `unsigned_radix` helpers, `hex_integer`, `binary_integer`, `octal_integer`, boolean parser/validator/fallback helpers, `is_float`, Result-returning `float`, `float_optional`, `float_or`, `float_unchecked`, and trait-backed `Parse` with `parse<T>`/`parse_or<T>`/`is_parse<T>` for `i64`, `u64`, `bool`, and `f64`; future richer parse errors and more precise float range diagnostics. |
| `std::encoding` | Validate text encodings and convert bytes to portable text forms. | Current `is_ascii`, UTF-8/UTF-16 counts, detailed `Utf8Error` validation, UTF-8 byte-string decode, UTF-8 scalar decode/encode helpers, lowercase hex encode/decode, fallible hex decode, standard base64 encode/decode, and fallible base64 decode with natural names plus `_in` compatibility aliases where allocation is explicit; future URL-safe/MIME base64 variants, richer codec decode errors, normalization/transcoding, and optional compression policy in a separate module. |
| text/path kinds | Keep byte strings, validated text, OS strings, paths, and C strings from collapsing into one API shape. | Current byte-oriented `std::string::String` with `StringBuilder` as the explicit append-oriented alias, strict and `try_` byte access/removal plus stable byte `retain`, borrowed `std::string::Utf8`, `OsStr`, borrowed `std::c::CStr`, owned `std::c::CString`, borrowed `std::path::PathBytes`, owned `std::path::PathBuf` as a POSIX byte-buffer alias, and direct string-literal coercion into expected borrowed boundary views; future owned `Utf8String`, `OsString`, non-alias/platform-specific `PathBuf`, Windows path/OS-string semantics, fallible owned conversions, and richer C string handoff policy. |
