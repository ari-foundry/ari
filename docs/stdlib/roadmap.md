...
- Keep `std::target` as the cross-cutting target fact module. It can expose
  ...
  raw syscall wrapper.
- Grow `std::path` from lexical POSIX-style helpers, borrowed
  `Path`/`PathBytes`, and the current distinct owned POSIX `PathBuf` into
  platform-aware path representations once Windows path policy and
