...
| `std::parse` | Parse whole byte-slice values with names that read naturally at call sites. | Current ASCII-trimmed `integer`, `is_integer`, `integer_or`, base-2-through-36 `integer_radix`/`is_integer_radix`/`integer_radix_or`, `hex_integer`, `binary_integer`, boolean parser/validator/fallback helpers, `is_float`, `float_or`, and panicking `float`; future overflow policy, richer parse errors, and `Option[f64]`/`Result[f64,E]` after float enum payloads are supported. |
