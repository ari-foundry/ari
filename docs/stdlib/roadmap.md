...
predicates, explicit reserve growth plus tracked-local zone inference for
common growth calls, direct iterator support, natural
`contains_key` and `contains_value` map predicates, open-addressed `HashMap`/`HashSet` with
hash-set relationship predicates, tombstones, and live-bucket iterators, red-black-tree `TreeMap`/`TreeSet` with
tree-set relationship predicates, map entry iteration, ordered boundary access, link-rebuild removal, and sorted iterators, growable `Deque`, fixed `RingBuffer`, zone-backed
...
| `std::collections` | Store keyed, queue-like, linked, and priority data beyond vectors. | Current linear `Set[T]` with tracked-local zone inference for `insert`, `replace`, `reserve`, and `reserve_extra`, `Deque[T]`, `RingBuffer[T]`, `LinkedList[T]`, `BinaryHeap[T]`, `PriorityQueue[T]`, open-addressed `HashMap`/`HashSet`, red-black-tree `TreeMap`/`TreeSet`, natural `contains_key`/`contains_value` map predicates, `MapEntry[K,V]` map entry iterators, ordered tree boundary access, link-rebuild tree removal, explicit hash/comparator constructors, lookup/update/removal where implemented, FIFO/linked/heap tests, live-bucket hash iterators, sorted tree iterators; future direct red-black deletion optimization, broader tracked-local collection zone inference, and trait-driven constructors. |
...
| `std::vec` | Own growable contiguous storage while keeping borrowed sequence work allocation-free. | Current explicit-zone `Vec[T]`, metadata, `try_*` accessors including optional indexed removal, mutation/growth, stable in-place `retain`, copy, iteration, raw pointer access, direct `slice`, `split_at`, `find`, `contains_slice`, `compare`, `chunks`, `windows`, and delimiter `split` wrappers over live storage, plus in-place `reverse`, rotation, sorting, sortedness, binary search, and min/max wrappers; future root/source vector unification and stronger trait-driven collection constructors. |
