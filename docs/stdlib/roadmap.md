...
| `std::thread` | Start and join OS threads with clear ownership transfer. | Current `Thread`, `Builder`, `spawn`, `join`, advisory `is_finished`, `yield_now`, `sleep`, `id`, `is_main`, `available_parallelism`, and `is_join_error` for plain `fn() -> i64` entries; future captured/capability entries, user-facing `ThreadLocal[T]`, applied stack-size/name options, richer status/result values, and `std::sync` integration. |
