...
| `std::iter` | Compose sequence processing without forcing every operation onto each collection type. | Current `range`, `range_inclusive`, `Iterator`, `IntoIterator`, Slice/Vec iterator sources, mutable value cursors, lazy `map`, `filter`, `take`, `skip`, `enumerate`, `zip`, eager `fold`, `reduce`, and zone-backed `collect`; future captured closures, richer adapter inference, first-class reference-valued iterator items, and collect targets beyond `std::vec::Vec[T]`. |
