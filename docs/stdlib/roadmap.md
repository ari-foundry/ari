...
- Grow `std::process` from current-process helpers and the first POSIX
  ...
  of `std::os::OwnedFd`, plus method-style TCP/Unix stream buffer helpers;
  next network work should add IPv6 handles, buffer-oriented UDP datagrams,
  peer/local address reporting, socket options such as reuse-address and
  nodelay, and direct `Result[..., Error]` payloads.
