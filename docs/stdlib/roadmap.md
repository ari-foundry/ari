...
descriptor views, nonblocking flags, millisecond socket timeouts, stream
shutdown, and TCP/Unix `std::io` adapters,
...
| `std::fs` | Work with files and directories through explicit handles. | Current `File`, `OpenOptions`, `Dir`, `DirEntry`, `FileKind`, `Metadata`, `Permissions`, `exists`, access-style `can_read`/`can_write`/`can_execute` and `permissions`, target-following `metadata`/`try_metadata`, no-follow `symlink_metadata`/`try_symlink_metadata`, metadata access/modification/status-change timestamps, `try_file_type` plus `is_file`/`is_dir`/`is_symlink`/`is_other`, POSIX permission `mode`/`try_mode`/`set_mode`/`set_permissions`, `canonicalize`/`try_canonicalize`, `remove`, `rename`, `hard_link`, `symbolic_link`, `read_link`/`try_read_link`, non-truncating `ensure_file`, single-directory `create_dir`/`ensure_dir`/`remove_dir`, recursive `create_dir_all`/`ensure_dir_all`, recursive `remove_dir_all`, directory-name reads with one-shot `try_read_dir`/`read_dir`, entry reads with `try_read_dir_entries`/`read_dir_entries`, lazy `DirEntry` metadata/file-kind predicates, and streaming `try_open_dir`/`Dir::next`/`Dir::close`, mode-string `open`/`try_open`/`open_result` with `"r"`, `"w"`, `"a"`, `"rw"`, `"r+"`, `"w+"`, and `"a+"`, named `OpenOptions` read/write/append/truncate/create/create-new opens plus `OpenOptions::open_result`, `create`/`try_create`/`create_result`, compatibility `open_*`/`try_open_*` wrappers, byte `read_byte`/`write_byte`/`write_bytes`, cursor `position`/`seek`, whole-file `read`/`try_read`/`write`/`write_result`/`try_write`/`append`/`append_result`/`try_append`, `truncate`, source streaming `copy`/`copy_result`/`try_copy`, `read_to_string`, `try_read_to_string`, `descriptor`, and `close`; future owned resource policy, richer per-entry errors, richer owner/group/ACL and portable creation/birth-time policy, owned path values, temporary files, richer link metadata/platform symlink policy, optional file locking, and direct `Result[File, Error]` filesystem errors. |
...
| `std::net` | Represent network addresses, DNS results, and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4 `lookup_v4`, hosted IPv4 `TcpListener`/`TcpStream` bind/connect/accept/local-port/local-address/peer-address helpers, hosted IPv4 `UdpSocket` bind/local-address/send-byte/receive-byte helpers, hosted Unix stream listener/connect/accept helpers, descriptor views, explicit close, nonblocking flags, millisecond read/write/accept timeouts, `Shutdown`, TCP/Unix `std::io::Reader`/`Writer` byte adapters, TCP/Unix `read_exact`/`write_all` stream buffer helpers, and restricted-host `PermissionDenied` error bridge tests; future IPv6 socket handles, buffer-oriented UDP datagrams, UDP source address helpers, broader socket options, `std::time::Duration` timeout overloads, and richer owned socket policy. |
...
- Grow `std::process` from current-process helpers and the first POSIX
  ...
  IPv4 local-address reporting; next network work should add IPv6 handles,
  buffer-oriented UDP datagrams, UDP source address reporting, socket options such
  as reuse-address and nodelay, and direct `Result[..., Error]` payloads.
