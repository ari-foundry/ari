...
| `std::net` | Represent network addresses, DNS results, and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4 `lookup_v4`, hosted IPv4 `TcpListener`/`TcpStream` bind/connect/accept/local-port/local-address helpers, hosted IPv4 `UdpSocket` bind/local-address/send-byte/receive-byte helpers, hosted Unix stream listener/connect/accept helpers, descriptor views, explicit close, nonblocking flags, millisecond read/write/accept timeouts, `Shutdown`, TCP/Unix `std::io::Reader`/`Writer` byte adapters, TCP/Unix `read_exact`/`write_all` stream buffer helpers, and restricted-host `PermissionDenied` error bridge tests; future IPv6 socket handles, buffer-oriented UDP datagrams, peer address helpers, broader socket options, `std::time::Duration` timeout overloads, and richer owned socket policy. |
...
- Grow `std::process` from current-process helpers and the first POSIX
  ...
  buffer-oriented UDP datagrams, peer address reporting, socket options such
