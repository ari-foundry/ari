...
| `std::sync` | Share state between threads deliberately. | Current `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr[T]`, explicit `Ordering`, Result-returning compare-exchange methods with `_bool` compatibility, manual no-poison `RawMutex`/`RawRwLock` unlock guards, value-protecting `Mutex[T]`/`RwLock[T]` payload guards, `Once`, `OnceLock` with value-preserving `set` and fallible initializer status, generation `Condvar` with spin/yield timeout waits, `Barrier`, `Semaphore` with permit guards, and bounded MPSC channels with Result errors, timeout receives, and cloned sender handles; future target-native generic atomics, optional poison-aware lock types, blocking wait/wake, unbounded channel policy, sender-counted close semantics, and Linux futex-backed internals after ownership rules are stable. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  waits, unbounded channel and sender-counted close policy, and safer
  shared-state policy.
