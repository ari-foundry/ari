...
| `std::time` | Access monotonic and wall-clock time for CLIs, servers, and tests. | Current `Duration`, `Instant`, `SystemTime`, `Deadline`, `nanoseconds`, `microseconds`, `milliseconds`, `seconds`, `now`, `system_now`, `elapsed`, `sleep`, `deadline_at`, `timeout_after`, and `timeout`; future timers, interruption-aware sleep, UTC calendar conversion, and intentionally limited timezone policy. |
