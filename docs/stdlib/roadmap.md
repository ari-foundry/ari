...
| `std::thread` | Start and join OS threads with clear ownership transfer. | Current `Thread`, `Builder`, explicit `ThreadLocal[T]`, `spawn`, `join`, advisory `is_finished`, `yield_now`, `sleep`, `id`, `is_main`, `available_parallelism`, and `is_join_error` for plain `fn() -> i64` entries; future captured/capability entries, compiler-level `thread_local` declaration sugar, applied stack-size/name options, richer status/result values, and `std::sync` integration. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  convenience, available-parallelism hook, advisory completion check, and
  builder shape. `std::sync` has concrete atomic wrappers, source `Mutex`,
  ...
  applied stack-size options, richer statuses, target-native atomics,
  value-protecting locks, blocking waits, and safer shared-state policy.
