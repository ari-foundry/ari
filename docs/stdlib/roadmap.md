...
`try_copy`, `copy`, `write_all`, `flush`, stderr routing, and byte-slice output, current
...
single-directory `create_dir`/`ensure_dir`/`remove_dir`, `read_to_string`, and
