...
listener/stream handles, IPv4/IPv6 UDP single-byte datagrams, Unix stream sockets,
...
| `std::net` | Represent network addresses, DNS results, and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4/IPv6 `lookup_v4`/`lookup_v6` plus `_optional`/`try_*` absence-only compatibility and raw compatibility lookup results, `"host:port"`/`"[host]:port"` `resolve` plus `_optional`/`try_*` and `resolve_raw_result`, `to_socket_addrs`, `ToSocketAddrs`, host-port `connect_host`/`tcp_connect_host`, module-level `listen`/`connect`/`tcp_listen`/`tcp_connect`/`tcp_listen_v6`/`tcp_connect_v6`/`udp_bind`/`udp_bind_v6`/`unix_listen`/`unix_connect`, hosted IPv4/IPv6 `TcpListener`/`TcpStream` bind/connect/accept/local-port/local-address/peer-address helpers with natural direct `Error` names, IPv6-specific local/peer address helpers, optional compatibility helpers, and raw compatibility result forms, hosted IPv4/IPv6 `UdpSocket` bind/local-address/send-byte/receive-byte helpers with direct `Error` names, optional compatibility bind helpers, and raw compatibility result forms, hosted Unix stream listener/connect/accept helpers with direct `Error` names, optional compatibility helpers, and raw compatibility result forms, descriptor views, explicit close, nonblocking flags, TCP listener/UDP reuse-address helpers, TCP nodelay helpers, `std::time::Duration` read/write/accept timeouts plus raw millisecond compatibility helpers, `Shutdown`, TCP/Unix `std::io::Reader`/`Writer` byte adapters, TCP/Unix `read_exact`/`write_all` stream buffer helpers, and restricted-host `PermissionDenied` error bridge tests; future multi-address DNS iterators, service-name resolution, buffer-oriented UDP datagrams, UDP source address helpers, remaining socket options, timeout-specific error results, and richer owned socket policy. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  `std::net` now has IPv4/IPv6 DNS lookup, host-port endpoint resolution, TCP, UDP, and Unix stream handles on top
  ...
  Grow them toward explicit ownership transfer, user-facing TLS, richer
