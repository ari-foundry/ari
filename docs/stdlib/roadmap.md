...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  reporting, common socket options, IPv4 multicast controls, IPv4 TTL, IPv6 hop-limit, and direct `Error` result helpers; next
  network work should add fuller DNS iteration, host service-database lookup,
  multi-descriptor poll/event-loop APIs,
  IPv6 multicast policy, and Unix peer-credential helpers. TLS
