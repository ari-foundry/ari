...
checked/wrapping/overflowing/saturating arithmetic, division-rounding and
...
| `std::c` | Keep C ABI boundary helpers readable without making every program write raw loader and errno bindings. | Current `CStr`, `CString`, `Library`, `Symbol`, borrowed C string wrappers, zone-backed NUL-terminated storage, `errno`/`error`, `dlopen`/`dlsym`/`dlclose`/`dlerror` wrappers, and root type aliases; future typed dynamic symbols, fallible string constructors, target-specific errno symbols, and explicit FFI escape rules for zone-backed buffers. |
...
| text/path kinds | Keep byte strings, validated text, OS strings, paths, and C strings from collapsing into one API shape. | Current borrowed `std::string::Utf8`, `OsStr`, `CStr`, borrowed `std::c::CStr`, owned `std::c::CString`, and `std::path::PathBytes`; future owned `Utf8String`, `OsString`, `PathBuf`, Windows path/OS-string semantics, fallible owned conversions, and richer C string handoff policy. |
...
- Expand `std::math` from i64 signatures to generic numeric helpers when the
  ...
  wrapping/overflowing add, powers, division rounding, and divisor helpers.
...
- Add checked multiplication and policy-specific parser overflow handling after
  compiler intrinsics and diagnostics make those operations reliable across
  integer widths.
