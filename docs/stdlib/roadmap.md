...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  concrete atomic wrappers, source `Mutex`/`RwLock` unlock guards, `Once`,
  `OnceLock`, `Condvar`, `Barrier`, and a single-slot MPSC channel. Grow them
  toward explicit ownership transfer, user-facing TLS, richer statuses,
  target-native atomics, value-protecting locks, blocking waits, and safer
  shared-state policy.
