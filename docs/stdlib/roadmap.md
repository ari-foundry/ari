...
checked add/sub/mul, wrapping/overflowing add/sub, saturating arithmetic,
division-rounding and
...
| `std::math` | Provide arithmetic helpers whose names communicate policy better than raw operators. | Current natural `i64` sign/parity helpers, checked add/sub/mul/neg/abs, wrapping/overflowing add/sub, saturating add/sub/mul/neg/abs, powers, floor/ceil division, `gcd`, and `lcm`; future wrapping/overflowing multiplication, generic numeric traits, and floating helpers. |
...
- Expand `std::math` from i64 signatures to generic numeric helpers when the
  ...
  for signs, sign predicates, parity, checked/saturating add/sub/mul/neg/abs,
  wrapping/overflowing add/sub, powers, division rounding, and divisor helpers.
...
- Add wrapping/overflowing multiplication and policy-specific parser overflow
  handling after compiler intrinsics and diagnostics make those operations
  reliable across integer widths.
