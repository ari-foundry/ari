...
`read`/`write`/`append`, `truncate`, streaming `copy`/`try_copy`, `rename`,
single-directory `create_dir`/`remove_dir`, `read_to_string`, and
`Option[String]` `try_read`/`try_read_to_string`,
