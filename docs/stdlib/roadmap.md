...
| `std::net` | Represent network addresses and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4 `TcpListener`/`TcpStream` bind/connect/accept/local-port helpers, descriptor views, explicit close, `std::io::Reader`/`Writer` byte adapters, and restricted-host `PermissionDenied` error bridge tests; future DNS lookup, UDP sockets, Unix domain sockets, IPv6 socket handles, socket options, nonblocking mode, `std::time::Duration` timeouts, shutdown, and richer owned socket policy. |
