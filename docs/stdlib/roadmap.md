...
| `std::error` | Give recoverable failures a shared vocabulary instead of bools and sentinel integers. | Current `Kind`, compact `Error`, strict and fallible constructors, POSIX `from_errno`, `from_raw`, `from_raw_result`, `to_raw_result`, `raw`, `kind`, `code`, predicate helpers, root `Error`/`ErrorKind` aliases, and direct `Result[T, Error]` conversion helpers; future Windows error mapping, owned messages, structured fields, formatter integration, and conversions from io/net/process wrappers. |
