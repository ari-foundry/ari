...
| `std::sync` | Share state between threads deliberately. | Current `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr[T]`, explicit `Ordering`, Result-returning compare-exchange methods with `_bool` compatibility, source primitive no-poison `Mutex`/`RwLock` unlock guards, value-protecting `MutexValue[T]`/`RwLockValue[T]` payload guards, `Once`, `OnceLock` with value-preserving `set` and fallible initializer status, generation `Condvar` with spin/yield timeout waits, `Barrier`, and capacity-1 MPSC channels with Result errors, timeout receives, and cloned sender handles; future target-native generic atomics, shorter `Mutex[T]`/`RwLock[T]` spelling after a compatibility plan, optional poison-aware lock types, optional `Semaphore`, blocking wait/wake, configurable channel capacity, sender-counted close semantics, and Linux futex-backed internals after ownership rules are stable. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  should add readiness-based large output draining, interactive streaming
  stdin handles, richer platform status fields, and Windows mapping.
  ...
  timeout-specific error results, readiness/poll abstractions, linger,
  ...
  concrete atomic wrappers, source no-poison `Mutex`/`RwLock` unlock guards,
  `MutexValue[T]`/`RwLockValue[T]` payload guards, `Once`, `OnceLock`,
  ...
  statuses, target-native atomics, shorter generic lock spelling, optional
  poison-aware variants, blocking waits, richer channel capacity and close
