...
| `std::error` | Give recoverable failures a shared vocabulary instead of bools and sentinel integers. | Current `Kind`, compact `Error`, strict and fallible constructors, POSIX `from_errno`, `from_raw`, `from_raw_result`, `to_raw_result`, `raw`, `kind`, `code`, predicate helpers, root `Error`/`ErrorKind` aliases, direct `Result[T, Error]` conversion helpers, and first direct fs/io/net result surfaces; future Windows error mapping, owned messages, structured fields, formatter integration, and remaining process wrapper conversions. |
