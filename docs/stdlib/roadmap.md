...
| `std::thread` | Start and join OS threads with clear ownership transfer. | Current `ThreadId`, owning `JoinHandle`, `JoinError`, typed `ThreadResult` wrappers over joined `i64` values, raw `Thread` info handles, `Builder`, explicit `ThreadLocal[T]` with capacity introspection and recoverable capacity errors, Result-returning `spawn`, `spawn_configured`, `join`, `join_value`, `available_parallelism`, explicit `detach`, advisory `is_finished`, `yield_now`, `sleep`, `id`, `is_main`, raw/compatibility join and parallelism helpers, and `is_join_error` for plain `fn() -> i64` entries; future captured/capability entries, generic `JoinHandle[T]`, scoped threads, compiler-level `thread_local` declaration sugar, generic thread return payloads, and deeper `std::sync` integration. |
