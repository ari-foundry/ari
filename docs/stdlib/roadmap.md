...
strict and fallible `FormatSpec` builders, fixed-arity one-through-four runtime
template helpers, and matching writer-backed helpers, `cmp` three-way ordering and value
...
`program_name` plus current-process environment Result helpers
`get`/`get_os`/`set`/`remove`, compatibility `try_get`/`get_or_default`/
`set_unchecked` helpers, OS-string views for arguments/environment/path-like values, and
Result-returning path-state helpers `current_dir`/`current_dir_path`/
`set_current_dir`/`executable_path`/`executable_path_os` with `_optional`,
`_or_default`, `_raw`, and `try_*` compatibility forms,
...
no-poison `Mutex`/`RwLock`, `Once`, `OnceLock`, `Condvar`, `Barrier`,
`Semaphore`, and bounded MPSC channel helpers with Result errors and cloned sender handles,
`time`
...
`net` source IP/socket-address values plus hosted IPv4/IPv6 DNS lookup and
host-port/bracketed IPv6 endpoint resolution, IPv4/IPv6 TCP
listener/stream handles, zone-backed resolver lists, IPv4/IPv6 UDP datagrams
with source-address reporting, connected UDP send/receive, common socket
options, Unix stream sockets, descriptor views, nonblocking flags,
`std::time::Duration` socket timeouts with raw millisecond compatibility
helpers, stream shutdown, and TCP/Unix `std::io` adapters,
...
| --- | --- | --- |
| `std::io` | Provide byte-oriented process IO contracts that other libraries can share without hiding raw hooks. | Current `Reader`, `Writer`, `Seek`, one-byte `ReadByte` status for byte/EOF/adapter-error separation, `Stdin`, `Stdout`, `Stderr`, `Pipe`, `PipeReader`, `PipeWriter`, `Cursor`, `BufReader`, `BufWriter`, `stdin`, `stdout`, `stderr`, `pipe`, `cursor`, `buf_reader`, `buf_reader_in`, `buf_writer`, `buf_writer_in`, direct `Error` helpers `read_one`/`read`/`read_exact`/`copy`/`write`/`write_all`/`flush`, natural `Writer::write`/`Writer::write_all` methods, compatibility `read_exact_unchecked`/`try_copy`/`copy_unchecked`/`write_all_unchecked`/`flush_unchecked`, zone-backed `read_all`, zone-backed `read_to_string`, zone-backed buffered constructors, stderr routing, pipe adapters over `std::os::Pipe`, `PipeWriter` close-on-exec helpers, `std::fs::File` Reader/Writer/Seek adapters, best-effort `BufWriter` drop cleanup, and raw scalar/byte/line hooks; future work is target/runtime-specific read-error taxonomy beneath the current `read_byte` sentinel and deeper formatter streaming. |
| `std::env` | Read startup and environment state without exposing raw runtime hooks. | Current `arg_count`, `arg`, `arg_os`, `has_arg`, `try_arg`, `try_arg_os`, `program_name`, `program_name_os`, Result-returning `get`, `get_os`, `set`, `remove`, `has`, compatibility `try_get`, `try_get_os`, `get_or_default`, `get_os_or_default`, `set_unchecked`, `remove_unchecked`, Result-returning `current_dir`, `current_dir_os`, `current_dir_path`, `set_current_dir`, `executable_path`, `executable_path_os`, optional/default/raw compatibility path helpers, and `try_*` path aliases; future path normalization and platform-specific expansion. |
...
| `std::process` | Represent the current process and child processes explicitly. | Current `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, typed `ExitCode`, status/root predicates, POSIX `fork`/`wait_status`/`wait` direct `Error` helpers, raw compatibility `fork_raw`/`wait_raw`, child/error predicates, typed `Signal` helpers, `Arg`/`EnvVar` wrappers, `Command` builder with `arg`/`arg_bytes`/`args`/`env`/`env_bytes`/`env_var`/`clear_env`/`inherit_env`/`current_dir`/`current_dir_path`, by-value chainable `with_arg`/`with_env`/`with_clear_env`/`with_inherit_env`/`with_current_dir`, module-level and method `spawn`/`spawn_piped`/`status`/`status_with_stdin`/`status_with_stdin_string`/`exit_status`/`output`/`output_in`/`exec`, `Child` handle, `ChildPipes`, `ChildStdin`/`ChildStdout`/`ChildStderr` aliases, typed `ExitStatus`, POSIX wait-status detail accessors for stopped/continued/core-dump states, `Output` capture handle, `kill`, `kill_signal`, `terminate`, explicit detach vocabulary, Result-returning current/executable path wrappers with optional/default compatibility forms, temp file/temp dir constructors, child environment inheritance/clearing policy plus selected assignments, child working-directory setup, parent-visible child setup and `execvp` errors through close-on-exec setup pipes, bounded pipe-backed stdin plus file-backed or `/dev/null` stdin redirection at execution time, interactive stdin/stdout/stderr pipes, normal/signal status inspection, and readiness-drained stdout/stderr capture; future portable Windows mapping, process groups/background reaping or daemon helpers as optional policy. |
| `std::fs` | Work with files and directories through explicit handles. | Current `File`, `OpenOptions`, `Dir`, `DirEntry`, `DirEntryInfo`, `PathError`, `TwoPathError`, `Operation`, `FileKind`, `Metadata`, `Permissions`, existence/access checks, target-following and no-follow metadata, POSIX owner/group ids, explicit optional creation/birth-time policy, metadata access/modification/status-change timestamps, file-kind predicates, POSIX permission mode lookup/mutation, canonicalization, links, non-truncating file creation, single and recursive directory helpers, directory-name and entry reads, lazy `DirEntry` metadata, per-entry metadata Result snapshots through `read_dir_info`, streaming directory handles, hosted temp file/temp dir wrappers, POSIX advisory file locks, natural `Result[..., Error]` names for open/create/read/write/append/copy/query/mutation/directory helpers, structured `_detailed` helpers for path/source-target context, `_optional`/`try_*` helpers that discard error detail, `_bool` and `_unchecked` compatibility helpers, raw `*_raw` compatibility variants, byte/file-cursor/whole-file helpers with EOF/error-splitting host read hooks, `std::io` adapters, descriptor views, and explicit close; future ACL policy, platform-specific owned path values, richer link metadata/platform symlink policy, and Windows locking policy. |
| `std::path` | Manipulate path bytes without opening the filesystem. | Current hosted Linux/POSIX-style `Path`/`PathBytes`, distinct owned `PathBuf` over zone-backed byte strings, `bytes`, `from_os`, `from_bytes`, `from_string`, `to_string`, `is_empty`, `contains_nul`, `as_bytes`, method-style path wrappers, `is_separator`, `is_absolute`, `is_relative`, explicit `windows_*` drive/UNC lexical classifiers, `trim_trailing_separators`, borrowed `components`, kinded components for POSIX root/current/parent/normal and Windows drive/UNC prefixes, file-name/parent/extension/stem helpers, allocation-free `has_*` predicates, component-aware `starts_with`/`ends_with` and borrowed `strip_prefix`/`strip_suffix`, zone-backed `with_file_name_in`/`with_extension_in`, `join_in`, owned-path `join`/`join_many`, `current_dir_join`, and `normalize_in`; existing-path runtime canonicalization lives in `std::fs`; future platform-specific path representations and verbatim Windows path policy. |
| `std::net` | Represent network addresses, DNS results, and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4/IPv6 `lookup_v4`/`lookup_v6` plus `_optional`/`try_*` absence-only compatibility and raw compatibility lookup bridges, deterministic `service_port`/`service_port_bytes` helpers for common service-name ports, `"host:port"`/`"[host]:port"` `resolve` plus `_optional`/`try_*` and `resolve_raw`, zone-backed `resolve_all`/`resolve_service`/`to_socket_addrs`/`to_socket_addrs_service`, `ToSocketAddrs` returning `Vec[SocketAddr]`, host-port `connect_host`/`tcp_connect_host`, module-level `listen`/`connect`/`tcp_listen`/`tcp_connect`/`tcp_listen_v6`/`tcp_connect_v6`/`udp_bind`/`udp_bind_v6`/`unix_listen`/`unix_connect`/`unix_datagram`/`unix_datagram_unbound`, hosted IPv4/IPv6 `TcpListener`/`TcpStream` bind/connect/accept/local-port/local-address/peer-address helpers with natural direct `Error` names, IPv6-specific local/peer address helpers, optional compatibility helpers, and raw compatibility forms, hosted IPv4/IPv6 `UdpSocket` bind/local-address/send-byte/receive-byte helpers, datagram `send_to`/`recv_from`/`peek_from` source-address helpers, connected UDP `connect`/`send`/`recv`, optional compatibility bind helpers, and raw compatibility forms, hosted Unix stream listener/connect/accept helpers with direct `Error` names, connected Unix stream peer credential snapshots, Unix datagram bind/unbound/connect/send/recv helpers, optional compatibility helpers, and raw compatibility forms, descriptor views, explicit close, close-on-exec, nonblocking flags, single-descriptor readiness probes, TCP listener/UDP reuse-address and reuse-port helpers, TCP nodelay/keepalive/linger helpers, UDP broadcast and IPv4 multicast loop/TTL/membership helpers, TCP/UDP send/receive buffer-size helpers, IPv4 TTL and IPv6 hop-limit helpers, `std::time::Duration` read/write/accept timeouts plus raw millisecond compatibility helpers, shared timeout/would-block/interrupted/refused/retryable error predicates, `Shutdown`, TCP/Unix `std::io::Reader`/`Writer` byte adapters, TCP/Unix `read`/`write`/`read_exact`/`write_all`/`read_to_end`/`read_to_string` stream buffer helpers, and restricted-host `PermissionDenied` error bridge tests; future fuller DNS iteration and host service-database resolution, IPv6 multicast policy, multi-descriptor poll/event-loop APIs, and richer owned socket policy. |
...
| `std::thread` | Start and join OS threads with clear ownership transfer. | Current `ThreadId`, owning `JoinHandle`, `JoinError`, typed `ThreadResult` wrappers over joined `i64` values, `ThreadGroupResult`, `ThreadScope`, raw `Thread` info handles, `Builder`, explicit `ThreadLocal[T]` with capacity introspection and recoverable capacity errors, Result-returning `spawn`, `spawn_raw`, `spawn_configured`, `spawn_raw_configured`, `join`, `join_value`, `try_join`, `try_join_value`, `available_parallelism`, explicit `detach`, advisory `is_finished`, `yield_now`, `sleep`, `id`, `is_main`, raw/compatibility join and parallelism helpers, and `is_join_error` for plain `fn() -> i64` and explicit `fn(ptr u8) -> i64` entries; future captured/capability entries, generic `JoinHandle[T]`, borrowed scoped threads, compiler-level `thread_local` declaration sugar, generic thread return payloads, and deeper `std::sync` integration. |
| `std::sync` | Share state between threads deliberately. | Current `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr[T]`, explicit `Ordering`, Result-returning compare-exchange methods with `_bool` compatibility, manual no-poison `RawMutex`/`RawRwLock` unlock guards, value-protecting `Mutex[T]`/`RwLock[T]` payload guards, `Once`, `OnceLock` with value-preserving `set` and fallible initializer status, generation `Condvar` with spin/yield timeout waits, `Barrier`, `Semaphore` with permit guards, and bounded MPSC channels with Result errors, timeout receives, cloned sender handles, and sender-counted close semantics; future target-native generic atomics, optional poison-aware lock types, blocking wait/wake, unbounded channel policy, and Linux futex-backed internals after ownership rules are stable. |
...
| `std::collections` | Store keyed, queue-like, linked, and priority data beyond vectors. | Current linear `Set[T]`, `Deque[T]`, `RingBuffer[T]`, `LinkedList[T]`, `BinaryHeap[T]`, `PriorityQueue[T]`, open-addressed `HashMap`/`HashSet`, red-black-tree `TreeMap`/`TreeSet`, tracked-local zone inference for growable mutation calls, target-zone copy across collection families, natural `contains_key`/`contains_value` predicates and `get_or` fallback lookup for maps, direct `get_mut`/`try_get_mut` mutable map value access, named map `replace`, `HashMapEntry`/`TreeMapEntry` update handles with `or_insert`/`or_insert_with`/`or_default`/`and_modify` plus direct `insert`/`insert_entry`/`remove`/`key`/`value`/`value_mut`, `remove_entry` key-value removal, set representative `get`/`try_get`, lazy set algebra iterators for `Set`/`HashSet`/`TreeSet`, `MapEntry[K,V]` map entry iterators and tree boundary entries with `key()`/`value()` accessors, map `iter()` aliases, direct map `IntoIterator`, mutable map value cursors, map `iter_mut()` over copied-key mutable-value handles, in-place `retain` for `Set`/`HashMap`/`HashSet`/`TreeMap`/`TreeSet`, collection drains for maps and sets, ordered tree boundary and lower/upper-bound access, tree `range_mut`, tree split/append helpers, direct red-black tree deletion with compacting slot movement, natural default-policy `new` constructors, explicit `with_hash`/`with_less` custom-policy constructors, trait-driven `with_capacity` constructors for ordinary hash/tree/heap maps, sets, and queues, lookup/update/removal where implemented, FIFO/linked/heap tests, live-bucket hash iterators, sorted tree iterators; future general borrowed lookup and first-class reference-valued iterator item support for tighter mutable cursor ergonomics. |
...
| `std::hash` | Provide deterministic non-cryptographic hashing without tying hash policy to one collection type. | Current `Hasher`, `Hash[T]`, `new`, `reset`, `finish`, `write`, `value`, `bytes`, fixed-width integer and bool write helpers, `collections::hash_i64` compatibility, and trait-driven hash collection constructors through `HashMap::with_capacity` / `HashSet::with_capacity`; future aggregate/derive impls. |
| `std::random` | Provide OS seed material and reproducible non-cryptographic random streams without making cryptography promises. | Current Result-returning `entropy` and direct OS byte `fill`, strict `entropy_unchecked`/`fill_unchecked`, raw `entropy_raw`/`fill_raw`, deterministic `Prng`, `seed`, Result-returning `from_entropy`/`seed_from_os`, strict `from_entropy_unchecked`/`seed_from_os_unchecked`, `next`, `boolean`, unbiased `below`/`range` plus fallible bound checks, `float`, PRNG byte fill, and generic `shuffle`; future larger distribution tests, cryptographic stream policy, and distribution helpers. |
...
| `std::parse` | Parse whole byte-slice values with names that read naturally at call sites. | Current ASCII-trimmed overflow-checked signed `integer`, `is_integer`, `integer_or`, base-2-through-36 signed `integer_radix`/`is_integer_radix`/`integer_radix_or`, unsigned `unsigned`/`is_unsigned`/`unsigned_or` and `unsigned_radix` helpers, `ParseError`/`ParseErrorKind` diagnostics with offsets, stable labels, messages, and `is_*` predicates for signed/unsigned integer, radix, and float parsers, finite/subnormal float boundary diagnostics, explicit `*_with_underscores` signed/unsigned/radix/base-wrapper numeric parsers for human-edited values, `hex_integer`, `binary_integer`, `octal_integer`, boolean parser/validator/fallback helpers, `is_float`, Result-returning `float`, `float_optional`, `float_or`, underscore-aware float helpers, `float_unchecked`, and trait-backed `Parse` with `parse<T>`/`parse_or<T>`/`is_parse<T>` for `i64`, `u64`, `bool`, and `f64`; future taxonomy splits only when real callers need finer categories. |
| `std::encoding` | Validate text encodings and convert bytes to portable text forms. | Current `is_ascii`, UTF-8/UTF-16 counts, detailed `Utf8Error` validation with stable diagnostic names/messages, UTF-8 byte-string decode, UTF-8 scalar decode/encode helpers, lowercase hex encode/decode, structured `CodecError` names/messages for hex/base64 validators, fallible hex decode, standard base64 encode/decode, MIME line-wrapped base64 encode/decode, URL-safe base64 padded/unpadded encode and padded/unpadded decode, and fallible base64 decode with natural names plus `_in` compatibility aliases where allocation is explicit; future Unicode normalization/transcoding and optional compression policy in a separate module. |
| text/path kinds | Keep byte strings, validated text, OS strings, paths, and C strings from collapsing into one API shape. | Current byte-oriented `std::string::String` with strict and `try_` byte access/removal plus stable byte `retain`, borrowed `std::string::Utf8`, owned `std::string::Utf8String`, borrowed `OsStr`, owned `OsString`, borrowed `std::c::CStr`, owned `std::c::CString`, borrowed `std::path::PathBytes`, distinct owned POSIX `std::path::PathBuf`, explicit Windows drive/UNC byte classifiers, and direct string-literal coercion into expected borrowed boundary views; future platform-specific path and OS-string values, verbatim Windows path semantics, and richer C string handoff policy. |
| `std::error` | Give recoverable failures a shared vocabulary instead of bools and sentinel integers. | Current `Kind` categories including filesystem, IO, and network cases such as `ConnectionRefused`, compact `Error`, strict and fallible constructors, POSIX `from_errno`/`from_os_code`, errno-result and raw-result conversion helpers, `to_raw`, `raw`, `kind`, `code`, predicate helpers, root and fs/io/net/os/process `Error`/`ErrorKind` aliases, Display/Debug formatting, direct `Result[T, Error]` conversion helpers, and first direct fs/io/net/process result surfaces; future Windows error mapping, owned messages, structured fields, and remaining wrapper conversions. |
...
- Add collection helpers in small slices: slice methods, vector methods, the
  ...
  String-key borrowed byte-slice lookup, natural default-policy constructors,
  and then general borrowed lookup.
- Keep the common constructor spelling natural: `HashMap::new`,
  `HashSet::new`, `TreeMap::new`, `TreeSet::new`, `BinaryHeap::new`, and
  `PriorityQueue::new` use the default `Hash`/`Ord` policy. Keep custom
  hash/comparator policy explicit through `with_hash`, `with_less`, or the
  lower-level constructor functions so unusual ordering remains obvious at
  call sites.
...
- Grow `std::hash` toward derived aggregate impls.
...
- Keep validation, UTF-8 scalar helpers, UTF-8 byte-string decoding, and byte
  codecs in `std::encoding`, not in `std::hash` or `std::algo`. Hex/base64 are
  implemented now; compression remains optional future work after byte-buffer
  ownership and error values improve.
...
- Keep `std::target` as the cross-cutting target fact module. It can expose
  ...
  raw syscall wrapper.
- Grow `std::path` from lexical POSIX-style helpers, explicit Windows
  drive/UNC classifiers, borrowed `Path`/`PathBytes`, and the current distinct
  owned POSIX `PathBuf` into platform-aware path representations once verbatim
  Windows path policy and canonicalization policy are stable.
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  `ExitCode`, typed `ExitStatus`/`Signal`, temp path constructors, file or
  `/dev/null` stdin redirection, and the current `Command`/`Child`/`Output`
  builder into fully portable child-process handles. The next process slices
  should add Windows mapping plus optional process groups/background reaping
  or daemon helpers.
  `std::time` and `std::fs` now have their first thin wrappers; time
  should grow toward timers
  and interruption-aware sleep, while filesystem work should next add richer
  ACL permission policy, platform-specific owned path values, richer link
  metadata/platform symlink policy, Windows locking policy, and richer
  platform-specific error fields now that EOF and host read failures are split
  below `read_byte`.
  `std::net` now has IPv4/IPv6 DNS lookup, host-port endpoint resolution,
  zone-backed multi-address resolver lists, TCP, UDP, Unix stream, and Unix datagram handles
  on top of `std::os::OwnedFd`, plus method-style TCP/Unix stream buffer
  helpers, UDP datagram source-address helpers, IPv4/IPv6 local/peer address
  reporting, connected Unix stream peer credentials, common socket options, IPv4 multicast controls, IPv4 TTL, IPv6 hop-limit, and direct `Error` result helpers; next
  network work should add fuller DNS iteration, host service-database lookup,
  multi-descriptor poll/event-loop APIs,
  IPv6 multicast policy. TLS
  should stay out of `std::net` for now and live in a future arix package-layer
  library once package and certificate policy are deliberate.
  `std::thread` now has function-pointer and explicit raw-data spawn/join
  wrappers, sleep convenience, available-parallelism hook, advisory completion
  check, nonblocking try-join helpers, configured builder spawn,
  fixed-capacity scoped join owners, and explicit thread-local handle.
  `std::sync` has
  concrete atomic wrappers, manual no-poison `RawMutex`/`RawRwLock` guards,
  value-protecting `Mutex[T]`/`RwLock[T]` payload guards, `Once`, `OnceLock`,
  `Condvar`, `Barrier`, `Semaphore`, and a bounded MPSC channel with cloned
  sender handles and sender-counted close semantics.
  Grow them toward explicit ownership transfer, user-facing thread-local storage, richer
  statuses, target-native atomics, optional poison-aware variants, blocking
  waits, unbounded channel policy, and safer shared-state policy.
