...
- Add collection helpers in small slices: slice methods, vector methods, the
  ...
  String-key borrowed byte-slice lookup, trait-driven constructors, and then
  general borrowed lookup.
- Keep custom hash/comparator policy explicit through `with_hash` or the
  lower-level constructor functions. `with_capacity` is the current
  trait-driven common path for hash/tree maps and sets.
- When constructor overload or breaking-change policy is ready, consider moving
  the common constructor spelling from `with_capacity` to `HashMap::new`,
  `HashSet::new`, `TreeMap::new`, and `TreeSet::new` while keeping custom
  policy in explicit names so unusual ordering remains obvious at call sites.
