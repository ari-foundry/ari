...
| `std::encoding` | Validate text encodings and convert bytes to portable text forms. | Current `is_ascii`, UTF-8/UTF-16 counts, detailed `Utf8Error` validation, UTF-8 byte-string decode, UTF-8 scalar decode/encode helpers, lowercase hex encode/decode, fallible hex decode, standard base64 encode/decode, URL-safe base64 padded/unpadded encode and padded/unpadded decode, and fallible base64 decode with natural names plus `_in` compatibility aliases where allocation is explicit; future MIME line-wrapped base64 variants, richer codec decode errors, normalization/transcoding, and optional compression policy in a separate module. |
