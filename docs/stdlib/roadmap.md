...
no-poison `Mutex`/`RwLock`, `Once`, `OnceLock`, `Condvar`, `Barrier`, and
single-slot MPSC channel helpers with Result errors and cloned sender handles,
`time`
...
| `std::sync` | Share state between threads deliberately. | Current `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr[T]`, explicit `Ordering`, Result-returning compare-exchange methods with `_bool` compatibility, manual no-poison `RawMutex`/`RawRwLock` unlock guards, value-protecting `Mutex[T]`/`RwLock[T]` payload guards, `Once`, `OnceLock` with value-preserving `set` and fallible initializer status, generation `Condvar` with spin/yield timeout waits, `Barrier`, and capacity-1 MPSC channels with Result errors, timeout receives, and cloned sender handles; future target-native generic atomics, optional poison-aware lock types, optional `Semaphore`, blocking wait/wake, configurable channel capacity, sender-counted close semantics, and Linux futex-backed internals after ownership rules are stable. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  should add interactive streaming stdin/stdout/stderr handles, richer
  platform status fields, and Windows mapping.
  ...
  `Condvar`, `Barrier`, and a capacity-1 MPSC channel with cloned sender
  handles.
  ...
  waits, richer channel capacity and close
  policy, and safer shared-state policy.
