...
| `std::fs` | Work with files and directories through explicit handles. | Current `File`, `OpenOptions`, `Dir`, `DirEntry`, `FileKind`, `Metadata`, `Permissions`, existence/access checks, target-following and no-follow metadata, metadata access/modification/status-change timestamps, file-kind predicates, POSIX permission mode lookup/mutation, canonicalization, links, non-truncating file creation, single and recursive directory helpers, directory-name and entry reads, lazy `DirEntry` metadata, streaming directory handles, hosted temp file/temp dir wrappers, POSIX advisory file locks, natural `Result[..., Error]` names for open/create/read/write/append/copy/query/mutation/directory helpers, `_optional`/`try_*` helpers that discard error detail, `_bool` and `_unchecked` compatibility helpers, raw `*_raw` compatibility variants, byte/file-cursor/whole-file helpers, `std::io` adapters, descriptor views, and explicit close; future richer per-entry errors, richer owner/group/ACL and portable creation/birth-time policy, platform-specific owned path values, richer link metadata/platform symlink policy, Windows locking policy, and structured filesystem error fields. |
...
- Grow `std::process` from current-process helpers, POSIX fork/wait,
  ...
  per-entry directory errors, portable metadata creation/birth-time policy,
  owner/group/ACL permission policy, platform-specific owned path values,
  richer link metadata/platform symlink policy, Windows locking policy, and
  structured filesystem error fields.
