...
| `std::fs` | Work with files and directories through explicit handles. | Current `File`, `OpenOptions`, `Dir`, `DirEntry`, `FileKind`, `Metadata`, `Permissions`, existence/access checks, target-following and no-follow metadata, metadata access/modification/status-change timestamps, file-kind predicates, POSIX permission mode lookup/mutation, canonicalization, links, non-truncating file creation, single and recursive directory helpers, directory-name and entry reads, lazy `DirEntry` metadata, streaming directory handles, hosted temp file/temp dir wrappers, POSIX advisory file locks, mode-string and named `OpenOptions` opens where natural `open`/`create`/`OpenOptions::open` return `Result[File, Error]`, `_optional`/`try_*` helpers discard error detail, `_unchecked` helpers preserve invalid-handle compatibility, and existing `open_result`/`create_result`/`OpenOptions::open_result` remain compatibility aliases, `metadata_result`/`symlink_metadata_result`/`file_type_result`/`mode_result` direct query results, `canonicalize_result`/`read_link_result` and directory `open_dir_result`/`read_dir_result`/`read_dir_entries_result`, `read_result`/`read_to_string_result` direct `Result[String, Error]` forms, `remove_result`/`rename_result`/`create_dir_result`/`remove_dir_result` direct `Result[(), Error]` forms, `write_result`/`append_result`/`copy_result` direct `Result[i64, Error]` forms, raw `*_raw_result` compatibility variants, byte/file-cursor/whole-file helpers, `std::io` adapters, descriptor views, and explicit close; future richer per-entry errors, richer owner/group/ACL and portable creation/birth-time policy, platform-specific owned path values, richer link metadata/platform symlink policy, Windows locking policy, and structured filesystem error fields. |
...
| `std::net` | Represent network addresses, DNS results, and explicit socket handles. | Current `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, constructors, strict and fallible indexed accessors, family predicates, loopback/unspecified checks, port helpers, hosted IPv4/IPv6 `lookup_v4`/`lookup_v6` plus `_optional`/`try_*` absence-only compatibility and raw compatibility lookup results, `"host:port"`/`"[host]:port"` `resolve` plus `_optional`/`try_*` and `resolve_raw_result`, zone-backed `resolve_all`/`to_socket_addrs`, `ToSocketAddrs` returning `Vec[SocketAddr]`, host-port `connect_host`/`tcp_connect_host`, module-level `listen`/`connect`/`tcp_listen`/`tcp_connect`/`tcp_listen_v6`/`tcp_connect_v6`/`udp_bind`/`udp_bind_v6`/`unix_listen`/`unix_connect`, hosted IPv4/IPv6 `TcpListener`/`TcpStream` bind/connect/accept/local-port/local-address/peer-address helpers with natural direct `Error` names, IPv6-specific local/peer address helpers, optional compatibility helpers, and raw compatibility result forms, hosted IPv4/IPv6 `UdpSocket` bind/local-address/send-byte/receive-byte helpers, datagram `send_to`/`recv_from`/`peek_from` source-address helpers, connected UDP `connect`/`send`/`recv`, optional compatibility bind helpers, and raw compatibility result forms, hosted Unix stream listener/connect/accept helpers with direct `Error` names, optional compatibility helpers, and raw compatibility result forms, descriptor views, explicit close, close-on-exec, nonblocking flags, TCP listener/UDP reuse-address and reuse-port helpers, TCP nodelay/keepalive helpers, UDP broadcast helpers, TCP/UDP send/receive buffer-size helpers, `std::time::Duration` read/write/accept timeouts plus raw millisecond compatibility helpers, `Shutdown`, TCP/Unix `std::io::Reader`/`Writer` byte adapters, TCP/Unix `read`/`write`/`read_exact`/`write_all`/`read_to_end`/`read_to_string` stream buffer helpers, and restricted-host `PermissionDenied` error bridge tests; future fuller DNS iteration and service-name resolution, timeout-specific error results, linger/TTL/hop-limit/multicast options, readiness/poll abstractions, Unix datagram/peer-credential helpers, and richer owned socket policy. |
