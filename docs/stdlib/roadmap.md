...
- Grow `std::process` from current-process helpers and the first POSIX
  ...
  of `std::os::OwnedFd`; next network work should add IPv6 handles,
  buffer-oriented UDP/TCP helpers, peer/local address reporting, socket
  options such as reuse-address and nodelay, and direct `Result[..., Error]`
  payloads.
