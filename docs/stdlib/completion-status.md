...
| `std::net` | 89% | IPv4/IPv6 address values, hosted IPv4/IPv6 DNS lookup, bracketed IPv6 endpoint parsing, `resolve_all`/`to_socket_addrs`, IPv4/IPv6 TCP and UDP sockets, Unix streams, socket IO adapters, UDP `send_to`/`recv_from`/`peek_from`, common options, close-on-exec, nonblocking, timeouts, single-descriptor readiness probes, keepalive, reuse port, broadcast, and buffer sizes. | Full DNS result iteration beyond the current small resolver list, service-name ports, canonical names, timeout-specific errors, multi-descriptor poll/event-loop APIs, linger, TTL/hop-limit, multicast, Unix datagram, peer credentials, and package-layer TLS policy. |
...
| `std::net` | Fuller DNS iteration, service-name ports, canonical names, timeout-specific errors, multi-descriptor poll/event-loop APIs, linger, TTL/hop-limit, multicast, Unix datagram, peer credentials, and keeping TLS as a future package-layer decision. |
