...
| Formatting and logging | 88% | Display/Debug traits, scalar formatting, strict/fallible `FormatSpec`, `format_in!`, stdout/stderr value printing, writer-backed helpers, direct scalar/text streaming writer helpers, `concat2`/`concat3`/`concat4`, `format_value`, fixed-arity runtime template `format`/`format2`/`format3`/`format4`, and matching writer helpers including explicit `write_format_stream*` spellings. | Variadic/default-zone formatting policy, generic per-value streaming display dispatch beyond current fixed arities, structured logging, scoped filters, and capture/backtrace integration. |
...
| `std::fmt` | Variadic/default-zone formatting policy, generic per-value streaming `Display` dispatch beyond the current `Display::format_in` string step, structured logging, scoped filters, and capture/backtrace integration. Fixed runtime templates and writer helpers currently cover one through four placeholders, and scalar/text helpers can now stream directly to `io::Writer`. |
