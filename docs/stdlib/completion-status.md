...
| Text, string, encoding, path, C boundary | 90% | Byte-oriented `String`, parser-friendly string helpers, UTF-8 codepoint helpers, explicit `Utf8`, `OsStr`, `CStr`/`CString`, UTF-8 validation/decode/encode, hex/base64 including MIME and URL-safe shapes, POSIX `PathBytes`/`PathBuf` alias, joins and normalization. | Owned `Utf8String`/`OsString`, non-alias platform-specific `PathBuf`, Windows drive/UNC/OS-string policy, richer codec errors, Unicode normalization/transcoding, grapheme/locale policy. No separate string-builder type is planned for the basic stdlib slice; byte `String` and `Vec[u8]` cover that role. |
...
| `std::thread` | Generic `JoinHandle[T]`, captured closures, scoped threads, compiler-level thread-local declarations, richer thread result/status values, and lower-friction `ThreadLocal` ergonomics. |
...
| `std::fmt` | General runtime template formatting, default-zone formatting policy, richer streaming writer formatter, structured logging, scoped filters, and capture/backtrace integration. |
...
| `std::parse` | More precise float range diagnostics and a richer parse error taxonomy only where callers need it. |
| `std::encoding` | Richer codec decode errors, normalization/transcoding, and optional compression policy outside the core encoding module. |
