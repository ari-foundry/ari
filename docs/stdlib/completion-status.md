...
| Collections and iterators | 93% | `Vec`, slices, iterators, algorithms, `HashMap`/`HashSet`, `TreeMap`/`TreeSet`, `Deque`, `RingBuffer`, `LinkedList`, `BinaryHeap`, `PriorityQueue`, entry APIs, drains, ordered boundaries, natural default-policy `new` constructors, explicit `with_hash`/`with_less` custom policy constructors, trait-driven `with_capacity` constructors, String-key borrowed byte lookup, mutable value cursors, tree `range_mut`, tree split/append helpers, and in-place retain across linear, hash, and tree collections. | General borrowed lookup beyond String keys, lazy set algebra, and first-class reference-valued iterator items for tighter mutable iteration. |
| Formatting and logging | 87% | Display/Debug traits, scalar formatting, strict/fallible `FormatSpec`, `format_in!`, stdout/stderr value printing, writer-backed helpers, `concat2`/`concat3`/`concat4`, `format_value`, fixed-arity runtime template `format`/`format2`/`format3`/`format4`, and matching writer helpers. | Variadic/default-zone formatting policy, fully streaming template writers beyond current fixed arities, structured logging, scoped filters, and capture/backtrace integration. |
...
- `std::collections` has natural `new` constructors, String-key borrowed
  byte-slice lookup, tree `range_mut`, tree split/append, and focused tests for
  those collection-polish paths.
...
| `std::collections` | General borrowed lookup beyond String keys, lazy set algebra, and first-class reference-valued iterator items. |
| `std::fmt` | Variadic/default-zone formatting policy, per-value streaming `Display` writers beyond the current `Display::format_in` string step, structured logging, scoped filters, and capture/backtrace integration. Fixed runtime templates and writer helpers currently cover one through four placeholders. |
