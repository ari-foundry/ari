...
| `std::sync` | 89% | Concrete atomics with Result compare-exchange, no-poison primitive `Mutex`/`RwLock` guards with explicit drop unlock, value-protecting `MutexValue[T]`/`RwLockValue[T]` payload guards, `Once`, `OnceLock` fallible init status, `Condvar` timeout shape, `Barrier`, and capacity-1 MPSC channels with Result send/recv/recv-timeout errors and clonable sender handles. | Final shorter `Mutex[T]`/`RwLock[T]` spelling after a name-migration/type-overload policy, optional poison-aware types, futex or OS blocking waits, configurable channel capacity, sender-counted close semantics, semaphores, and target-native generic atomics. |
...
| `std::sync` | Final shorter `Mutex[T]`/`RwLock[T]` spelling after a compatibility plan for the primitive names, optional poison-aware lock types, futex or OS blocking waits, configurable channel capacity, sender-counted close semantics, semaphores, and target-native generic atomics. |
