...
| `std::io` | Byte-oriented process IO contracts and hooks. | `Reader`, `Writer`, `Seek`, `Stdin`, `Stdout`, `Stderr`, `Pipe`, `PipeReader`, `PipeWriter`, `Cursor`, `BufReader`, `BufWriter`, `stdin`, `stdout`, `stderr`, `pipe`, `cursor`, `buf_reader`, `buf_writer`, `read_exact`, `write_all`, `flush`, `write_bytes`, `read_line`. |
...
| `std::string` | Zone-backed owned byte string and typed borrowed text-boundary views. | `String`, `Utf8`, `OsStr`, direct string-literal coercion to `Slice[u8]` / `Vec[u8]` / `[u8, N]` / `Utf8` / `OsStr` / `PathBytes` / `CStr`, `utf8`, `os_str`, `c_str`, `c_len`, `c_bytes`, `bytes`, `new`, `empty`, `from`, `copy`, `from_string`, `from_slice_in`, `join_in`, `push`, `try_get`, `try_pop`, `remove`, `try_remove`, `append`, `append_byte`, `append_bytes`, `find_text`, `contains_text`, `split`, `chunks`, `windows`, `push_codepoint_in`, `try_utf8`, `is_utf8`, `codepoint_count`, `codepoint_at`, `equals_text`, `equals_text_ignore_case`, `trim`, `trimmed`, `parse_decimal`, `parse_decimal_prefix`, `as_slice`. `c_str` returns the shared `std::c::CStr` type. |
...
| `std::vec` | Zone-backed growable sequence. | `Vec[T]`, `new<T>`, `push`, `push_in`, `try_get`, `try_remove`, `retain`, `slice`, `split_at`, `find`, `contains_slice`, `compare`, `chunks`, `windows`, `split`, `reverse`, `rotate_left`, `rotate_right`, `sort`, `stable_sort`, `is_sorted`, `binary_search`, `min`, `max`, `as_slice`, `iter`. |
...
select, clamp, or range-check values without compiler-known comparison
intrinsics.
...
`metadata`/`try_metadata`, `try_file_type`, direct path-kind predicates,
...
access in comparator order. Map entry iterators yield `MapEntry[K,V]` values
...
caller-provided buffers, and the process stream hooks. `PipeReader` and
`PipeWriter` adapt `std::os::Pipe` into the same traits. The next IO roadmap
items are file adapters, zone-owning buffered constructors, and resource
flush/drop rules.
