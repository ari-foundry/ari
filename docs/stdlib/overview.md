...
| `std::fs` | Byte-oriented filesystem handles. | `File`, `exists`, `remove`, `open`, `try_open`, `create`, `try_create`, compatibility `open_read`/`open_write`/`open_append`, `read_byte`, `write_byte`, `write_bytes`, whole-file `read`, `write`, `append`, `truncate`, `copy`, `read_to_string`, `close`. |
...
`create`/`try_create`, `write_bytes`, whole-file `read`, `write`, `append`,
`truncate`, `copy`, byte-string `read_to_string`, and the `File` methods are
ordinary Ari source over the raw hooks. The handle is a visible value today and
