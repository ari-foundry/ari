...
| `std::hash` | Deterministic non-cryptographic hashing. | `Hasher`, `Hash[T]`, `new`, `reset`, `finish`, `write`, `value`, `bytes`, fixed-width integer and bool write helpers. |
...
| `std::cmp` | Comparison traits and helpers. | `Eq`, `Ord`, primitive scalar impls, `Ordering`, `compare`, `then_compare`, `min`, `max`, `clamp`, `is_between`. |
...
`Eq`/`Ord` impls and the minimal `Ord[T]::lt` method so generic code can
compare, chain lexicographic ordering, select, clamp, or range-check values
