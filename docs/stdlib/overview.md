...
`status`, `output_in`, `exec`, `Child` handles, `Output` handles, stdout/stderr
capture for small outputs, and `kill`. Large-stream readiness, stdin
redirection, richer status values, and Windows process mapping remain roadmap
work.
