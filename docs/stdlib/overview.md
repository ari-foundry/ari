...
| `std::io` | Byte-oriented process IO contracts and hooks. | `Reader`, `Writer`, `Seek`, `Stdin`, `Stdout`, `Cursor`, `stdin`, `stdout`, `cursor`, `read_exact`, `write_all`, `flush`, `write_bytes`, `read_line`. |
...
`write_all`, `flush`, `Stdin`, `Stdout`, and `Cursor` are source Ari over
`Slice[u8]`, raw pointers, and the single-byte hooks. `stderr`, `pipe`, and
buffered readers/writers stay on the roadmap until owned OS handles and buffer
drop/flush rules are explicit.
