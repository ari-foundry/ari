...
| `std::ascii` | Source-only ASCII byte and slice helpers. | `is_digit`, `is_printable`, `equals_ignore_case`, `starts_with_ignore_case`, `trim`, `parse_decimal`. |
...
borrowed-slice case-insensitive comparison, trimming, and small integer parsing
need no compiler knowledge.
