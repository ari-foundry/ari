...
| `std::time` | Monotonic time, wall-clock time, sleep, deadlines, and UTC calendar values. | `Duration`, `Instant`, `SystemTime`, `Deadline`, `UtcDateTime`, strict and fallible duration constructors, strict and fallible Unix timestamp constructors, `now`, `system_now`, `elapsed`, `sleep`, `timeout`, `timeout_after`, `deadline_at`. |
...
| `std::random` | OS entropy and deterministic non-cryptographic PRNG helpers. | `Prng`, `entropy`, `fill`, `seed`, `from_entropy`, `seed_from_os`, `next`, `below`, `try_below`, `range`, `try_range`, `float`, `fill_from`, `shuffle`. |
...
follow the same strict/fallible split. Use `Instant` and `Deadline` for
elapsed time and timeout policy; use `SystemTime` only for wall-clock
timestamps and convert it to UTC with `to_utc()` when a calendar value is
needed.
