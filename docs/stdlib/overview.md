...
- Keep allocation explicit through `Region` lifetimes and `Allocator`
  capabilities; `Zone` remains the low-level runtime compatibility name, and
  there is no invisible global heap.
...
| `std::c` | C ABI boundary helpers. | `CStr`, `CString`, `Library`, `Symbol`, `from`, `from_ptr`, `from_slice_in`, `from_cstr_in`, `is_null`, `errno`, `error`, `open`, `main_program`, `symbol`, `function`, `close`, `last_error`, `lazy`, `now`, `local`, `global`. |
...
| `std::thread` | Function-pointer and explicit raw-data thread spawn/join, runtime ids, sleep/yield hints, hosted parallelism, fixed-capacity scoped join owners, and explicit thread-local handles. | `ThreadId`, raw `Thread`, `JoinHandle`, `JoinError`, `ThreadResult`, `ThreadGroupResult`, `ThreadScope`, `Builder`, `ThreadLocal`, Result-returning `spawn`, `spawn_raw`, `join`, `join_value`, `try_join`, `try_join_value`, `available_parallelism`, `detach`, advisory `is_finished`, `yield_now`, `sleep`, `id`, `id_raw`, `current`, `is_main`, `scope`, `ThreadLocal` capacity/try-init helpers, and raw/compatibility helpers. |
...
| `std::region` | Preferred public bulk allocation lifetime facade. | `Region`, `create`, `default_capacity`, `allocator`, raw and typed allocation, placement `new`, `promote`, logical counter helpers, preflight helpers, `reset`, `destroy`. |
| `std::allocator` | Public allocation-capability view over region-backed storage. | `Allocator`, `from_region`, `from_zone`, `from_data`, `of`, `of_mut`, raw and typed allocation helpers, logical counter helpers, preflight helpers. |
| `std::zone` | Low-level region runtime and compatibility layer. | `create`, `alloc`, `capacity`, `used`, `remaining`, `alloc<T>`, `alloc_array<T>`, `new<T>`, `promote<T>`, `allocation_zone`, module-scoped metadata compatibility helpers, `reset`, `destroy`. |
...
| `std::collections` | Source collection handles beyond sequences. | Linear `Set[T]` with insertion-order access, set-relationship predicates, and lazy insertion-order set algebra, `Deque[T]`, `RingBuffer[T]`, `LinkedList[T]`, `BinaryHeap[T]`, `PriorityQueue[T]`, target-zone copy across collection families, hash-table `HashMap[K,V]`/`HashSet[T]` with natural `contains_key`/`contains_value`/`get_or` map lookup, String-key borrowed byte-slice lookup/removal helpers, entry update handles, set representative lookup, entry iteration, live-bucket set relationships, and lazy live-bucket set algebra, red-black-tree `TreeMap[K,V]`/`TreeSet[T]` with natural `contains_key`/`contains_value`/`get_or` map lookup, entry update handles, set representative lookup, sorted entry iteration, ordered key/value/entry boundary access, lower/upper bound lookup, key-value removal, direct red-black deletion, ordered-set relationships, and lazy comparator-order set algebra, explicit hash/comparator constructors, lookup, insertion, replacement, removal, reserve, clear, retain filtering for linear/hash/tree collections, FIFO/linked/heap iteration where applicable, live-bucket hash iteration, and sorted tree iteration. |
...
target region. Heap-backed handles can expose an `Allocator` from their own
backing allocation, so natural growth methods such as `vec.push(value)`,
...
Use `region::destroy(region)` or compatibility `zone::destroy(zone)` when a
manually created region is no longer needed. Pointers, strings, vectors,
boxes, and slices derived from that region become invalid after `reset` or
`destroy`, and sema rejects later use where it can track provenance.
...
`std::thread` is the first thread slice. `spawn`, `spawn_raw`, `join`,
`detach`, `is_finished`, `yield_now`, and the raw parallelism hook are
runtime-backed because they call the host threading or process APIs and install
Ari's per-thread runtime id before source code runs. Natural `spawn` returns
`Result[JoinHandle, Error]`, raw-data spawn accepts an explicit
`fn(ptr u8) -> i64` entry plus caller-owned payload pointer, natural join
returns `Result[i64, JoinError]`, `try_join` gives a nonblocking
`Result[Option[i64], JoinError]` path, and `available_parallelism` preserves
host failure as `Result[u64, Error]`. `sleep` delegates to `std::time`, while
`id`, `ThreadId`, `is_main`, `is_join_error`, the `Builder` accessors, and
most `Thread`/`JoinHandle` methods are source helpers. `Builder` records a
requested name and stack size for the LLVM/Linux pthread backend.
`ThreadScope` is a zone-backed fixed-capacity owner for groups of join handles
and returns `ThreadGroupResult` from `join_all`. `ThreadLocal[T]` is an
explicit zone-backed fixed-capacity handle for current-thread values;
compiler-level `thread_local` statics, capturing closures, generic
`JoinHandle[T]`, borrowed scoped threads, richer status values, and send/share
...
`windows_*` helpers classify Windows drive and UNC prefixes without changing
default path behavior. Platform-specific path forms remain future work.
Interior NUL bytes are preserved by lexical helpers and should be rejected
before C-string or hosted filesystem boundaries.
...
capabilities. `allocator::from_region(ref mut region)` captures a capability
from a region, `allocator::of(ref value)` and `allocator::of_mut(ref mut
value)` capture it from a region-backed handle, and `Allocator` can allocate,
report counters, and preflight capacity.
It is currently implemented over the older `ZoneMetadata` bridge so existing
provenance checks keep working.

`std::zone` keeps the current low-level runtime visible. Raw byte allocation
and lifecycle hooks are runtime-backed, while `alloc_array<T>` is source Ari
that packages the common "count times `size_of<T>()` at `align_of<T>()`"
pattern for library authors. `allocation_zone(data)`, `metadata(data)`,
`from_zone(ref mut zone)`, `ZoneMetadata`, and `ZoneBacked` remain
module-scoped compatibility and implementation building blocks for current
region-backed handles; they are no longer root prelude aliases.
