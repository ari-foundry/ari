...
- [std::fmt](fmt.md): `Debug` and `Display` trait dispatch, source
  ...
  scalar text helpers, fixed-arity runtime templates, writer-backed/stdout
  formatting, short writer-backed concatenation helpers, and the current split
...
- [std::parse](parse.md): whole-input ASCII-trimmed decimal, radix, hex,
  ...
  validation/fallback helpers and integer/float parse diagnostics.
