...
Fallible validation, counting, scalar encoding, and decoding helpers use the
natural API names and return `Result[..., std::error::Error]`. The `_optional`
helpers keep the compact compatibility surface when callers only need presence
or absence, `try_*` names are compatibility aliases for those optional helpers,
and `_unchecked` helpers keep the old asserting behavior for trusted input.
...
encoding::validate_utf8(bytes) -> Result[(), std::error::Error]
...
`validate_utf8` returns `Result[(), Error]` for code that composes with other
fallible APIs. Call `utf8_error` when the diagnostic needs the exact byte and
failure category.
...
encoding::encode_utf8_in(ref mut zone, scalar) -> Result[String, std::error::Error]
...
above `U+10FFFF`. `encode_utf8_in` returns `Error(InvalidData)` for invalid
scalars. `encode_utf8_optional_in` and its `try_encode_utf8_in` compatibility
alias return `None` for invalid scalars. `encode_utf8_unchecked_in` is the
asserting compatibility form and panics for invalid scalars.
...
`hex_decoded_len` and `decode_hex_in` return `Error(InvalidData)` for invalid
length or non-hex bytes. `hex_decoded_len_optional`, `decode_hex_optional_in`,
and the older `try_decode_hex_in` compatibility alias return `None` for
invalid input. `decode_hex_unchecked_in` is the asserting compatibility form
and panics for invalid input.
...
accepted by this first slice. `base64_decoded_len` and `decode_base64_in`
return `Error(InvalidData)` for invalid length, alphabet, or padding
placement. The `_optional` helpers and older `try_decode_base64_in`
compatibility alias discard that category into `None`.
...
```ari
match encoding::decode_hex_in(ref mut zone, input) {
  std::Ok(text) => {
    // use text
  }
  std::Err(reason) => {
...
fn decode_known_base64(zone: ref mut Zone, text: Slice[u8]) -> String {
  return encoding::decode_base64_in(zone, text).unwrap();
}
```
...
encoding, optional compatibility scalar encoding, and unchecked scalar
encoding.
