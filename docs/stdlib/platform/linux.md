...
| file open errors | `std::fs::open`, `create`, and `OpenOptions::open` return direct `Result[File, Error]`; `*_optional`/`try_*` helpers intentionally discard the reason, `_unchecked` helpers preserve invalid-handle compatibility, existing `*_result` aliases remain for migration, and `*_raw_result` variants preserve compact raw errno values for compatibility. | Add richer path/action fields after owned error messages land. |
| file read errors | `std::fs::read_result` and `read_to_string_result` return direct `Result[String, Error]` for open and close failures while preserving the existing byte-oriented read loop. | Split EOF from read failure below `read_byte` so mid-read errors can be reported precisely. |
| file query errors | `metadata_result`, `symlink_metadata_result`, `file_type_result`, `mode_result`, `canonicalize_result`, `read_link_result`, `open_dir_result`, `read_dir_result`, and `read_dir_entries_result` preserve hosted `stat(2)`, `lstat(2)`, `realpath(3)`, `readlink(2)`, and directory-open/close failures as `Error`. | Add path/action fields and richer per-entry directory errors after error payloads grow beyond the compact one-word form. |
| file mutation errors | `std::fs::remove_result`, `rename_result`, `create_dir_result`, and `remove_dir_result` return direct `Result[(), Error]`; raw variants stay compatibility-only. | Add structured operation/path context after error payloads grow. |
| file byte-count errors | `std::fs::write_result`, `append_result`, and `copy_result` return `Result[i64, Error]`, preserving successful byte counts while surfacing hosted open/write/close failures as `Error`. | Direct read-error reporting still needs a separate EOF/error split below `read_byte`; raw byte-count variants remain compatibility-only. |
...
| DNS lookup | `std::net::lookup_v4`, `"host:port"` `resolve`, `to_socket_addrs`, and the `ToSocketAddrs` seed use hosted `getaddrinfo` for a one-address IPv4 slice; `_optional`/`try_*` helpers intentionally discard error detail and `_raw_result` helpers expose compact raw compatibility errors. | Add multi-address result storage, service names, canonical-name policy, bracketed IPv6 endpoint parsing, and detailed resolver error mapping. |
| TCP sockets | `std::net::listen`/`connect`, explicit `tcp_listen`/`tcp_connect`, host-port `connect_host`/`tcp_connect_host`, `TcpListener`, and `TcpStream` use hosted `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, `getpeername`, `setsockopt`, `getsockopt`, and `shutdown` for the IPv4 handle slice. TCP listeners/streams expose local IPv4 socket-address lookup, TCP listeners expose reuse-address policy, TCP streams expose peer IPv4 socket-address lookup and nodelay policy, and `TcpStream` adapts to `std::io::Reader`/`Writer` through descriptor byte reads/writes and exposes method-style `read_exact`/`write_all` stream buffer helpers. | Add IPv6 socket handles, lower-overhead bulk send/recv runtime hooks, remaining socket options, and more operation-specific errors. |
| UDP sockets | `std::net::udp_bind` and `UdpSocket` use hosted IPv4 datagram sockets, `bind`, `getsockname`, `sendto`, `recvfrom`, and timeout/nonblocking/reuse-address descriptor policy for the single-byte datagram slice. It exposes local IPv4 socket-address lookup after bind. | Add slice send/receive, `recv_from` address reporting, connected UDP, multicast, and IPv6 UDP. |
