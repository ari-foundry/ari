...
| file open errors | `std::fs::open`, `create`, and `OpenOptions::open` return direct `Result[File, Error]`; `*_optional`/`try_*` helpers intentionally discard the reason, `_unchecked` helpers preserve invalid-handle compatibility, and `*_raw_result` variants preserve compact raw errno values for compatibility. | Add richer path/action fields after owned error messages land. |
| file read errors | `std::fs::read` and `read_to_string` return direct `Result[String, Error]` for open and close failures while preserving the existing byte-oriented read loop. | Split EOF from read failure below `read_byte` so mid-read errors can be reported precisely. |
...
| file mutation errors | `std::fs::remove`, `rename`, `create_dir`, and `remove_dir` return direct `Result[(), Error]`; raw variants stay compatibility-only. | Add structured operation/path context after error payloads grow. |
| file byte-count errors | `std::fs::write`, `append`, and `copy` return `Result[i64, Error]`, preserving successful byte counts while surfacing hosted open/write/close failures as `Error`. | Direct read-error reporting still needs a separate EOF/error split below `read_byte`; raw byte-count variants remain compatibility-only. |
