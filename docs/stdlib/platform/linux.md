...
| DNS lookup | `std::net::lookup_v4`, `lookup_v6`, `"host:port"` and bracketed `"[host]:port"` `resolve`, `to_socket_addrs`, and the `ToSocketAddrs` seed use hosted `getaddrinfo` for a one-address IPv4/IPv6 slice; `_optional`/`try_*` helpers intentionally discard error detail and `_raw_result` helpers expose compact raw compatibility errors. | Add multi-address result storage, service names, canonical-name policy, and detailed resolver error mapping. |
| TCP sockets | `std::net::listen`/`connect`, explicit `tcp_listen`/`tcp_connect`, IPv6-specific `tcp_listen_v6`/`tcp_connect_v6`, host-port `connect_host`/`tcp_connect_host`, `TcpListener`, and `TcpStream` use hosted `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, `getpeername`, `setsockopt`, `getsockopt`, and `shutdown` for IPv4 and IPv6 handles. TCP listeners/streams expose local socket-address lookup, IPv6-specific local/peer helpers, TCP listener reuse-address policy, TCP stream peer-address lookup and nodelay policy, and `TcpStream` adapts to `std::io::Reader`/`Writer` through descriptor byte reads/writes and exposes method-style `read_exact`/`write_all` stream buffer helpers. | Add lower-overhead bulk send/recv runtime hooks, remaining socket options, and more operation-specific errors. |
| UDP sockets | `std::net::udp_bind`, `udp_bind_v6`, and `UdpSocket` use hosted IPv4/IPv6 datagram sockets, `bind`, `getsockname`, `sendto`, `recvfrom`, and timeout/nonblocking/reuse-address descriptor policy for the single-byte datagram slice. It exposes local socket-address lookup after bind, including `local_addr_v6`. | Add slice send/receive, `recv_from` address reporting, connected UDP, and multicast. |
...
| output capture and stdin | `Command::output_in(ref mut zone)` captures small child stdout/stderr into a zone-backed `Output` handle using `pipe(2)` and `dup2(2)`. `spawn_with_stdin_file`, `status_with_stdin_file`, and their `/dev/null`/byte/path variants redirect child stdin at execution time. | Add readiness or nonblocking draining before promising large-output capture, then add pipe-backed streaming stdin and parent-visible setup errors. |
