...
| symbolic-link target reads | `std::fs::try_read_link(ref mut zone, path)` and `read_link(ref mut zone, path)` are backed by hosted `readlink(2)` and copy the stored link target into a zone-owned `String`. | Add no-follow metadata and richer link/error policy before exposing platform-specific link management. |
