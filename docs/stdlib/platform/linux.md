...
| DNS lookup | `std::net::lookup_v4`, `lookup_v4_result`, `"host:port"` `resolve`, `resolve_result`, `to_socket_addrs`, and the `ToSocketAddrs` seed use hosted `getaddrinfo` for a one-address IPv4 slice. | Add multi-address result storage, service names, canonical-name policy, bracketed IPv6 endpoint parsing, and detailed resolver error mapping. |
