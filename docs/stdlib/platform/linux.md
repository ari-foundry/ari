...
| pipe | `std::os::pipe()` is backed by the hosted `pipe(2)` ABI and returns an owning `Pipe` value with separate read/write ends. | Add `pipe2`-style close-on-exec/nonblocking-at-creation policy where host APIs support it, then add `std::io` adapters. |
