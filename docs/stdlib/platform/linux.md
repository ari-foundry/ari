...
| file open errors | `std::fs::open_result`, `create_result`, and `OpenOptions::open_result` return direct `Result[File, Error]`; `*_raw_result` variants preserve compact raw errno values for compatibility. | Add richer path/action fields after owned error messages land. |
