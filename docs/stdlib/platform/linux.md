...
| file open errors | `std::fs::open_result`, `create_result`, and `OpenOptions::open_result` bridge hosted `errno` through compact `std::error::Error.raw()` values. | Promote to direct `Result[File, Error]` after mixed payload storage is the standard result shape. |
| file mutation errors | `std::fs::remove_result`, `rename_result`, `create_dir_result`, and `remove_dir_result` return unit-success `Result[(), i64]` and bridge hosted `errno` through compact `std::error::Error.raw()` values. | Promote to direct `Result[(), Error]` after mixed payload storage is the standard result shape. |
| file byte-count errors | `std::fs::write_result`, `append_result`, and `copy_result` return `Result[i64, i64]`, preserving successful byte counts and hosted open/write/close `errno` values. | Direct read-error reporting still needs a separate EOF/error split below `read_byte`; promote the error payload to `Error` when mixed payload storage is normal. |
