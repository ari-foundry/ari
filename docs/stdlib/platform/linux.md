...
| TCP sockets | `std::net::listen`/`connect`, explicit `tcp_listen`/`tcp_connect`, `TcpListener`, and `TcpStream` use hosted `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, `getpeername`, `setsockopt`, and `shutdown` for the IPv4 handle slice. TCP listeners/streams expose local IPv4 socket-address lookup, and TCP streams expose peer IPv4 socket-address lookup. `TcpStream` adapts to `std::io::Reader`/`Writer` through descriptor byte reads/writes and exposes method-style `read_exact`/`write_all` stream buffer helpers. | Add IPv6 socket handles, lower-overhead bulk send/recv runtime hooks, nodelay/reuse-address options, and more operation-specific errors. |
| UDP sockets | `std::net::udp_bind` and `UdpSocket` use hosted IPv4 datagram sockets, `bind`, `getsockname`, `sendto`, `recvfrom`, and timeout/nonblocking descriptor policy for the single-byte datagram slice. It exposes local IPv4 socket-address lookup after bind. | Add slice send/receive, `recv_from` address reporting, connected UDP, multicast, and IPv6 UDP. |
