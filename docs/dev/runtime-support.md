...
| Linux descriptor APIs | Provide epoll, inotify, fanotify, eventfd, timerfd, signalfd, pidfd, memfd, and optional io_uring wrappers. | `std::os::OwnedFd` owns ordinary descriptors; `std::os::poll_read`/`poll_write` expose single-descriptor readiness; `std::net` uses owned descriptors for hosted IPv4 TCP/UDP and Unix stream sockets. `std::target` reports Linux API-family availability for event APIs that are not exposed yet. | Add richer errno/result policy, multi-descriptor event loops, and target guards before implementing epoll/eventfd/timerfd/signalfd/memfd wrappers. |
| Linux kernel views | Provide safe access to procfs, sysfs, cgroups, namespaces, seccomp, and capabilities where appropriate. | `std::env::executable_path(ref mut zone)` reads `/proc/self/exe` and copies the result into `String`; the rest are platform-roadmap items. | Keep optional and privilege-aware; do not make them portable `std` APIs. |
