...
2. Reuse AST/IR summary package caches for file-backed modules.
   ...
   statements, `drop` statements, method-call expressions, borrow expressions,
   postfix `?`, `??`, prelude macro invocations, `if`/block/`match`
   expressions, or final expression statements, allowing those dependency
   bodies and summary-safe impl methods to materialize from the cache after
   validation. The remaining package-cache work is to extend that path to
   future IR summaries for broad executable function and impl bodies, then skip
