...
3. Stabilize and implement user-defined compile-time meta expansion for
   ...
   `tokens_ends_with(input, "}")` and `input.ends_with("}")`,
   ...
   splice captures from a successful branch condition into `tokens!(...)`
   output with `~name`, so item macros can match declaration input and emit the
   original declaration plus related generated declarations such as a
   constructor `impl` for a captured struct name. They may also
   return token slices
   ...
   as `return type!(ptr input);`. Attribute macros over top-level declarations
   can rewrite the annotated function, struct, enum, trait, or impl declaration
   by returning declaration output; the annotated declaration is supplied as the
   meta input and is omitted unless the output explicitly splices `input` back
   in. Attribute `token_stream -> token_stream` macros can inspect their own
   balanced argument token tree with `attribute_args_present(input)`,
   `attribute_args_empty(input)`, `attribute_args_count(input)`,
   `attribute_args_match(input, "...", "$name", ...)`,
   `attribute_args_capture(input, "name", "...", "$name", ...)`, and
   `attribute_args_slice(input, start, end)`. Captures from
   `attribute_args_match` are available to the selected branch's `tokens!(...)`
   output through the same `~name` splice used for input token captures.
   Attribute,
   ...
   patterns, fixed-range plus named token/span extraction, and token output
   that splices branch captures with `~name`. Non-identity
   ...
   generate related declarations next to the original input. Pattern-position
   `token_stream -> token_stream` bodies can use it to generate one pattern.
   Type-position
   `type -> type` meta
   ...
   User-defined attribute macros can now rewrite top-level function, struct,
   enum, trait, or impl declarations when their reserving `meta fn` returns
   declaration output through `decl!(...)` or `token_stream` output through
   `tokens!(...)`. The attribute macro input is the annotated declaration token
   tree without its attributes; bare uses of the meta parameter name inside the
   output splice that declaration back in, so `return decl!(input fn extra() ->
   i64 { return 1; });` preserves the annotated declaration and inserts another
   declaration. Attribute macro output uses the same parse-and-splice path as
   item macros, including support for generated use, inline module, constant,
   function, struct, enum, trait, or impl declarations and rejection of
   file-backed module imports, nested item macros, and generated meta functions.
   Attribute macro token-stream bodies can branch on their own balanced
   argument tokens and splice captured argument tokens into generated
   declaration output with `~name`. When a macro output declaration carries a
   rewriting attribute, sema expands that generated attribute through the same
   declaration rewrite path before later declaration collection, with a
   recursion cap to keep accidental self-generating attributes finite.
   User attribute
