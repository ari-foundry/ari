...
1. Extend associated items on traits.
   ...
   elements. Keep Ari on trait composition instead of struct inheritance:
   ...
   - [small-follow-up] propagate expected payload types into enum constructor
     arguments so expected-type-driven calls also work inside generic
     `Some(...)` / `Ok(...)`-style payload construction
