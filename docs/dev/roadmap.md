...
1. Start allocator-backed growable `Vec[T]`.
   ...
   `is_empty`, and `as_slice` builtin/method shape checks, while the shared
   constant value model, constant-to-IR literal construction, scalar literal
   folding, constant binary result evaluation, and static integer folding for
