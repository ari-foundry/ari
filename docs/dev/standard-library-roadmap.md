...
- source `std::collections` handles: linear insertion-order `Set[T]` with
  ...
  key-value removal, set representative `get`/`try_get`, map `iter()`
  aliases, `MapEntryMut`-based `iter_mut()` cursors, direct map
  `IntoIterator`, mutable map value cursors, and map/set draining cursors;
