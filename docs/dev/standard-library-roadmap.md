...
| `std::process` | Grow from the current `id`/`uid`/`gid`/`exit`/`abort`/POSIX fork seed into child process handles. | current `id`, `uid`, `gid`, explicit exit status, abort hook lowering, source status/root predicates, `std-process-fork-wait` POSIX child branch and raw wait-status decode, `std-process-result` direct `Error` fork/wait helpers and errno payload checks; future spawn/exec/kill result handling, richer status values, optional daemon helpers, and platform guards. | Current id/uid/gid/exit/abort use runtime hooks; direct process result helpers use hosted POSIX `fork`/`waitpid` and compact `std::error::Error`. Portable spawn/exec/wait/kill needs runtime wrappers for POSIX/Windows split, richer status modeling, and handle ownership. |
