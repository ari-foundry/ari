...
| `std::fs` | Grow from byte-oriented files into owned resource handles, metadata, permission mutation, directory reads, links, temporary files, owned path values, and optional locking. | current tests cover existence/remove, mode-string and `OpenOptions` opens, direct `Result[File, Error]` open/create helpers with raw compatibility variants, whole-file write/append/copy direct `Result[i64, Error]` helpers with raw compatibility variants, unit-success mutation direct `Result[(), Error]` helpers with raw compatibility variants, `Option` reads and opens, truncation, seek/position, `std::io` Reader/Writer/Seek adapters, descriptor views, rename, recursive directory create/remove, non-truncating `ensure_file`, streaming and one-shot directory reads, `DirEntry` lazy metadata, hard/symbolic links, link target reads, no-follow symlink metadata, access permissions, POSIX mode lookup/mutation, target-following metadata and timestamps, and canonicalization; future invalid close, richer per-entry directory errors, temp-file, owned path, richer owner/group/ACL and portable creation/birth-time policy, richer link metadata/platform symlink policy, locking, and structured filesystem error fields. | Current file hooks use LLVM runtime calls and stack-local byte-path adapters over `access`, `stat`, `lstat`, `chmod`, `realpath`, `readlink`, `unlink`, `rename`, `link`, `symlink`, `mkdir`, `rmdir`, `opendir`, `readdir`, `closedir`, `open`, `read`, `write`, `lseek`, and `close`; `OpenOptions` lowers named policy directly to hosted open flags, direct fs results convert errno through compact `std::error::Error`, and raw fs results are compatibility-only. Future richer directory/temp/locking work needs OS-resource ownership/drop policy and platform-specific wrappers. |
