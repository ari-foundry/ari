...
- IO/input/context/env declarations and source helpers such as
  ...
  `io::BufReader`, `io::BufWriter`, `io::stderr`, `io::read_exact_result`,
  `io::copy_result`, `io::write_all_result`, `io::flush_result`,
  `io::read_exact`, `io::read_all`, `io::read_to_string`, `io::try_copy`,
  `io::copy`, `io::write_all`, `io::flush`, `io::write_bytes`,
...
- the first `std::process` current-process helpers: `id`, `uid`, `gid`, `exit`,
  `abort`, and source status/root helpers, plus POSIX `fork`/`wait`,
  `Command`/`Child`/`ExitStatus`/`Output`, `spawn`, `status`, `exit_status`,
  `output_in`, `exec`, `kill`, child environment setup, child working-directory
  setup, typed normal/signal status inspection, and small stdout/stderr capture
- the first `std::thread` helpers: `Thread`, `Builder`, function-pointer
  `spawn`, `join`, advisory `is_finished`, `yield_now`, `sleep`, runtime thread
  ids, available parallelism, and source predicates/method wrappers
- the first `std::sync` helpers: concrete atomic wrappers, explicit
  `Ordering`, source `Mutex`/`RwLock`/`Once`, sync-facing `OnceLock`,
  generation-based `Condvar`, reusable `Barrier`, and a single-slot MPSC
  channel built on the atomic primitive
...
- the first `std::random` helpers: OS entropy through the hosted Linux
  `getrandom` path with `/dev/urandom` fallback, recoverable entropy/fill
  `Result` forms, deterministic non-cryptographic `Prng`, deterministic
  booleans, unbiased bounded integers, unit floats, direct OS byte filling,
  PRNG byte filling, and generic slice shuffle
- the first `std::fs` helpers: file existence/removal, access-permission
  checks, read/write open hooks, byte reads/writes, file cursor
  `position`/`seek`, close, source
  ...
  `create_dir`/`remove_dir`, recursive `create_dir_all`/`ensure_dir_all`,
  `read_to_string`, and `Option[String]` `try_read`/`try_read_to_string`
...
- source `std::collections` handles: linear insertion-order `Set[T]` with
  ...
  fallback `get_or` map lookup, direct `get_mut`/`try_get_mut` mutable value
  access, named `replace`, entry update handles, `remove_entry` key-value
  removal, set representative `get`/`try_get`, map `iter()` aliases,
  `MapEntryMut`-based `iter_mut()` cursors, direct map `IntoIterator`,
  mutable map value cursors, and map/set draining cursors;
...
- `Slice[T]` and `std::vec::Vec[T]` metadata, asserting element access,
  `Option`-returning element access helpers, capacity convenience,
  `extend`/`append`, whole-vector draining, in-place range insert/remove/splice
  helpers, and borrowed range/split/subsequence/compare/chunk/window helpers
  plus receiver-form algorithm wrappers that return views, scalar answers, or
  mutate existing storage without allocating
...
- `std::parse` whole-input ASCII-trimmed signed/unsigned integer, bool, typed
  `Parse` dispatch, and decimal-float parsers with validation/fallback helpers
...
- `std::error` source recoverable error categories, compact error values,
  ...
  `Error`/`ErrorKind` aliases, direct `Result[T, Error]` conversion helpers,
  and raw scalar compatibility bridges for runtime/FFI result shapes
...
- Grow collection families in this order: slice helpers, vector methods,
  ...
  containers, deque/ring-buffer/linked-list/heap families, direct red-black
  deletion, then trait-driven constructors. The iterator slice is now in place for live
...
- Keep `HashMap`/`HashSet` and `TreeMap`/`TreeSet` on explicit
  ...
  select default policies. When that lands, ordinary `new` constructors should
  be trait-driven (`Hash + Eq` for hash collections and `Ord` for tree/heap
  collections), while custom policy stays in explicit `with_hash`/`with_less`
  style constructors.
...
- Add thin wrappers for file, time, process, thread, synchronization, and
  ...
  implemented in `std::os`; filesystem has direct `Error` result forms plus raw
  compatibility bridges for open, mutation, and byte-count operations; portable child-process handles, richer thread
...
| --- | --- | --- | --- |
| `std::io` | Grow from the current `Reader`/`Writer`/`Seek`, `Stdin`, `Stdout`, `Stderr`, `PipeReader`/`PipeWriter`, `Cursor`, caller-buffered `BufReader`/`BufWriter`, `std::fs::File` adapters, direct `Error` helpers `read_exact_result`/`copy_result`/`write_all_result`/`flush_result`, compatibility `read_exact`, `read_all`, `read_to_string`, `try_copy`/`copy`, `write_all`, and `flush` slice into zone-owning buffered constructors and drop-time writer flush. | current `std-io-byte-slice` raw stdout byte-slice output, `std-io-traits-cursor` trait/cursor/exact-read/write-all/flush checks, current `std-io-result` direct `Error` exact-read/write-all/flush/copy checks, current `std-io-read-all` whole-stream collection over `Cursor` and `BufReader[Cursor]`, current `std-io-read-to-string` owned byte-string collection over `Cursor` and `BufReader[Cursor]`, current `std-io-copy` Reader-to-Writer copy count/bool wrappers and writer failure checks, `std-io-stderr` stderr routing and stdout/stderr separation checks, current `std-io-pipe` `std::os::Pipe` ownership splitting, trait-based pipe writes/reads, EOF after writer close, and raw fd read/write hook checks, current `std-io-buffered` caller-provided buffer fill/flush checks, current `std-fs-io-traits` file Reader/Writer adapter checks, and current `std-fs-seek` file Seek adapter checks; future zone-owned buffered storage and drop-time flush policy. | Current stdout/stderr/caller-buffered/pipe/file source slice uses runtime stream, file, descriptor, and seek hooks. Direct IO results map generic writer/read failures into compact `std::error::Error` while raw process hooks stay compatibility boundaries. Future zone-owning buffered wrappers need std raw-buffer provenance support; fully owned file resources still need drop/ownership policy. |
...
| `std::process` | Grow from current-process helpers and POSIX child-process control into portable process handles. | current `id`, `uid`, `gid`, explicit exit status, abort hook lowering, source status/root predicates, `std-process-fork-wait` POSIX child branch and raw wait-status decode, `std-process-result` direct `Error` fork/wait helpers and errno payload checks, `std-process-exit-status` typed `ExitStatus`/`ExitCode` normal/signal inspection, `std-process-command` `Command`/`Child` builder coverage for args, environment, cwd, method and module-level `spawn`, `status`, `exit_status`, `exec`, `kill`, and wait, `std-process-output` method and module-level small stdout/stderr capture plus `Output` accessors, and `std-process-high-level` coverage for explicit-zone `Command::arg`/`env_var`, typed `Signal`, child stream aliases, current path wrappers, and temp file/temp dir; future large-output readiness or nonblocking draining, stdin redirection, richer platform status fields, optional daemon helpers, environment inheritance/clearing policy, Windows mapping, and platform guards. | Current id/uid/gid/exit/abort use runtime hooks; direct process result helpers use hosted POSIX `fork`/`waitpid`; command helpers use `fork`, `execvp`, `setenv`, `chdir`, `pipe`, `dup2`, `kill`, `mkstemp`, and `mkdtemp` with compact `std::error::Error`. Fully portable process handling needs POSIX/Windows split wrappers, richer status modeling, large-stream pipe ownership, and handle lifetime policy. |
...
| `std::fs` | Grow from byte-oriented files into owned resource handles, metadata, permission mutation, directory reads, links, temporary files, owned path values, and optional locking. | current tests cover existence/remove, mode-string and `OpenOptions` opens, direct `Result[File, Error]` open/create helpers with raw compatibility variants, direct `Result[String, Error]` whole-file read helpers, direct `Result` query helpers for metadata/no-follow metadata/path kind/mode/canonicalize/read-link/directory reads, whole-file write/append/copy direct `Result[i64, Error]` helpers with raw compatibility variants, unit-success mutation direct `Result[(), Error]` helpers with raw compatibility variants, `Option` reads and opens, truncation, seek/position, `std::io` Reader/Writer/Seek adapters, descriptor views, rename, recursive directory create/remove, non-truncating `ensure_file`, streaming and one-shot directory reads, `DirEntry` lazy metadata, hard/symbolic links, link target reads, no-follow symlink metadata, access permissions, POSIX mode lookup/mutation, target-following metadata and timestamps, and canonicalization; future EOF/read-error split, invalid close, richer per-entry directory errors, temp-file, owned path, richer owner/group/ACL and portable creation/birth-time policy, richer link metadata/platform symlink policy, locking, and structured filesystem error fields. | Current file hooks use LLVM runtime calls and stack-local byte-path adapters over `access`, `stat`, `lstat`, `chmod`, `realpath`, `readlink`, `unlink`, `rename`, `link`, `symlink`, `mkdir`, `rmdir`, `opendir`, `readdir`, `closedir`, `open`, `read`, `write`, `lseek`, and `close`; `OpenOptions` lowers named policy directly to hosted open flags, direct fs results convert errno through compact `std::error::Error`, and raw fs results are compatibility-only. Future richer directory/temp/locking work needs OS-resource ownership/drop policy and platform-specific wrappers. |
...
| `std::net` | Grow from source address values into runtime-backed sockets. | current `std-net-addresses` IPv4/IPv6 constructors, generic `IpAddr` predicates, socket-address construction, port replacement, loopback, and unspecified checks; current `std-net-address-validation` strict/fallible octet and segment accessor checks; current `std-net-tcp-loopback` IPv6 unsupported errors, hosted IPv4 TCP bind/connect/accept/local-port/local-address/peer-address helpers, direct `Error` and raw compatibility result helpers, `std::time::Duration` and raw millisecond timeout helpers, nonblocking helpers, TCP listener reuse-address, TCP stream nodelay, shutdown, `std::io` stream adapters, method-style `read_exact`/`write_all` stream buffer helpers, explicit close, and restricted-host `PermissionDenied` bridge behavior; current `std-net-udp-socket` IPv4 UDP bind/local-port/local-address/send-byte/receive-byte helpers plus direct `Error`/raw compatibility bind results, `Duration`/raw timeout, nonblocking, reuse-address, and restricted-host checks; current `std-net-unix-socket` Unix stream bind/connect/accept direct `Error`/raw compatibility results, bidirectional byte and buffer IO, `Duration`/raw timeout helpers, nonblocking, shutdown, close, and socket-file cleanup checks; current `std-net-dns-lookup` IPv4 numeric lookup through `lookup_v4`, direct `Error` `lookup_v4_result`, `lookup_v4_raw_result`, `"host:port"` `resolve`/`resolve_result`/`resolve_raw_result`, `ToSocketAddrs`, and host-port connect input validation; future IPv6 socket handles, multi-address DNS iterators, service-name resolution, buffer-oriented UDP send/receive, UDP source address helpers, remaining socket options, timeout-specific error results, and richer error propagation tests. | Current socket slices use LLVM runtime wrappers for `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, `getpeername`, `getaddrinfo`, `sendto`, `recvfrom`, `setsockopt`, `getsockopt`, `shutdown`, and byte read/write over owned descriptors. Future work needs IPv6 sockaddr lowering, multi-result resolver storage, buffer/slice datagram send/receive ABI, UDP source address helpers, remaining socket option wrappers, timeout error classification, and fuller owned socket handle/drop semantics. |
| `std::thread` | Grow from the current plain function-pointer spawn/join handle into safer ownership-transfer and result policy. | current `std-thread-basic` main/child id, spawn/join, invalid-handle, method-wrapper, root `Thread`, and yield checks; current `std-thread-runtime-helpers` available-parallelism and sleep wrapper checks; current `std-thread-builder` builder option surface, configured name/stack spawn hook, and advisory completion checks; current `std-thread-local` explicit `ThreadLocal[T]` handle checks; future moved capture rejection, compiler-level `thread_local` declaration sugar, richer status/result values, shared state diagnostics, and platform guards. | Current pthread trampoline uses a runtime packet and thread-local id, hosted available parallelism uses `sysconf`, configured builder spawn uses pthread attributes plus best-effort `pthread_setname_np`, advisory completion uses `pthread_kill(handle, 0)`, and explicit thread-local handles use zone-backed state plus the Ari thread id on the LLVM host path. Future work needs send/share trait policy, owned handle semantics, TLS destructor policy, and platform-specific wrappers. |
| `std::sync` | Grow from concrete atomics and primitive coordination into value-protecting locks and channels. | current `std-sync-atomic-i64` load/store/swap/fetch-add/compare-exchange, method-wrapper, root alias, and LLVM atomic lowering checks; current `std-sync-mutex-once` primitive mutex lock/unlock and once state checks; current `std-sync-rwlock` explicit read/write state transitions, reader counts, root alias, and method wrappers; current `std-sync-concurrency-api` explicit ordering with hosted LLVM order-specific lowering, `AtomicBool`/`AtomicUsize`/`AtomicPtr`, `OnceLock`, `Condvar`, `Barrier`, and single-slot channel checks; future target-native generic atomics, `Mutex[T]`/`RwLock[T]` guard policy, `LazyLock`, optional `Semaphore`, blocking wake, timeout waits, and futex-backed implementation tests. | Current atomic hooks lower directly to LLVM atomic instructions, including explicit ordering dispatch for load/store/RMW/compare-exchange; source wrappers and synchronization primitives reuse those hooks. The compiler recognizes sync channel handles as standard zone-backed pointer carriers so Sender/Receiver do not store redundant zone fields. Future work needs thread-safety trait checks, guard lifetime modeling, blocking wait/wake runtime hooks, Linux futex internals hidden behind portable APIs, and non-LLVM target atomic policy. |
| `std::cell` | Provide local interior mutability and one-time initialization. | current `std-cell-basic` coverage for `Cell`, `RefCell`, `OnceCell`, `Lazy`, guard drops, fallible borrows, and lazy initialization; future closure/capture initializers, fallible initialization, and stronger move-aware contracts. | Source implementations rely on pointer helper lowering and zone-backed slot provenance for `OnceCell`/`Lazy`. The compiler recognizes those handles as standard zone-backed structs. |
| `std::rc` | Provide shared ownership handles with weak references. | current `std-rc-arc-weak` coverage for `Rc`, `Arc`, `Weak`, clone, downgrade, upgrade, pointer equality, and last-strong drop behavior; future send/share trait checks, unique mutable access, and root shared-owner policy aliases. | Source implementations use an atomic shared control block allocated in a caller zone. The compiler recognizes `Rc`/`Arc`/`Weak` as standard zone-backed handles without storing redundant zone fields. |
| `std::collections` | Add trait-driven constructors after the current queue/list/heap and direct tree-deletion slices. | current set insertion/duplicate/replace/access/optional access/representative lookup/reserve/removal/iteration/copy/after-reset/same-zone tests; current `std-collections-map-mut-access` asserting `get_mut`, optional `try_get_mut` value handles, and named map `replace`; current map entry tests cover `or_insert`, `or_insert_with`, `or_default`, `and_modify`, `insert`, `insert_entry`, direct removal, key/value access, and `remove_entry`; current `std-collections-retain` covers `Set`, `HashSet`, and `HashMap` retain filtering, hash tombstone reuse, and value mutation through hash-map retain predicates; current `std-collections-view-api` map `iter()` aliases, `MapEntryMut`-based `iter_mut()` cursors, direct map `IntoIterator`, mutable value cursors, and map/set drains; hash collision/tombstone and live-bucket iterator tests; tree rotation/replacement, direct red-black deletion with compacting slot movement, and sorted iterator tests; deque circular growth tests; ring-buffer full/overwrite tests; linked-list node reuse tests; binary-heap and priority-queue pop-order tests; future borrowed hash lookup, lazy set algebra iterators, tree retain/range mutation, split/append, `Hash + Eq` default hash constructors, `Ord` default tree/heap constructors, and first-class reference-valued iterator item tests. | Current collection handles have zone provenance recognition; next compiler work is borrowed key view design, trait-driven `Hash`/`Eq`/`Ord` dispatch, default function-value selection for constructor policy, richer comparator policies, and reference-valued iterator item lowering for natural mutable pair cursors. |
| `std::algo` | Grow from production-shaped source slice algorithms into move-aware ordering and slice transformation helpers. | current `std-algo-slice-helpers` sorting/stable sorting, comparator sorting, binary search, lower/upper/equal-range bounds, partition point, min/max/clamp, reverse/rotate, partition, fill, copy, dedup, and swap checks; current `std-algo-final-sort` checks introsort symbols, heapsort fallback symbols, merge-sort stable sort, explicit temporary-zone stable sort, `try_stable_sort*`, stable duplicate-key ordering, and 10,000-element random/sorted/reverse/duplicate-heavy inputs; current `std-vec-range-mutation` checks half-open `copy_within`, `fill_range`, `reverse_range`, and `rotate_range`; current `std-algo-by-helpers` comparator sortedness/search/bounds/equal-range, partition point, min/max/clamp and receiver-wrapper checks over values without `Ord`; current `prelude-slice-sequence` receiver wrappers over the same policies; future ownership-valued algorithm rejection/move policy tests. | Current algorithms need no compiler work for scalar/plain values. Remaining compiler work is move-aware temporary storage/place transfer, first-class `Copy`/ownership constraints, by-reference comparator lowering, true fallible zone allocation, and trait-driven `Ord` dispatch improvements. |
...
| `std::random` | Grow from OS entropy and deterministic PRNG helpers into larger distribution tests and explicit crypto/distribution policy. | current `std-random-basic` entropy hook reachability, deterministic seed behavior, deterministic boolean checks, unbiased bounded integer checks including a wide upper bound, unit float bounds, deterministic byte filling, direct OS byte filling, and generic shuffle checks; current `std-random-result` recoverable entropy/fill result checks and invalid-slice error mapping; future larger byte-buffer fill tests, statistical range tests, and distribution helpers if they belong in core std. | Current `entropy`/`fill` and result variants need LLVM runtime hooks around hosted Linux `getrandom` with `/dev/urandom` fallback; the PRNG, deterministic boolean helper, unbiased bounded integer path, float, deterministic byte-fill, and shuffle helpers are source Ari. Future distribution APIs need clearer policy and richer tests. |
...
| `std::ascii` | Add broader byte-window helpers after collection substring policy grows beyond first-match search. | Byte classification behavior, case-insensitive comparison/search, slice trimming/parsing, unsigned and signed decimal parser behavior, overflow rejection at `i64` boundaries, prefix parser consumed-length behavior, source symbol checks, and future parser edge cases. | None for current whole-slice, prefix, and overflow-checked helpers; wider integer parser APIs may need diagnostics or wider intermediate arithmetic. |
| `std::parse` | Grow from the current whole-input integer/bool/decimal-float slice into richer parse errors and overflow policy. | current `std-parse-basic` signed decimal/radix/hex/binary/octal integer, unsigned decimal/radix integer, bool, float validation/conversion, fallback, trait-backed typed `parse<T>` dispatch, and invalid whole-input cases; future exponent edge, richer parse-error, and locale-rejection tests. | Current helpers are pure source Ari. `parse[f64]`/`parse_or[f64]` provide typed strict/fallback float parsing today; future `Option[f64]` or `Result[f64,E]` needs float enum payload lowering. |
...
| `std::error` | Grow from compact shared error values into platform-specific mappings and richer error payloads. | current `std-error-basic` error-kind predicates, POSIX errno mapping including network `ECONNREFUSED`, root aliases, string names, compact raw bridge, direct `Result[T, Error]` conversion, and raw result compatibility flow; current `std-error-validation` fallible code/raw/errno constructor checks; current `std-error-integration` module aliases, errno-result conversion, connection-refused predicates, conversion traits, and Display/Debug checks; current fs/io/net/process direct `Error` result tests; future Windows last-error mapping, owned messages, and structured error fields. | Current helpers are pure source Ari with a one-word `Error`. Richer messages and structured operation/path fields need a larger error payload policy. |
...
| `std::vec` | Grow from the current safe access/growth/iterator and direct borrowed sequence wrappers toward root/source Vec unification. | Method, `try_*` access, capacity convenience, slice `extend`, iterator `extend_iter`, `append`, `resize_with`, `swap_remove`, `split_off`, whole/range `drain`, range insert/remove/splice, half-open `copy_within`/`fill_range`/`reverse_range`/`rotate_range`, direct `slice`/`split_at`/`find`/`compare`/`chunks`/`windows`/`split`, in-place `reverse`/rotation/introsort/merge-stable-sort/search/equal-range/partition-point/dedup/fill/copy/partition wrappers, explicit-zone and `Result` stable-sort wrappers, `iter`/`iter_mut`, borrow, owner-drop, same-zone, `std-vec-convenience-api`, `std-vec-resize-with`, `std-vec-complete-convenience-api`, `std-vec-range-mutation`, `std-algo-final-sort`, and `std::vec::collect` tests. | Iterator lowering, generic aggregate monomorphization, explicit-zone provenance, clone-aware growth traits, and future move-aware resource-element transfers. |
| `std::iter` | Lazy adapter and eager consumer layer over the canonical `std::Iterator[T]` protocol. | `std-iter-adapters` covers `map`, `filter`, `take`, `skip`, `enumerate`, `zip`, `fold`, `reduce`, and `collect` over Vec cursors. `std-iter-consumers` covers `count`, `count_if`, `nth`, `last`, `find_if`, `position`, `any`, and `all`. `std-iter-repeat-with` covers generator-backed `repeat_with` bounded by `take` and consumed by `collect` / `Vec.extend_iter`. `std-iter-once-empty` covers finite `once` and `empty` source iterators. `std-iter-slice-vec` covers root `SliceIter`, `SliceIterMut` mutable handles, `Vec.iter_mut`, `Vec.extend_iter`, and collection into `Vec`. | Function-pointer callback typing, declaring-module field type resolution, first-class reference-valued iterator items, and general iterator protocol lowering. |
