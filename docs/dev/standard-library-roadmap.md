...
| `std::error` | Grow from compact shared error values into platform-specific mappings and richer error payloads. | current `std-error-basic` error-kind predicates, POSIX errno mapping, root aliases, string names, compact raw bridge, direct `Result[T, Error]` conversion, and raw result compatibility flow; current `std-error-validation` fallible code/raw/errno constructor checks; current `std-error-integration` module aliases, errno-result conversion, conversion traits, and Display/Debug checks; current fs/io/net/process direct `Error` result tests; future Windows last-error mapping, owned messages, and structured error fields. | Current helpers are pure source Ari with a one-word `Error`. Richer messages and structured operation/path fields need a larger error payload policy. |
