...
- `std::source` source-coordinate values for compiler tools: `FileId`,
  ...
  `SourceFile` text lookup, and natural containment, boundary, line, same-file,
  merge, and ordering helpers
...
| `std::source` | Grow from value coordinates and borrowed text lookup into an owned source-map layer for compiler diagnostics. | current `std-source-location` file id, half-open span, one-based line/column, location, containment, touch, same-file merge, ordering, and method wrapper checks; current `std-source-text` borrowed source text, line counts, line starts/ends, line spans, byte-offset locations, EOF locations, and method wrapper checks; future file-name ownership, cached line-start tables, labels, notes, and golden diagnostic rendering tests. | Current helpers are pure source Ari. Future owned source maps mainly need `Vec`/`String` ergonomics and diagnostic formatting, not compiler hooks. |
