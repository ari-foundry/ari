...
- IO/input/context/env declarations and source helpers such as
  ...
  `io::BufReader`, `io::BufWriter`, `io::read_exact`, `io::write_all`,
  `io::flush`, `io::write_bytes`,
...
- the first `std::fs` helpers: file existence/removal, read/write open hooks,
  byte reads/writes, close, source `File` methods, `Option[File]` open
...
- Add thin wrappers for file, time, process, thread, synchronization, and
  ...
  filesystem, POSIX fork/wait process, function-pointer thread, and concrete
  atomic integer slices are implemented; portable child-process handles,
  richer thread statuses, generic synchronization, directories, metadata, and
...
| --- | --- | --- | --- |
| `std::io` | Grow from the current `Reader`/`Writer`/`Seek`, `Stdin`, `Stdout`, `Cursor`, caller-buffered `BufReader`/`BufWriter`, `read_exact`, `write_all`, and `flush` slice into `stderr`, `pipe`, file adapters, zone-owning buffered constructors, and drop-time writer flush. | current `std-io-byte-slice` raw stdout byte-slice output, `std-io-traits-cursor` trait/cursor/exact-read/write-all/flush checks, and `std-io-buffered` caller-provided buffer fill/flush checks; future stderr routing, pipe EOF/close behavior, zone-owned buffered storage, drop-time flush policy, and `File` trait-adapter tests. | No compiler work for the current caller-buffered source slice. Future zone-owning buffered wrappers need std raw-buffer provenance support, while `stderr` and `pipe` need runtime hooks and owned OS handle policy. |
...
| `std::fs` | Grow from byte-oriented files into owned resource handles, metadata, permissions, directory iteration, links, temporary files, path helpers, and optional locking. | current `std-fs-basic` existence/remove, mode-string open/read/write/close, byte-slice write, and `Option[File]` checks; current `std-fs-append` append mode, preservation, and failed append checks; current `std-fs-open-modes` `"r"`/`"w"`/`"a"`/`"rw"`/`"r+"`/`"w+"`/`"a+"`/empty/invalid mode checks; current `std-fs-read-write` whole-file `write`/`append`, read-to-byte-string, missing-file empty read, and truncating rewrite checks; current `std-fs-create-truncate-copy` `create`/`try_create`, natural `read`, `truncate`, source streaming `copy`, and missing-source copy failure; current `std-fs-rename-dir` runtime-backed `rename`, single-directory `create_dir`/`remove_dir`, duplicate-create failure, and missing-source rename failure; future invalid close, metadata, permissions, read directory, recursive directory helpers, hard-link, symlink, canonicalize, temp-file, path, locking, and options-builder tests. | Current file hooks use LLVM runtime calls to `access`, `unlink`, `rename`, `mkdir`, `rmdir`, `open`, `read`, `write`, and `close`; future metadata/permissions/directory-iteration/link/canonicalize/temp/locking work needs OS-resource ownership/drop policy and platform-specific wrappers. |
