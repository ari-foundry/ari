...
- Keep the boundary types distinct in source APIs: C strings are
  NUL-terminated `string`/`CStr`, owned `String` is bytes, `Utf8` means
  validated UTF-8 bytes, `OsStr` means raw OS string bytes, and `PathBytes`
  means bytes under path policy.
