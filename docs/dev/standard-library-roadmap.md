...
| `std::net` | Grow from source address values into runtime-backed sockets. | current `std-net-addresses` IPv4/IPv6 constructors, generic `IpAddr` predicates, socket-address construction, port replacement, loopback, and unspecified checks; current `std-net-address-validation` strict/fallible octet and segment accessor checks; current `std-net-tcp-loopback` IPv6 unsupported errors, hosted IPv4 TCP bind/connect/accept/local-port helpers, `std::io` stream adapters, explicit close, and restricted-host `PermissionDenied` bridge behavior; future DNS lookup, UDP send/receive, Unix domain sockets, IPv6 socket handles, socket options, nonblocking mode, timeout behavior, shutdown, and richer error propagation tests. | Current TCP slice uses LLVM runtime wrappers for `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, and byte read/write over owned descriptors. Future work needs `getaddrinfo`, `send`, `recv`, `setsockopt`, socket-level nonblocking/timeout policy, shutdown, peer/local address helpers, and fuller owned socket handle/drop semantics. |
