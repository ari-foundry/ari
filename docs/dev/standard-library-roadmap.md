...
- the first `std::random` helpers: OS entropy through the hosted Linux
  ...
  non-cryptographic `Prng`, bounded integers, unit floats, byte filling, and
  generic slice shuffle
...
| `std::random` | Grow from OS entropy and deterministic PRNG helpers into fallible entropy, direct byte filling, unbiased bounded integers, and explicit crypto/distribution policy. | current `std-random-basic` entropy hook reachability, deterministic seed behavior, bounded integer checks, unit float bounds, deterministic byte filling, OS byte filling, and generic shuffle checks; future fallible entropy-result tests, larger byte-buffer fill tests, unbiased range tests, and distribution helpers if they belong in core std. | Current `entropy` needs an LLVM runtime hook around hosted Linux `getrandom` with `/dev/urandom` fallback; the PRNG, float, byte-fill, and shuffle helpers are source Ari. Future fallible APIs need richer `Error` result payloads. |
