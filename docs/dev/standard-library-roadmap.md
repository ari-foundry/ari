...
| `std::net` | Grow from source address values into runtime-backed sockets. | current `std-net-addresses` IPv4/IPv6 constructors, generic `IpAddr` predicates, socket-address construction, port replacement, loopback, and unspecified checks; current `std-net-address-validation` strict/fallible octet and segment accessor checks; current `std-net-tcp-loopback` IPv6 unsupported errors, hosted IPv4 TCP bind/connect/accept/local-port helpers, timeout/nonblocking helpers, shutdown, `std::io` stream adapters, explicit close, and restricted-host `PermissionDenied` bridge behavior; current `std-net-udp-socket` IPv4 UDP bind/local-port/send-byte/receive-byte helpers plus timeout/nonblocking and restricted-host checks; current `std-net-unix-socket` Unix stream bind/connect/accept, bidirectional IO, timeout/nonblocking, shutdown, close, and socket-file cleanup checks; current `std-net-dns-lookup` IPv4 numeric lookup through `lookup_v4` and `lookup_v4_result`; future IPv6 socket handles, buffer-oriented send/receive, peer/local address helpers, broader socket options, `std::time::Duration` timeout APIs, and richer error propagation tests. | Current socket slices use LLVM runtime wrappers for `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, `getaddrinfo`, `sendto`, `recvfrom`, `setsockopt`, `shutdown`, and byte read/write over owned descriptors. Future work needs IPv6 sockaddr lowering, buffer/slice send/receive ABI, peer/local address helpers, richer socket option wrappers, and fuller owned socket handle/drop semantics. |
