...
- `std::diag` source diagnostic severities, labels, diagnostic values,
  source-location lookup, and first stderr summary output
...
| `std::diag` | Grow from a single-label diagnostic value into a source-map-backed diagnostic builder and renderer. | current `std-diag-basic` severity helpers, label construction, diagnostic construction, label attachment, source location lookup, and method wrapper checks; current `std-diag-write` stderr summary output; future multi-label builders, notes, fix-its, owned messages, JSON output, golden renderer tests, and source-map integration. | Current helpers are pure source Ari over borrowed byte slices, `std::source`, and `std::log`. Future renderers mainly need owned source maps, richer `String` ergonomics, and golden-test tooling. |
