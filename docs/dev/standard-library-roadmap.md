...
| `std::sync` | Grow from concrete atomics and primitive coordination into value-protecting locks and channels. | current `std-sync-atomic-i64` load/store/swap/fetch-add/compare-exchange, method-wrapper, root alias, and LLVM atomic lowering checks; current `std-sync-mutex-once` primitive mutex lock/unlock and once state checks; current `std-sync-rwlock` explicit read/write state transitions, reader counts, root alias, and method wrappers; current `std-sync-concurrency-api` explicit ordering, `AtomicBool`/`AtomicUsize`/`AtomicPtr`, `OnceLock`, `Condvar`, `Barrier`, and single-slot channel checks; future target-native generic atomics, `Mutex[T]`/`RwLock[T]` guard policy, `LazyLock`, optional `Semaphore`, blocking wake, timeout waits, and futex-backed implementation tests. | Current atomic hooks lower directly to LLVM atomic instructions; source wrappers and synchronization primitives reuse those hooks. The compiler recognizes sync channel handles as standard zone-backed pointer carriers so Sender/Receiver do not store redundant zone fields. Future work needs full memory-order lowering, thread-safety trait checks, guard lifetime modeling, blocking wait/wake runtime hooks, and Linux futex internals hidden behind portable APIs. |
