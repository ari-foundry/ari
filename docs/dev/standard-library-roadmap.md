...
| `std::vec` | Grow from the current safe access/growth/iterator and direct borrowed sequence wrappers toward root/source Vec unification. | Method, `try_*` access, capacity convenience, `extend`/`append`, `drain`, range insert/remove/splice, direct `slice`/`split_at`/`find`/`compare`/`chunks`/`windows`/`split`, in-place `reverse`/rotation/sort/search/equal-range/partition-point/dedup/fill/copy/partition wrappers, iterator, borrow, owner-drop, same-zone, `std-vec-convenience-api`, and `std::vec::collect` tests. | Iterator lowering, generic aggregate monomorphization, and explicit-zone provenance. |
