...
- the first `std::os` helpers: a non-owning `Fd` descriptor view, standard
  descriptor constructors, invalid-descriptor checks, descriptor predicates,
  identity comparison, and `std::fs::File.descriptor()`
...
- Add thin wrappers for file, time, process, thread, synchronization, and
  ...
  implemented; the first non-owning descriptor view is implemented in
  `std::os`; portable child-process handles, richer thread statuses, generic
  synchronization, blocking locks, directories, metadata, owned descriptors,
  and raw OS wrappers still need ownership policy.
...
| `std::os` | Grow from non-owning descriptor views into owned OS resource wrappers. | current `std-os-fd` covers `Fd`, standard descriptors, invalid descriptor, descriptor predicates, identity comparison, and `File.descriptor`; future owned descriptor close/dup, close-on-exec, nonblocking mode, `fcntl`, `ioctl`, `poll`, `select`, Linux `epoll`, `eventfd`, `timerfd`, `signalfd`, optional `pidfd`, optional `memfd`, signals, and memory mappings. | Current `Fd` is pure source Ari. Future wrappers need runtime hooks, errno/error mapping, ownership/drop policy, and target guards. |
