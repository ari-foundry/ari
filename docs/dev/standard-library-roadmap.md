...
| `std::algo` | Grow from the first source slice algorithms into faster sorts, hashing, and encoding families. | current `std-algo-slice-helpers` sorting/stable sorting, comparator sorting, binary search, min/max/clamp, reverse/rotate, partition, fill, copy, dedup, and swap checks; future large-slice sort stress tests, ownership-valued algorithm rejection/move policy tests, hash trait tests, hex/base64 encoding tests, and optional compression fixtures. | Current algorithms need no compiler work. Faster generic algorithms may need move-aware temporary storage, `Copy`/ownership constraints, and trait-driven `Hash`/`Eq`/`Ord` dispatch. |
