...
| `std::error` | Grow from compact shared error values into platform-specific mappings and richer error payloads. | current `std-error-basic` error-kind predicates, POSIX errno mapping, root aliases, string names, compact raw bridge, direct `Result[T, Error]` conversion, and raw result compatibility flow; current `std-error-validation` fallible code/raw/errno constructor checks; future Windows last-error mapping, io/net/process conversion tests, owned messages, formatter integration, and structured error fields. | Current helpers are pure source Ari with a one-word `Error`. Richer messages need owned strings or formatter integration; structured operation/path fields need a larger error payload policy. |
