...
- IO/input/context/env declarations and source helpers such as
  ...
  `io::read_all`, `io::read_to_string`, `io::try_copy`, `io::copy`,
...
- Add thin wrappers for file, time, process, thread, synchronization, and
  ...
  implemented in `std::os`; filesystem has raw-error open, mutation, and
  byte-count result bridges; portable child-process handles, richer thread
...
| `std::fs` | Grow from byte-oriented files into owned resource handles, metadata, permission mutation, directory reads, links, temporary files, owned path values, and optional locking. | current `std-fs-basic` existence/remove, mode-string open/read/write/close, byte-slice write, and `Option[File]` checks; current `std-fs-append` append mode, preservation, and failed append checks; current `std-fs-open-modes` `"r"`/`"w"`/`"a"`/`"rw"`/`"r+"`/`"w+"`/`"a+"`/empty/invalid mode checks; current `std-fs-open-options` named `OpenOptions` read/write/append/truncate/create/create-new opens, exclusive creation, non-truncating read/write, and invalid option checks; current `std-fs-open-result` `open_result`, `create_result`, and `OpenOptions::open_result` raw-error bridges for invalid input, missing files, exclusive-create failures, and successful handles; current `std-fs-read-write` whole-file `write`/`try_write`/`append`/`try_append`, read-to-byte-string, missing-file empty read, and truncating rewrite checks; current `std-fs-byte-result` `write_result`, `append_result`, and `copy_result` byte-count successes plus raw-error open failures; current `std-fs-try-read` `Option[String]` read behavior that distinguishes missing files from empty files; current `std-fs-create-truncate-copy` `create`/`try_create`, natural `read`, `truncate`, source streaming `copy`/`try_copy`, missing-source copy failure, and byte-count checks; current `std-fs-io-traits` covers `File` as generic `std::io::Reader`/`Writer`, file-to-file copy, whole-slice writes, flush, EOF, and invalid handles; current `std-fs-seek` covers `File` as generic `std::io::Seek`, direct `position`/`seek` hooks, method syntax, generic dispatch, and negative seek rejection; current `std-fs-rename-dir` runtime-backed `rename`, single-directory `create_dir`/`remove_dir`, duplicate-create failure, and missing-source rename failure; current `std-fs-create-dir-all` recursive parent creation, existing-directory idempotence, file-path rejection, blocked child creation, and nested writes; current `std-fs-remove-dir-all` recursive tree removal, non-directory root rejection, missing-path failure, empty-directory removal, internal file and symlink unlinking, and no-follow directory symlink behavior; current `std-fs-ensure-file` non-truncating `ensure_file` idempotence, existing-file preservation, missing-file creation, directory rejection, and missing-parent failure; current `std-fs-read-dir` runtime-backed `Dir` open/next/close, one-shot `try_read_dir`/`read_dir`, lightweight `DirEntry` name/path reads, dot-entry skip, missing-directory failure, invalid-handle `None`, and cleanup; current `std-fs-dir-entry-metadata` lazy `DirEntry` target-following metadata, no-follow symlink metadata, file-kind predicates, byte length, permission snapshots, and stored zone path reuse; current `std-fs-links` runtime-backed `hard_link`/`symbolic_link`, read-through checks, missing-source failure, and duplicate-link failure; current `std-fs-read-link` covers `readlink`-backed symbolic-link target reads, `Option[String]` failure for regular/missing paths, and the asserting wrapper; current `std-fs-symlink-metadata` covers no-follow `lstat` metadata, target-following metadata over links, direct `is_symlink` behavior, and missing-path `None`; current `std-fs-permissions` access-style read/write/execute checks, `Permissions` methods, and missing-path behavior; current `std-fs-mode` stat-backed permission mode lookup, chmod-backed mutation, structured `Permissions` constructors/conversion, invalid-mode rejection, and missing-path failure; current `std-fs-metadata` target-following `Option[Metadata]`, byte length, `FileKind`, regular-file and directory predicates, and missing-path behavior; current `std-fs-metadata-times` access, modification, and POSIX status-change timestamps on target-following, no-follow, and `DirEntry` metadata; current `std-fs-canonicalize` `Option[String]` existing-path resolution, absolute paths, filename preservation, and missing-path behavior; current `std-os-fd` covers the non-owning `File.descriptor()` bridge; future invalid close, richer per-entry directory errors, temp-file, owned path, richer owner/group/ACL and portable creation/birth-time policy, richer link metadata/platform symlink policy, locking, and direct `Result[File, Error]` fs errors. | Current file hooks use LLVM runtime calls and stack-local byte-path adapters over `access`, `stat`, `lstat`, `chmod`, `realpath`, `readlink`, `unlink`, `rename`, `link`, `symlink`, `mkdir`, `rmdir`, `opendir`, `readdir`, `closedir`, `open`, `read`, `write`, `lseek`, and `close`; `OpenOptions` lowers named policy directly to hosted open flags, and raw-error fs results bridge errno through compact `std::error` raw values. Future richer directory/temp/locking work needs OS-resource ownership/drop policy and platform-specific wrappers. |
...
| `std::net` | Grow from source address values into runtime-backed sockets. | current `std-net-addresses` IPv4/IPv6 constructors, generic `IpAddr` predicates, socket-address construction, port replacement, loopback, and unspecified checks; current `std-net-address-validation` strict/fallible octet and segment accessor checks; current `std-net-tcp-loopback` IPv6 unsupported errors, hosted IPv4 TCP bind/connect/accept/local-port/local-address/peer-address helpers, `std::time::Duration` and raw millisecond timeout helpers, nonblocking helpers, shutdown, `std::io` stream adapters, method-style `read_exact`/`write_all` stream buffer helpers, explicit close, and restricted-host `PermissionDenied` bridge behavior; current `std-net-udp-socket` IPv4 UDP bind/local-port/local-address/send-byte/receive-byte helpers plus `Duration`/raw timeout, nonblocking, and restricted-host checks; current `std-net-unix-socket` Unix stream bind/connect/accept, bidirectional byte and buffer IO, `Duration`/raw timeout helpers, nonblocking, shutdown, close, and socket-file cleanup checks; current `std-net-dns-lookup` IPv4 numeric lookup through `lookup_v4` and `lookup_v4_result`; future IPv6 socket handles, buffer-oriented UDP send/receive, UDP source address helpers, broader socket options, timeout-specific error results, and richer error propagation tests. | Current socket slices use LLVM runtime wrappers for `socket`, `bind`, `listen`, `accept`, `connect`, `getsockname`, `getpeername`, `getaddrinfo`, `sendto`, `recvfrom`, `setsockopt`, `shutdown`, and byte read/write over owned descriptors. Future work needs IPv6 sockaddr lowering, buffer/slice datagram send/receive ABI, UDP source address helpers, richer socket option wrappers, timeout error classification, and fuller owned socket handle/drop semantics. |
