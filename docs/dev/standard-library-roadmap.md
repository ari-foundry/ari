...
| `std::iter` | Lazy adapter and eager consumer layer over the canonical `std::Iterator[T]` protocol. | `std-iter-adapters` covers `map`, `filter`, `take`, `skip`, `enumerate`, `zip`, `fold`, `reduce`, and `collect` over `Vec` cursors. `std-iter-slice-vec` covers root `SliceIter`, `SliceIterMut` mutable handles, `Vec.iter_mut`, `Vec.extend_iter`, and collection into `Vec`. | Function-pointer callback typing, declaring-module field type resolution, first-class reference-valued iterator items, and general iterator protocol lowering. |
