...
with notes, diagnostic formatting, small parser utilities, and golden-test
tools.
...
| Diagnostics | Basic logging, panic, formatting, test helpers, `std::diag` diagnostic values with one borrowed note, and `std::source` source-coordinate values plus borrowed `SourceFile` and cached `LineMap` line/column lookup exist. | Owned source maps, multi-label diagnostic builders, multiple notes, fix-it text, and stable golden output. |
