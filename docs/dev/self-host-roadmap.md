...
- **36-41% ready** to begin full compiler bootstrapping
...
values, borrowed source text lookup, cached line maps, first diagnostic values,
diagnostic formatting, small parser utilities, and golden-test tools.
...
| Diagnostics | Basic logging, panic, formatting, test helpers, `std::diag` diagnostic values, and `std::source` source-coordinate values plus borrowed `SourceFile` and cached `LineMap` line/column lookup exist. | Owned source maps, multi-label diagnostic builders, notes, fix-it text, and stable golden output. |
