...
- `std_vec_semantics` for source-prelude `std::vec::RawVec<T>` and
  ...
  pointer-return methods such as `Vec.as_ptr()`, keeping allocator-backed Vec
...
- `pointer_memory_semantics` for raw pointer helper, layout-query, and
  `std::mem` value-helper lowering: explicit type-argument checks, pointer
  arithmetic/load/store diagnostics, `size_of<T>` / `align_of<T>` layout
  literals, mutable-place validation, borrow-mark release, hidden temporary
...
- `pattern_semantics` for pure pattern binding/or-pattern detection,
  ...
  direct rest aliases, ownership-carrying known-length suffix guards, and
...
- `meta_token_eval` for token_stream meta return validation/evaluation,
  ...
  end-exclusive slice and named token/span capture extraction used before
...
- `loop_state_semantics` for loop-carried state comparison helpers that let
  ...
  for merging loop state snapshots that are already unavailable or preserve the
  same borrow provenance conservatively
...
- `borrow_return_semantics` for small signature-level borrow-return contracts,
  including the single-source parameter rule and `@borrow_return(source.path)`
