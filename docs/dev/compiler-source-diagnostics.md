...
| Ownership/borrow/drop diagnostics | already SourceId/Span-backed for supported ownership checks | Borrow conflict, use-after-move, moving borrowed owners, partial moves, and unsupported dynamic owner-element moves point at the expression or binding span; `diagnostic-borrow-conflict.diagnostic`, `diagnostic-use-after-move.diagnostic`, `diagnostic-move-borrowed-owner.diagnostic`, `diagnostic-ownership-partial-move.diagnostic`, and `diagnostic-ownership-vector-dynamic-move.diagnostic` are the golden fixtures. |
