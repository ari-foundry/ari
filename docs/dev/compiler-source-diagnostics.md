...
| ABI/FFI diagnostics | already SourceId/Span-backed for common paths | Invalid extern ABI strings, extern bodies, generic extern declarations, invalid link names, unsupported varargs surfaces, `c_void` parameters, non-`@repr(C)` by-value aggregate imports, oversized aggregate imports, and target-unsupported aggregate imports are classified as `A0001` and checked by focused diagnostic artifacts. |
