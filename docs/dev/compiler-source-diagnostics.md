...
| Lexer diagnostics | already SourceId/Span-backed | Tokens and lexer errors carry spans. `diagnostic-unexpected-character.diagnostic`, `diagnostic-lexer-unterminated-string.diagnostic`, `diagnostic-lexer-unterminated-block-comment.diagnostic`, and `diagnostic-lexer-unicode-escape-eof.diagnostic` verify code, source id/path, byte span, line/column, label, snippet, and deterministic rendering for EOF-reaching lexer spans. |
| Parser diagnostics | already SourceId/Span-backed | Expected/unexpected token failures use token spans. `diagnostic-parser-expected.diagnostic`, `diagnostic-parser-top-level.diagnostic`, `diagnostic-parser-malformed-expression.diagnostic`, and `diagnostic-parser-malformed-struct-field.diagnostic` are the representative goldens. |
| Module/import/visibility diagnostics | already SourceId/Span-backed for common paths | Missing modules, ambiguous module candidates, cyclic imports, private access, and imported-file semantic failures preserve the source of the import, qualified access, or child-file expression. `diagnostic-missing-module.diagnostic`, `diagnostic-ambiguous-module.diagnostic`, `diagnostic-cyclic-module.diagnostic`, `diagnostic-private-access.diagnostic`, and `diagnostic-imported-semantic-source-id.diagnostic` verify the artifact rows. |
| Semantic/type/name diagnostics | already SourceId/Span-backed for common paths | Unknown names, duplicate declarations, type mismatch, wrong arity, wrong argument type, invalid return, invalid assignment, non-struct field access, unknown struct fields, match arm type mismatches, empty matches, non-matchable scrutinees, duplicate/unreachable arms, payload-pattern arity, wrong-enum cases, and non-exhaustive enum matches are covered by focused diagnostic artifacts. Shared type, value-construction, pattern, constant, vector, slice, pointer-memory, zone-allocation, format, meta, attribute, borrow-call, borrow-return, and borrow helper diagnostics now construct `CompileError(SourceLocation, message)` directly instead of reparsing a rendered `where(loc)` prefix. |
...
| ABI/FFI diagnostics | already SourceId/Span-backed for common paths | Invalid extern ABI strings, extern bodies, generic extern declarations, invalid link names, unsupported varargs surfaces, `c_void` parameters, non-`@repr(C)` by-value aggregate imports, oversized aggregate imports, target-unsupported aggregate imports, oversized C-header exports, and target-unsupported C-header exports are classified as `A0001` and checked by focused diagnostic artifacts. |
| Ownership/borrow/drop diagnostics | already SourceId/Span-backed for supported ownership checks | Borrow conflict, use-after-move, live-owner return/break/continue exits, moving borrowed owners, match-arm ownership state joins, invalid enum payload moves, undropped stored or matched enum owners, conditional runtime-owned enum payload-slot moves, compact enum payload ref-pattern rejection, ownership- or borrow-carrying aggregate enum payload rejection, partial moves, and unsupported dynamic or temporary owner-element moves point at the expression, binding, payload pattern, type payload, or control-flow keyword span; `diagnostic-borrow-conflict.diagnostic`, `diagnostic-use-after-move.diagnostic`, `diagnostic-return-live-owner.diagnostic`, `diagnostic-loop-break-live-owner.diagnostic`, `diagnostic-loop-continue-live-owner.diagnostic`, `diagnostic-move-borrowed-owner.diagnostic`, `diagnostic-match-branch-state-mismatch.diagnostic`, `diagnostic-enum-payload-invalid-move.diagnostic`, `diagnostic-stored-owned-enum-payload-undropped.diagnostic`, `diagnostic-match-runtime-owned-enum-payload-undropped.diagnostic`, `diagnostic-runtime-owned-enum-conditional-payload-slot-move.diagnostic`, `diagnostic-compact-enum-payload-ref.diagnostic`, `diagnostic-ownership-aggregate-enum-payload.diagnostic`, `diagnostic-borrow-aggregate-enum-payload.diagnostic`, `diagnostic-ownership-partial-move.diagnostic`, `diagnostic-ownership-vector-dynamic-move.diagnostic`, and `diagnostic-ownership-temporary-element-move.diagnostic` are the golden fixtures. |
...
| `CompileError(where(loc) + text)` | SourceLocation-backed transitional path | Older specialized, generated, backend, and internal helpers that still accept strings parse structured `:bytes=start..end` prefixes back into source-aware labels. Keep this path only while migrating call sites; common lexer, parser, semantic helper, and ownership helper diagnostics should use `CompileError(SourceLocation, message)` directly. |
...
| Line lookup | Byte offset to line/column mapping and one-based line/column back to byte offset. | empty file, one-line, multi-line, no-final-newline EOF, CRLF trailing-newline EOF, CRLF hidden columns, and UTF-8 byte-column policy in `tests/source_map_unit.cpp`. |
