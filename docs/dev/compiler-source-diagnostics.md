...
| Lexer diagnostics | already SourceId/Span-backed | Tokens and lexer errors carry spans. `diagnostic-unexpected-character.diagnostic`, `diagnostic-lexer-unterminated-string.diagnostic`, and `diagnostic-lexer-unterminated-block-comment.diagnostic` verify code, source id/path, byte span, line/column, label, snippet, and deterministic rendering for EOF-reaching lexer spans. |
