...
10. Define reads and borrows: the active arm should be usable only after a
   ...
   fields. Local aliases made from field loads or from destructuring patterns
   preserve the same proof requirement, so `fragment.0` is rejected until the
   alias is matched.
