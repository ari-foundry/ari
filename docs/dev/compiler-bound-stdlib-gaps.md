...
| `union by` | Parser and AST preserve `union by <selector> { arm => Type, ... }`; syntax/declaration tooling can print it. Sema validates earlier-field selector roots, nested struct-field selector segments, unique arm names, arm payload type references, and exact enum-case arm coverage for enum selectors. Enum-selector fields lower to internal enum storage, struct literals construct them with `field: arm => payload`, and statically visible selector values must match the constructor arm. | Remaining compiler work covers non-enum discriminant policy, public active-arm narrowing/match syntax, selector-mutation rules, active-arm drop diagnostics, and stable ABI naming. | `src/parser.cpp`, `src/ast.hpp`, `src/sema.cpp`, `src/llvm_codegen.cpp`, `docs/language/generic-aggregates.md`, `tests/cases/structs/ok/union-by-constructor.ari`, `tests/cases/structs/errors/union-by-constructor-unknown-arm.ari`, `tests/cases/structs/errors/union-by-constructor-selector-mismatch.ari`, `tests/cases/compiler-development/artifact/ok/syntax-union-by-field.*`, `tests/cases/compiler-development/artifact/errors/diagnostic-union-by-*.{ari,diagnostic}` |
...
8. Define reads and borrows: the active arm should be usable only after a
   discriminant check, match, or equivalent narrowing operation.
9. Define layout and ABI. The payload storage is union-like, but ownership/drop
   must run only for the active arm.
10. Extend positive fixtures after narrowing and active-arm drop behavior are
    real. Until then, ordinary enum ADTs remain the most ergonomic way to read
    the active payload.
