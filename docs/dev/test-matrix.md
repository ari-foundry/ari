...
| Structural capability parameters | Initial free-function and inherent `impl` method `has method(...) -> Type` plus grouped `has { method(...) -> Type, other(...) -> Type }` parameter requirements lower to hidden generics; explicit function/method generic bounds such as `T: has method(...) -> Type` name the structural type; non-generic capability aliases such as `type Serializable = has serialize() -> i64;` can be reused as supported function/method bounds; all forms check the concrete call-site type for matching static methods and monomorphize through ordinary method dispatch | unsupported `has` positions, rejected `interface` spelling, missing method, wrong non-receiver parameter type/count, wrong return type, capability aliases used as storage types or unsupported generic contexts, accidental dynamic dispatch expectations, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | parser diagnostic fixture covers unsupported type-position `has`; positive trait fixtures cover single, grouped, explicit generic-bound, inherent method, associated-function dispatch, and alias-bound dispatch; negative fixtures cover missing method, unsupported generic-bound positions, alias-as-type, generic alias, unsupported alias-bound positions, and return mismatch; remaining work is richer named-trait guidance, trait-method policy, generic capability aliases, and any future extension beyond method requirements | partial |
