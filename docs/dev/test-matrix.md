...
| Structural capability parameters | Initial free-function `has method(...) -> Type` and grouped `has { method(...) -> Type, other(...) -> Type }` parameter requirements lower to hidden generics, check the concrete call-site type for matching static methods, and monomorphize through ordinary method dispatch | unsupported `has` positions, rejected `interface` spelling, missing method, wrong non-receiver parameter type/count, wrong return type, accidental dynamic dispatch expectations, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | parser diagnostic fixture covers unsupported type-position `has`; positive trait fixtures cover single and grouped method dispatch; negative fixtures cover missing method and return mismatch; remaining work is generic impl-method satisfaction, richer named-trait guidance, and any future alias design | partial |
