...
public non-generic payload-bearing `@repr(C)` enum structs, and generated
fixed-array, tuple, fixed-capacity-vector, and aggregate-enum wrapper typedefs
are exposed for direct aggregate ABI values. Oversized,
...
- [x] root `Vec[T]` direct/function-pointer parameters, trait/impl method
      ...
      ABI, while returns, extern parameters/returns, trait method return types,
      and direct or generic-instantiated struct fields stay rejected until the
      owned runtime-capacity layout is defined; dyn dispatch erases those trait
      method parameters to the same view ABI
