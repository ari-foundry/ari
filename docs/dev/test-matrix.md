...
| Context and process runtime | `context::argc`, `context::arg`, source `context::has_arg`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `process::id`, `process::exit`, source process status helpers, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future environment variables, future child process handles | host runtime init, `getpid`, `exit`, plus source helper monomorphization | partial |
