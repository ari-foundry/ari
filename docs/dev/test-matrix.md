...
- [x] expose source `Option[T]` and `Result[T, E]` through the source prelude,
      with value-receiver `is_some`/`is_none`/`is_ok`/`is_err`, `unwrap_or`,
      and consuming `map`/`and_then`/`map_err` combinator methods implemented
      in `std::option` and `std::result`
