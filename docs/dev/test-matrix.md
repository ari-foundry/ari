...
| Prelude IO/format/input/assert | `print`, `println`, `std::print`, `std::println`, `std`-alias qualified print names, `{}` and `{:.N}` format placeholders, escaped braces, formatted `f32`/`f64` output on LLVM and freestanding, `print!`, `println!`, `format_in!` explicit-zone source `String` construction for `{}` lowercase `string`/integer/bool/`f32`/`f64`/Display values and `{:.N}` float precision with once-evaluated function-call/computed arguments, `matches!`, IO aliases, `read_byte`, host `read_line`/`input`, explicit-zone owned line helpers returning source `std::string::String`, root `String` / `std::String` aliases for the source string handle, source string append helpers for lowercase `string`, `i64`, bool, and `f64`, source `Option[T]` / `Result[T, E]` / `Slice[T]` and `slice(data, len)`, Slice `len`/`is_empty`/indexing/indexed assignment/range slicing/read/search/comparison methods, mutable local array/Vec `as_slice()` view creation, assertion function and macro helpers | bad placeholder count, nonliteral format, bad precision placeholder syntax/type, `format!` without an implicit allocation zone, unsupported `format_in! f128`/non-Display values, freestanding line input, owned line/root String use after zone reset, mismatched string append/growth zone, bad assert type, bad assert macro type, bad `matches!` arity/patterns, removed `Maybe[T]` alias | stdio, getchar/fgets, syscall byte IO, decimal float formatting, source string copying/appending for owned line and explicit text construction, assertion exits, pattern-engine macro lowering | partial |
...
- [x] source `std::string::String` same-zone `append_string_in`,
      `append_i64_in`, `append_bool_in`, and `append_f64_in` build
...
- [x] expose source `Slice[T]` and `slice(data, len)` as a non-owning
      ...
      checks; the same positive surface now runs through the raw freestanding
