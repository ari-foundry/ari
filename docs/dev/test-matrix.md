...
- [x] resolve source `std` child modules as package files under `lib/std/` for
      ...
      `std::input`, `std::io`, `std::iter`, `std::mem`, `std::string`,
      `std::vec`, and `std::zone`
