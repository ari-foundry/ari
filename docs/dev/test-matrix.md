...
| Standard parsing | `std::parse::Parse`, `parse<T>`, `parse_or<T>`, `is_parse<T>`, Result-returning signed `integer`/`integer_radix`, unsigned `unsigned`/`unsigned_radix`, integer `ParseError` diagnostics, hex/binary/octal signed wrappers, `boolean`, `_optional` compatibility forms, `is_float`, Result-returning `float`, `float_optional`, `float_or`, and `float_unchecked` over ASCII-trimmed `Slice[u8]` values | future exponent edge, float range diagnostic, and locale-rejection tests | pure source helper monomorphization today; generic calls now accept string-literal-to-slice coercion, natural integer/bool/float parser names return `std::error::Error` categories, and enum payload lowering supports `f64` inside `Option`/`Result` | partial |
...
- [x] source `std::parse` exposes integer `ParseError` diagnostics with
      `ParseErrorKind` and trimmed-input byte offsets
