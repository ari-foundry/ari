...
| Structural capability parameters | Reserved `has method(...) -> Type` parameter requirements such as `fn save(x: has serialize() -> String)` after the named-trait diagnostic path is solid | rejected `interface` spelling, accidental dynamic dispatch, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | targeted parser diagnostic fixture now reserves the spelling; future type-checking, trait-diagnostic, lowering, and positive fixtures must implement the capability before users can rely on it | partial |
