...
| Structural capability parameters | Planned `has method(...) -> Type` parameter requirements such as `fn save(x: has serialize() -> String)` after the named-trait diagnostic path is solid | rejected `interface` spelling, accidental dynamic dispatch, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | future parser, type-checking, trait-diagnostic, and lowering fixtures; capability inventory entry `structural-capability-parameters` tracks the work until implementation exists | planned |
| Discriminant-linked union fields | Planned `union by` fields whose active payload type follows a stable discriminant field or explicit context path | unstable selector expressions, non-exhaustive arms, unsafe active-arm borrowing, active-arm drop mistakes, and confusion with ordinary enum ADTs or unchecked C unions | future aggregate, ownership/drop, match/narrowing, layout, and diagnostic fixtures; capability inventory entry `union-by-fields` tracks the work until implementation exists | planned |
