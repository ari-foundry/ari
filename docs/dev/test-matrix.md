...
| Context, environment, and process runtime | `context::argc`, `context::arg`, source `context::has_arg`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `env::get`, `env::has`, `env::try_get`, `env::set`, `env::remove`, `process::id`, `process::exit`, source process status helpers, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future current-directory helpers, future child process handles | host runtime init, `getenv`, `setenv`, `unsetenv`, `getpid`, `exit`, plus source helper monomorphization | partial |
