...
| --- | --- | --- | --- | --- |
| Standard library | source `std` root and child modules, prelude aliases, `Option`/`Result` methods, `Slice` safe access/search/copy helpers, `Box`, `String` byte/try/search/ASCII case/trim-view/trim-copy/whole-parse/prefix-parse helpers, `ascii` byte classification/case-compare/case-search/slice/prefix-parser helpers, `bits` mask/rotate/power/alignment/zero-run/one-run scan helpers, source `Vec` safe access/growth/iterator helpers, source `std::collections::Set` linear membership/access/optional access/replace/reserve/iteration/copy helpers, real `HashMap`/`HashSet` hash table helpers, real `TreeMap`/`TreeSet` red-black tree helpers, `iter`, `cmp`, `convert`, `fmt`, `math` sign predicate/i64/division-rounding helpers, IO/input/context/env/process including current-process environment variables, current directory, executable path, explicit zone allocation, `std_api_manifest` guard, and a cross-library smoke program | missing or partial `std`, invalid builtin declarations, wrong helper arity/type, invalid ownership and borrow use, wrong zone provenance, same-zone growth violations, unsupported smart-pointer names, invalid formatting inputs | source function monomorphization, reserved `extern "ari"` runtime hooks, LLVM layout and pointer helpers, environment/path and process id/exit hooks, zone allocation calls, Drop lowering for handles, iterator lowering, source math/ascii byte/slice/bits/env/collections/process helper symbols, executable and optional LLVM-driver checks | good first pass |
...
| Context, environment, and process runtime | `context::argc`, `context::arg`, `context::thread_id`, source `context::has_args`, `context::has_arg`, `context::user_arg_count`, `context::has_user_args`, `context::is_main_thread`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `env::get`, `env::has`, `env::try_get`, `env::set`, `env::remove`, `env::current_dir`, `env::try_current_dir`, `env::set_current_dir`, `env::executable_path`, `env::try_executable_path`, `process::id`, `process::exit`, source process status helpers, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future child process handles, future path normalization, future nonzero spawned-thread ids | host runtime init, thread-local `@ari_thread_id`, `getenv`, `setenv`, `unsetenv`, `getcwd`, `chdir`, `readlink`, `getpid`, `exit`, plus source helper monomorphization | partial |
...
- [x] source `std::collections::HashMap<K,V>`/`HashSet<T>` and
      ...
      replacement, reserve growth, reset/destroy invalidation, and same-zone
      growth diagnostics
