...
| --- | --- | --- | --- | --- |
| Standard library | source `std` root and child modules, prelude aliases, `Option`/`Result` methods, `Slice` safe access/search/copy/subslice/split/chunk/window/compare helpers, `Box`, `String` byte/try/search/split/chunk/window/join/typed text-kind/ASCII case/UTF-8 scalar/trim-view/trim-copy/whole-parse/prefix-parse helpers, `ascii` byte classification/case-compare/case-search/slice/prefix-parser helpers, `parse` whole-input integer/bool/float helpers, `encoding` ASCII/UTF-8/UTF-16 validation plus UTF-8 scalar decode/encode and hex/base64 codecs, `path` lexical separator/PathBytes/component/join/normalization helpers, `bits` mask/rotate/power/alignment/zero-run/one-run scan helpers, source `Vec` safe access/growth/direct borrowed sequence/iterator helpers, source `std::collections::Set` linear membership/access/optional access/replace/reserve/iteration/copy helpers, real `HashMap`/`HashSet` hash table and live-bucket iterator helpers, real `TreeMap`/`TreeSet` red-black tree and sorted iterator helpers, `Deque`/`RingBuffer`/`LinkedList` queue and list helpers, `BinaryHeap`/`PriorityQueue` priority helpers, `iter`, `cmp`, `convert`, `fmt`, `hash`, `math` sign predicate/i64/division-rounding helpers, IO/input/context/test/log/target/env/process/thread/sync/time/fs/net including `Reader`/`Writer`/`Seek`, `Cursor`, `BufReader`, `BufWriter`, `stdin`/`stdout`/`stderr`, `read_exact`, `write_all`, `flush`, executable `test::Report` aggregation and scratch zones, level-prefixed `std::log` stderr diagnostics, compiler-known target triple/arch/OS/libc/object/debug/errno/syscall facts, current-process environment variables, current directory, executable path, process id/uid/gid/exit/abort/fork/wait, thread spawn/join/yield/runtime ids, concrete atomic i64 load/store/swap/fetch-add/compare-exchange plus source mutex/once, monotonic/wall-clock time, deadlines/timeouts, UTC calendar conversion, sleep, file existence/permissions/open/read/write/append/read_to_string/close/remove, network IP/socket-address values, explicit zone allocation, `std_api_manifest` guard, and a cross-library smoke program | missing or partial `std`, invalid builtin declarations, wrong helper arity/type, invalid ownership and borrow use, wrong zone provenance, same-zone growth violations, unsupported smart-pointer names, invalid formatting inputs, future test runner/source-location/structured-logging/backtrace gaps | source function monomorphization, reserved `extern "ari"` runtime hooks, LLVM layout and pointer helpers, IO trait helper monomorphization, test helper generic monomorphization, log helper stderr output, target constants, environment/path, process id/uid/gid/exit/abort/fork/wait, thread pthread spawn/join/yield hooks, LLVM atomic load/store/RMW/CAS hooks, time clock/sleep hooks, filesystem access/descriptor hooks, zone allocation calls, Drop lowering for handles, iterator lowering, source math/ascii/parse/encoding/path byte/slice/bits/hash/test/log/target/env/collections/process/thread/sync/time/fs/net helper symbols, executable and optional LLVM-driver checks | good first pass |
...
| Standard random | `std::random::entropy`, direct OS byte `fill`, deterministic non-cryptographic `Prng`, `seed`, `from_entropy`, `seed_from_os`, raw `next`, bounded integer `below`/`range`, unit `float`, PRNG byte `fill_from`, and generic `shuffle` over `Slice[T]` views | future fallible entropy result tests, larger byte-fill tests, unbiased bounded integer policy, CSPRNG stream policy, and distribution helpers if they belong in core std | `entropy` and `fill` are hosted runtime hooks using Linux `getrandom` with `/dev/urandom` fallback; PRNG helpers and shuffle are source Ari | partial |
| Standard parsing | `std::parse::integer`, `boolean`, `is_float`, `float_or`, and `float` over ASCII-trimmed `Slice[u8]` values | future overflow, exponent edge, and richer parse-error tests | pure source helper monomorphization today; `Option[f64]`/`Result[f64,E]` needs float enum payload lowering | partial |
| Standard encoding | `std::encoding::is_ascii`, `utf8_count`, `is_utf8`, `utf8_width`, `utf8_at`, `utf8_next_index`, `utf8_encoded_len`, `encode_utf8_in`, `utf16_count`, `is_utf16`, hex length/encode/decode guards, and base64 length/encode/decode guards | future URL-safe base64, MIME line wrapping, decode error values, normalization/transcoding, and optional compression tests | pure source helper monomorphization today; fallible `String` decoders need zone-backed enum payload support | partial |
| Standard paths | `std::path::is_separator`, `is_absolute`, `is_relative`, `trim_trailing_separators`, `components`, `file_name`, `parent`, `extension`, `stem`, `join_in`, and `normalize_in` over byte paths | future Windows drive/UNC paths, owned path buffers, richer component kinds, and canonicalization tests | pure source helper monomorphization today; future canonicalization needs runtime filesystem wrappers | partial |
| Context, environment, process, thread, sync, and filesystem runtime | `context::argc`, `context::arg`, `context::thread_id`, source `context::has_args`, `context::has_arg`, `context::user_arg_count`, `context::has_user_args`, `context::is_main_thread`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `env::get`, `env::has`, `env::try_get`, `env::set`, `env::remove`, `env::current_dir`, `env::try_current_dir`, `env::set_current_dir`, `env::executable_path`, `env::try_executable_path`, `process::id`, `process::uid`, `process::gid`, `process::exit`, `process::abort`, `process::fork`, `process::wait`, source process status/root/branch/error helpers, `thread::spawn`, `thread::join`, `thread::yield_now`, `thread::id`, `thread::is_main`, `thread::is_join_error`, `Thread` method wrappers, `sync::load`, `sync::store`, `sync::swap`, `sync::fetch_add`, `sync::compare_exchange`, `sync::try_lock`, `sync::lock`, `sync::unlock`, `sync::call_once`, `AtomicI64`, `Mutex`, and `Once` method wrappers, `fs::exists`, `fs::can_read`, `fs::can_write`, `fs::can_execute`, `fs::permissions`, `fs::remove`, `fs::rename`, `fs::hard_link`, `fs::symbolic_link`, `fs::create_dir`, `fs::remove_dir`, mode-string `fs::open`/`fs::try_open` with `"r"`, `"w"`, `"a"`, `"rw"`, `"r+"`, `"w+"`, and `"a+"`, `fs::create`, `fs::try_create`, compatibility `fs::open_*`/`fs::try_open_*` wrappers, `File` byte read/write/close helpers, source `fs::read`/`fs::write`/`fs::append`/`fs::truncate`/`fs::copy`/`fs::read_to_string`, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future portable child process handles, spawn/exec/kill, signal mask/action handling, memory mapping, future path normalization, future richer thread statuses, future generic atomics/shared-state, value-lock, channel, and futex-backed blocking policy, future owned filesystem resource policy, future metadata/permission-mutation/directory-iteration/recursive-directory/canonical/temp-file/locking filesystem slices plus richer link metadata/platform symlink policy | host runtime init, thread-local `@ari_thread_id`, `getenv`, `setenv`, `unsetenv`, `getcwd`, `chdir`, `readlink`, `getpid`, `getuid`, `getgid`, `exit`, `abort`, `fork`, `waitpid`, `pthread_create`, `pthread_join`, `sched_yield`, LLVM atomic load/store/RMW/CAS, source mutex/once helpers, `access`, `unlink`, `rename`, `link`, `symlink`, `mkdir`, `rmdir`, `open`, `read`, `write`, `close`, plus source helper monomorphization | partial |
...
- [x] source `std::parse` exposes ASCII-trimmed whole-input `integer`,
      `boolean`, `is_float`, `float_or`, and `float` helpers with natural
      names and focused invalid-input coverage
...
- [x] source `std::path` exposes POSIX-style lexical path separator,
      component, component-iterator, join, and lightweight normalization
      helpers
...
- [x] source `std::collections::Set<T>` exposes a tracked linear
      ...
      `clear`, `reserve`, `reserve_extra`, `as_slice`, `iter`, and `copy_to`,
      plus reset/destroy invalidation, iterator invalidation, and same-zone
      growth diagnostics
...
- [x] add source IO contracts and cursor helpers: `io::Reader`,
      ...
      `io::read_exact`, `io::write_all`, and `io::flush`
...
- [x] expose source `Slice[T]` and `slice(data, len)` as a non-owning
      ...
      element search, and `Slice[T]` exact/prefix/suffix checks,
      provenance-preserving `as_ptr`, plus `copy_to(ref mut Zone)` into
