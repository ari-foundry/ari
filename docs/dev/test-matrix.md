...
| Structural capability parameters | Initial free-function and inherent `impl` method `has method(...) -> Type` plus grouped `has { method(...) -> Type, other(...) -> Type }` parameter requirements lower to hidden generics; explicit function/method generic bounds such as `T: has method(...) -> Type` name the structural type; both forms check the concrete call-site type for matching static methods and monomorphize through ordinary method dispatch | unsupported `has` positions, rejected `interface` spelling, missing method, wrong non-receiver parameter type/count, wrong return type, accidental dynamic dispatch expectations, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | parser diagnostic fixture covers unsupported type-position `has`; positive trait fixtures cover single, grouped, explicit generic-bound, inherent method, and associated-function dispatch; negative fixtures cover missing method, unsupported generic-bound positions, and return mismatch; remaining work is richer named-trait guidance, trait-method policy, and any future alias design | partial |
