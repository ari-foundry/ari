...
| Discriminant-linked union fields | Reserved `union by` fields whose active payload type follows a stable discriminant field or explicit context path; parser/AST now preserve selector and arm types for tooling | unstable selector expressions, non-exhaustive arms, unsafe active-arm borrowing, active-arm drop mistakes, and confusion with ordinary enum ADTs or unchecked C unions | targeted type diagnostic fixture now proves syntax reaches sema before lowering; future aggregate, ownership/drop, match/narrowing, layout, and positive fixtures must implement the capability before users can rely on it | partial |
