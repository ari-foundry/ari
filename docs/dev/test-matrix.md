...
| Prelude IO/format/input/assert | `print`, `println`, `std::print`, `std::println`, `std`-alias qualified print names, `{}` and `{:.N}` format placeholders, escaped braces, formatted `f32`/`f64` output on LLVM and freestanding, `print!`, `println!`, `matches!`, IO aliases, `read_byte`, host `read_line`/`input`, explicit-zone owned line helpers returning source `std::string::String`, root `String` / `std::String` aliases for the source string handle, source `Option[T]` / `Result[T, E]` / `Slice[T]` and `slice(data, len)`, Slice `len`/`is_empty`/indexing/indexed assignment/range slicing, mutable local array/Vec `as_slice()` view creation, assertion function and macro helpers | bad placeholder count, nonliteral format, bad precision placeholder syntax/type, planned `format!`, freestanding line input, owned line/root String use after zone reset, bad assert type, bad assert macro type, bad `matches!` arity/patterns, removed `Maybe[T]` alias | stdio, getchar/fgets, syscall byte IO, decimal float formatting, source string copying for owned line input, assertion exits, pattern-engine macro lowering | partial |
