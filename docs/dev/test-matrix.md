...
| Shared libraries | compile library, Ari-mangled symbols, `@export`, `@export("symbol")`, `@no_mangle`, `--shared` public/default and private/hidden LLVM visibility | main not required only with `--shared`, invalid export symbols | dynamic symbol table | good first pass |
