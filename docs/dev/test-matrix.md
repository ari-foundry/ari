...
| Operators and literals | arithmetic, modulo, bit ops, bitwise-not, shifts, comparisons, logical ops, postfix `?` with compatible residual conversion, generic prelude `Option`/`Result` and non-generic Maybe/Result-style `??`, integer base prefixes, C/Unicode string escapes, integer and float-width explicit casts, `f32`/`f64`/`f128` LLVM ops, `f32`/`f64` freestanding arithmetic/comparisons | non-bool logic, non-integer bit ops, invalid `?`, incompatible `?` residuals, invalid `??`, invalid base/escape literals, invalid casts, freestanding integer/float casts/calls/f128 ops | LLVM IR and freestanding x86-64, with raw `f32`/`f64` support through local literal, raw pointer bit storage, SSE arithmetic, ordered comparisons, and width casts | good first pass |
