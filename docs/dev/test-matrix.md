...
| Structural capability parameters | Initial free-function, inherent `impl` method, trait method, and trait impl method `has method(...) -> Type` plus grouped `has { method(...) -> Type, other(...) -> Type }` parameter requirements lower to hidden generics; duplicate method names in one capability set are rejected; explicit function/method generic bounds such as `T: has method(...) -> Type` name the structural type; reusable capability aliases such as `type Serializable = has serialize() -> i64;` and generic aliases such as `type Mapper[Input, Output] = has map(Input) -> Output;` can be reused as supported function/method bounds; trait impl conformance compares structural method signatures exactly; all executable forms check the concrete call-site type for matching static methods and monomorphize through ordinary method dispatch | unsupported `has` positions, rejected `interface` spelling, duplicate capability method requirements, missing method, wrong non-receiver parameter type/count, wrong return type, trait/impl structural-bound mismatch, capability aliases used as storage types or unsupported generic contexts, alias arity mistakes, accidental dynamic dispatch expectations, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | parser diagnostic fixture covers unsupported type-position `has`; positive trait fixtures cover single, grouped, explicit generic-bound, inherent method, associated-function dispatch, alias-bound dispatch, generic alias-bound dispatch, and trait method contracts; negative fixtures cover duplicate method requirements, missing method, unsupported generic-bound positions, alias-as-type, alias arity, unsupported alias-bound positions, trait method structural-bound mismatch, and return mismatch; remaining work is richer named-trait guidance and any future extension beyond method requirements | partial |
| Discriminant-linked union fields | `union by` fields whose active payload type follows a stable discriminant field; parser/AST preserve selector and arm types for tooling, sema validates earlier-field selector roots, nested struct-field selector segments, unique arm names, arm payload type refs, exact enum-case arm coverage, exact bool `false`/`true` arm coverage, enum/bool selector struct literals with natural `field: arm(payload)` syntax or compatibility `field: arm => payload`, struct-payload shorthand `field: arm { field: value }`, direct enum/bool selector fields and selector-only nested paths inferred from omitted selectors, direct field matches with subject-type case resolution, and direct selector-path, selector-ancestor, or payload-field assignment rejection | unstable selector expressions, unknown constructor arms, missing bool arms, bool arms other than `false`/`true`, constructor arms that disagree with statically visible selector values, non-struct payload shorthand, conflicting omitted direct or nested selector inference, direct selector or payload mutation after construction, non-enum/non-bool discriminant policies, unsafe active-arm borrowing outside direct matches, active-arm drop mistakes, and confusion with ordinary enum ADTs or unchecked C unions | syntax artifact plus targeted diagnostics cover missing selector roots, missing nested selector fields, duplicate arms, missing enum-case arms, unknown enum-case arms, unknown constructor arms, same-literal selector mismatches, bool arm coverage/name diagnostics, shorthand non-struct payload diagnostics, direct and nested selector inference conflicts, and selector/payload-field assignment rejection; `make check-structs` covers executable enum-selector construction, bool-selector construction, bool call-style construction, struct-payload shorthand construction, direct selector inference, nested selector inference, direct field match reading, and selector/payload assignment diagnostics; future aggregate ownership/drop, broader narrowing, runtime selector consistency, non-enum/non-bool discriminant arm matching, and ABI fixtures remain | partial |
