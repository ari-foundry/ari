...
- [x] source `std::collections::Set<T>` exposes a tracked linear
      ...
      `index_of`, `insert`, `remove`, `take`, `pop`, `try_pop`, `clear`,
      `reserve`, `reserve_extra`, `as_slice`, `iter`, and `copy_to`, plus
      reset/destroy invalidation, iterator invalidation, and same-zone growth
      diagnostics
