...
| Structural capability parameters | Initial free-function, inherent `impl` method, trait method, and trait impl method `has method(...) -> Type` plus grouped `has { method(...) -> Type, other(...) -> Type }` parameter requirements lower to hidden generics; duplicate method names in one capability set are rejected; field/property-style requirements are rejected as method-only; explicit function/method generic bounds such as `T: has method(...) -> Type` name the structural type; reusable capability aliases such as `type Serializable = has serialize() -> i64;` and generic aliases such as `type Mapper[Input, Output] = has map(Input) -> Output;` can be reused as supported function/method bounds; trait impl conformance compares structural method signatures exactly; all executable forms check the concrete call-site type for matching static methods and monomorphize through ordinary method dispatch | unsupported `has` positions, rejected `interface` spelling, duplicate capability method requirements, field/property requirement attempts, missing method, wrong non-receiver parameter type/count, wrong return type, trait/impl structural-bound mismatch, capability aliases used as storage types or unsupported generic contexts, alias arity mistakes, accidental dynamic dispatch expectations, ambiguous structural method lookup, and diagnostics that hide when a named trait is the better API boundary | parser diagnostic fixture covers unsupported type-position `has`; positive trait fixtures cover single, grouped, explicit generic-bound, inherent method, associated-function dispatch, alias-bound dispatch, generic alias-bound dispatch, and trait method contracts; negative fixtures cover duplicate method requirements, field/property requirement attempts, missing method, unsupported generic-bound positions, alias-as-type, alias arity, unsupported alias-bound positions, trait method structural-bound mismatch, and return mismatch; remaining work is richer named-trait guidance and any future extension beyond method requirements | partial |
