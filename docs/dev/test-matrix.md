...
and generated fixed-array, tuple, fixed-capacity-vector, and aggregate-enum
wrapper typedefs are exposed for direct aggregate ABI values. Oversized,
...
| Primitive types | signed/unsigned widths, suffix literals, bool, float/string host values, LLVM local `f32`/`f64` literal storage and raw pointer copies | literal ranges, invalid casts, invalid suffixes | exact host types, width-aware LLVM scalar local loads/stores, byte-packed standalone scalar locals, raw `f32`/`f64` literal and pointer bit storage, LLVM backend tuple/struct/fixed-array byte layout, local aggregate enum layout, external ABI still pending | partial |
