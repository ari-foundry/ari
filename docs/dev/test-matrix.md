...
| --- | --- | --- | --- | --- |
| Standard library | source `std` root and child modules, prelude aliases, `Option`/`Result` methods, `Slice`, `Box`, `String`, source `Vec`, `iter`, `cmp`, `convert`, `fmt`, IO/input/context, explicit zone allocation, `std_api_manifest` guard, and a cross-library smoke program | missing or partial `std`, invalid builtin declarations, wrong helper arity/type, invalid ownership and borrow use, wrong zone provenance, same-zone growth violations, unsupported smart-pointer names, invalid formatting inputs | source function monomorphization, reserved `extern "ari"` runtime hooks, LLVM layout and pointer helpers, zone allocation calls, Drop lowering for handles, iterator lowering, executable and optional LLVM-driver checks | good first pass |
