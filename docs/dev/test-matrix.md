...
| --- | --- | --- | --- | --- |
| Standard library | source `std` root and child modules, prelude aliases, `Option`/`Result` methods, `Slice` safe access/search/copy helpers, `Box`, `String` byte/try/search/ASCII case/trim-view/trim-copy/whole-parse/prefix-parse helpers, `ascii` byte classification/case-compare/case-search/slice/prefix-parser helpers, `bits` mask/rotate/power/alignment/zero-run/one-run scan helpers, source `Vec` safe access/growth/iterator helpers, source `std::collections::Set` linear membership/access/optional access/replace/reserve/iteration/copy helpers, real `HashMap`/`HashSet` hash table and live-bucket iterator helpers, real `TreeMap`/`TreeSet` red-black tree and sorted iterator helpers, `iter`, `cmp`, `convert`, `fmt`, `math` sign predicate/i64/division-rounding helpers, IO/input/context/env/process/thread/time/fs including current-process environment variables, current directory, executable path, process id/exit/fork/wait, thread spawn/join/yield/runtime ids, monotonic/wall-clock time, sleep, file existence/open/read/write/append/close/remove, explicit zone allocation, `std_api_manifest` guard, and a cross-library smoke program | missing or partial `std`, invalid builtin declarations, wrong helper arity/type, invalid ownership and borrow use, wrong zone provenance, same-zone growth violations, unsupported smart-pointer names, invalid formatting inputs | source function monomorphization, reserved `extern "ari"` runtime hooks, LLVM layout and pointer helpers, environment/path, process id/exit/fork/wait, thread pthread spawn/join/yield hooks, time clock/sleep hooks, filesystem descriptor hooks, zone allocation calls, Drop lowering for handles, iterator lowering, source math/ascii byte/slice/bits/env/collections/process/thread/time/fs helper symbols, executable and optional LLVM-driver checks | good first pass |
...
- [x] resolve source `std` child modules as package files under `lib/std/` for
      ...
      `std::boxed`, `std::cmp`, `std::collections`, `std::context`, `std::convert`, `std::env`, `std::fmt`,
      `std::process`,
      `std::ascii`, `std::bits`, `std::input`, `std::io`, `std::iter`, `std::mem`, `std::option`,
