...
| Discriminant-linked union fields | `union by` fields whose active payload type follows a stable discriminant field; parser/AST preserve selector and arm types for tooling, sema validates earlier-field selector roots, nested struct-field selector segments, unique arm names, arm payload type refs, exact enum-case arm coverage, enum-selector struct literals construct fields with `field: arm => payload`, direct enum selector fields and selector-only nested paths can be inferred from omitted selectors, direct field matches bind arm payloads with subject-type case resolution, and direct selector-path, selector-ancestor, or payload-field assignments are rejected | unstable selector expressions, unknown constructor arms, constructor arms that disagree with statically visible selector values, conflicting omitted direct or nested selector inference, direct selector or payload mutation after construction, non-enum discriminant policies, unsafe active-arm borrowing outside direct matches, active-arm drop mistakes, and confusion with ordinary enum ADTs or unchecked C unions | syntax artifact plus targeted diagnostics cover missing selector roots, missing nested selector fields, duplicate arms, missing enum-case arms, unknown enum-case arms, unknown constructor arms, same-literal selector mismatches, direct and nested selector inference conflicts, and selector/payload-field assignment rejection; `make check-structs` covers executable enum-selector construction, direct selector inference, nested selector inference, direct field match reading, and selector/payload assignment diagnostics; future aggregate ownership/drop, broader narrowing, runtime selector consistency, non-enum discriminant arm matching, and ABI fixtures remain | partial |
