...
| Functions | `main`, calls, named params, unit-returning functions, tuple/fixed-array/`Slice[T]` runtime-sequence/struct/tuple-struct/alias/enum/wildcard parameter patterns, local `Vec[T]` parameters via capacity-specialized lowering, return values, recursion, void functions, function pointer values/calls, generics surface | missing `main`, wrong `main`, wrong arg count, duplicate concrete function paths, bad parameter pattern type, missing return, bad return type, private module function, root `Vec[T]` in function return and function pointer parameter/return signatures before runtime-capacity ABI | host and freestanding calls, function-entry parameter destructuring, indirect calls, unit return lowering, freestanding narrow scalar return normalization, freestanding direct `f32`/`f64` parameters and returns, freestanding direct/function-pointer aggregate parameters and returns through hidden pointers, LLVM local-Vec parameter specializations | good first pass |
...
- [x] root `Vec[T]` non-local ABI/storage use is rejected for function and
      extern parameters/returns, generic function specializations, function
      pointer signatures, trait method signatures, direct or
      generic-instantiated struct fields, and impl receivers until the
      runtime-capacity layout is defined
