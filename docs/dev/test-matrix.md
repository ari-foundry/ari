...
| Discriminant-linked union fields | Reserved `union by` fields whose active payload type follows a stable discriminant field; parser/AST preserve selector and arm types for tooling, and sema validates earlier-field selector roots, nested struct-field selector segments, unique arm names, arm payload type refs, and exact enum-case arm coverage before rejecting lowering | unstable selector expressions, non-enum discriminant policies, unsafe active-arm borrowing, active-arm drop mistakes, and confusion with ordinary enum ADTs or unchecked C unions | syntax artifact plus targeted diagnostics cover validated lowering rejection, missing selector roots, missing nested selector fields, duplicate arms, missing enum-case arms, and unknown enum-case arms; future aggregate, ownership/drop, match/narrowing, layout, non-enum discriminant arm matching, and positive fixtures must implement the capability before users can rely on it | partial |
