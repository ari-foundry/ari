...
| Context, environment, process, and filesystem runtime | `context::argc`, `context::arg`, `context::thread_id`, source `context::has_args`, `context::has_arg`, `context::user_arg_count`, `context::has_user_args`, `context::is_main_thread`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `env::get`, `env::has`, `env::try_get`, `env::set`, `env::remove`, `env::current_dir`, `env::try_current_dir`, `env::set_current_dir`, `env::executable_path`, `env::try_executable_path`, `process::id`, `process::exit`, `process::fork`, `process::wait`, source process status/branch/error helpers, `fs::exists`, `fs::remove`, mode-string `fs::open`/`fs::try_open` with `"r"`, `"w"`, `"a"`, `"rw"`, `"r+"`, `"w+"`, and `"a+"`, compatibility `fs::try_open_*` wrappers, `File` byte read/write/close helpers, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future portable child process handles, future path normalization, future nonzero spawned-thread ids, future owned filesystem resource policy | host runtime init, thread-local `@ari_thread_id`, `getenv`, `setenv`, `unsetenv`, `getcwd`, `chdir`, `readlink`, `getpid`, `exit`, `fork`, `waitpid`, `access`, `unlink`, `open`, `read`, `write`, `close`, plus source helper monomorphization | partial |
