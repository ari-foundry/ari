...
| Standard paths | `std::path::is_separator`, `is_absolute`, `is_relative`, `trim_trailing_separators`, `components`, `file_name`, `parent`, `extension`, `stem`, `join_in`, and `normalize_in` over byte paths | future Windows drive/UNC paths, owned path buffers, and richer component kinds | pure source helper monomorphization today; filesystem-backed existing-path canonicalization is covered by `std::fs` | partial |
| Context, environment, process, thread, sync, and filesystem runtime | `context::argc`, `context::arg`, `context::thread_id`, startup `context::cwd`, startup `context::executable_path`, source `context::has_args`, `context::has_arg`, `context::user_arg_count`, `context::has_user_args`, `context::is_main_thread`, startup cwd/executable optional and OS/path view helpers, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `env::get`, `env::has`, `env::try_get`, `env::set`, `env::remove`, `env::current_dir`, `env::try_current_dir`, `env::set_current_dir`, `env::executable_path`, `env::try_executable_path`, `process::id`, `process::uid`, `process::gid`, `process::exit`, `process::abort`, `process::fork`, `process::wait`, source process status/root/branch/error helpers, `thread::spawn`, `thread::join`, `thread::yield_now`, `thread::id`, `thread::is_main`, `thread::is_join_error`, `Thread` method wrappers, `sync::load`, `sync::store`, `sync::swap`, `sync::fetch_add`, `sync::compare_exchange`, `sync::try_lock`, `sync::lock`, `sync::unlock`, `sync::call_once`, `AtomicI64`, `Mutex`, and `Once` method wrappers, `fs::exists`, `fs::can_read`, `fs::can_write`, `fs::can_execute`, `fs::permissions`, `fs::metadata`, `fs::try_metadata`, `fs::mode`, `fs::try_mode`, `fs::set_mode`, `fs::set_permissions`, `fs::canonicalize`, `fs::try_canonicalize`, `Metadata`, `FileKind`, `fs::remove`, `fs::rename`, `fs::hard_link`, `fs::symbolic_link`, `fs::create_dir`, `fs::remove_dir`, `fs::open_dir`, `fs::try_open_dir`, `fs::read_dir`, `fs::try_read_dir`, `fs::read_dir_entries`, `fs::try_read_dir_entries`, `fs::read_dir_next`, `fs::close_dir`, `Dir`, `DirEntry`, mode-string `fs::open`/`fs::try_open` with `"r"`, `"w"`, `"a"`, `"rw"`, `"r+"`, `"w+"`, and `"a+"`, `fs::create`, `fs::try_create`, compatibility `fs::open_*`/`fs::try_open_*` wrappers, `File` byte read/write/close helpers, source `fs::read`/`fs::try_read`/`fs::write`/`fs::try_write`/`fs::append`/`fs::try_append`/`fs::truncate`/`fs::copy`/`fs::try_copy`/`fs::read_to_string`/`fs::try_read_to_string`, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future portable child process handles, spawn/exec/kill, signal mask/action handling, memory mapping, future path normalization, future richer thread statuses, future generic atomics/shared-state, value-lock, channel, and futex-backed blocking policy, future owned filesystem resource policy, future metadata-bearing directory-entry/recursive-directory/temp-file/locking filesystem slices plus richer owner/group/ACL/timestamps/no-follow symlink metadata and platform symlink policy | host runtime init, thread-local `@ari_thread_id`, `getenv`, `setenv`, `unsetenv`, `getcwd`, `chdir`, `readlink`, `getpid`, `getuid`, `getgid`, `exit`, `abort`, `fork`, `waitpid`, `pthread_create`, `pthread_join`, `sched_yield`, LLVM atomic load/store/RMW/CAS, source mutex/once helpers, `access`, `stat`, `chmod`, `realpath`, `unlink`, `rename`, `link`, `symlink`, `mkdir`, `rmdir`, `opendir`, `readdir`, `closedir`, `open`, `read`, `write`, `close`, plus source helper monomorphization | partial |
