...
- [x] `std::boxed::new<T>(ref mut Zone, value)` wraps placement construction in
      a tracked source `std::boxed::Box<T>` handle with `get`/`set`/`as_ptr`
      methods and reset/destroy invalidation
