...
- [x] expose source `Option[T]` and `Result[T, E]` through the source prelude,
      ...
      consuming `unwrap_or`, Option `map`/`and_then`/`flatten`/`transpose`,
      and Result `map`/`and_then`/`transpose`/`map_err`/`or` combinator
      methods implemented in `std::option` and `std::result`
