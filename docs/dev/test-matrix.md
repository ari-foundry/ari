...
| Standard random | `std::random::entropy`, direct OS byte `fill`, deterministic non-cryptographic `Prng`, `seed`, `from_entropy`, `seed_from_os`, raw `next`, unbiased bounded integer `below`/`range`, unit `float`, PRNG byte `fill_from`, and generic `shuffle` over `Slice[T]` views | future fallible entropy result tests, larger byte-fill tests, statistical bounded integer tests, CSPRNG stream policy, and distribution helpers if they belong in core std | `entropy` and `fill` are hosted runtime hooks using Linux `getrandom` with `/dev/urandom` fallback; PRNG helpers, unbiased bounded integer sampling, and shuffle are source Ari | partial |
