...
| Primitive types | signed/unsigned widths, suffix literals, bool, float/string host values | literal ranges, invalid casts, invalid suffixes | exact host types, width-aware freestanding scalar local loads/stores, raw backend stack packing and ABI layout still pending | partial |
