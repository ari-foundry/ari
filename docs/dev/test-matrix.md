...
| Standard random | `std::random::entropy`, direct OS byte `fill`, deterministic non-cryptographic `Prng`, `seed`, `from_entropy`, `seed_from_os`, raw `next`, `boolean`, unbiased bounded integer `below`/`range`, unit `float`, PRNG byte `fill_from`, and generic `shuffle` over `Slice[T]` views | future fallible entropy result tests, larger byte-fill tests, statistical bounded integer tests, CSPRNG stream policy, and distribution helpers if they belong in core std | `entropy` and `fill` are hosted runtime hooks using Linux `getrandom` with `/dev/urandom` fallback; PRNG helpers, deterministic booleans, unbiased bounded integer sampling, and shuffle are source Ari | partial |
| Standard parsing | `std::parse::integer`, `integer_radix`, `hex_integer`, `binary_integer`, `octal_integer`, `boolean`, `is_float`, `float_or`, and `float` over ASCII-trimmed `Slice[u8]` values | future overflow, exponent edge, and richer parse-error tests | pure source helper monomorphization today; `Option[f64]`/`Result[f64,E]` needs float enum payload lowering | partial |
...
- [x] source `std::parse` exposes ASCII-trimmed whole-input `integer`,
      ...
      and focused invalid-input coverage
