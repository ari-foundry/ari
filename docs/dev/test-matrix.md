...
| Context runtime | `context::argc`, `context::arg`, source `context::has_arg`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future environment variables | host runtime init plus source helper monomorphization | partial |
...
- [x] resolve source `std` child modules as package files under `lib/std/` for
      ...
      `std::boxed`, `std::cmp`, `std::context`, `std::convert`, `std::env`, `std::fmt`,
