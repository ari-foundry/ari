...
| Standard encoding | `std::encoding::is_ascii`, `utf8_count`, `is_utf8`, `utf16_count`, `is_utf16`, hex length/encode/decode guards, and base64 length/encode/decode guards | future URL-safe base64, MIME line wrapping, decode error values, and optional compression tests | pure source helper monomorphization today; fallible `String` decoders need zone-backed enum payload support | partial |
