...
| Context, environment, and process runtime | `context::argc`, `context::arg`, `context::thread_id`, source `context::has_args`, `context::has_arg`, `context::user_arg_count`, `context::has_user_args`, `context::is_main_thread`, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, `env::get`, `env::has`, `env::try_get`, `env::set`, `env::remove`, `process::id`, `process::exit`, source process status helpers, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, future current-directory helpers, future child process handles, future nonzero spawned-thread ids | host runtime init, thread-local `@ari_thread_id`, `getenv`, `setenv`, `unsetenv`, `getpid`, `exit`, plus source helper monomorphization | partial |
