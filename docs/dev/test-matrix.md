...
| Standard random | `std::random::entropy`, OS byte `fill`, deterministic non-cryptographic `Prng`, `seed`, `from_entropy`, `seed_from_os`, raw `next`, bounded integer `below`/`range`, unit `float`, PRNG byte `fill_from`, and generic `shuffle` over `Slice[T]` views | future fallible entropy result tests, direct runtime byte fill tests, unbiased bounded integer policy, CSPRNG stream policy, and distribution helpers if they belong in core std | `entropy` is a hosted runtime hook using Linux `getrandom` with `/dev/urandom` fallback; PRNG helpers and shuffle are source Ari | partial |
