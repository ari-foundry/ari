...
arrays, structs, fixed-capacity vector storage, and aggregate-layout enums,
then distinguishes direct 64-bit Unix values from indirect or unsupported
target/layout cases. Header rendering is narrower than classification:
public `@repr(C)` structs, public non-generic payload-bearing `@repr(C)` enum
...
`diagnostic-match-nonexhaustive.diagnostic`, the
`diagnostic-match-struct-pattern-*` goldens, and the match arm-validation
goldens lock source-aware aggregate and match failure artifacts for non-struct
...
| Core language | calls, multiple parameters, recursion, `let`, `var`, assignment, scalar inference, arithmetic, modulo, comparisons, boolean ops, bitwise ops, shifts, unary `-`, logical `!`, explicit casts, block expressions, `if`/`else`, nested `if`, `while`, `for` over ranges, `break`, `continue`, and returns from nested blocks/branches through `make check-core-language` | wrong call arity/type, missing/wrong returns, immutable/type-mismatched assignment, use before declaration, invalid binary/unary operators, invalid casts, break/continue outside loops, and branch expression type mismatches | runtime exit-code smoke, LLVM symbol/control-flow/operator greps, typed IR core scalar-flow artifact, and diagnostic artifact for use-before-declaration | complete for current executable core |
...
| Generics | generic functions, explicit `<T>` generic call type arguments, angle-bracket generic type annotations including nested `Type<Other<T>>`, generic type aliases with `type Name[T] = Target[T];`, repeated function specializations, generic function names specialized as function pointer values from expected `fn(...) -> ...` types, generic structs, explicit and inferred generic struct literal / tuple-struct construction, generic inherent impl methods with `self` receivers, explicit and inferred method-level generic calls on inherent impl methods, generic inherent associated functions, explicit and inferred method-level generic associated calls, generic trait impl methods with `self` receivers, explicit and inferred generic trait method calls, method-level bounds on generic trait methods, generic trait impl associated functions, trait-qualified associated function calls with explicit implementing type arguments and expected-result inference, generic impl bounds, generic trait impl coherence, generic traits, generic ADT surface, explicit/payload-inferred/expected-type generic enum constructors, generic enum constants, generic enum `?` / `??`, generic enum match and refutable payload bindings | inference conflicts, uninferred parameters, uninferred generic function pointer parameters, explicit type-argument arity/mismatch, duplicate generic parameters, type alias arity mismatch, recursive type aliases, unsatisfied generic impl bounds, overlapping generic/concrete trait impls, generic method arity mismatch, uninferred generic method parameters, generic trait method bound mismatch, generic associated arity mismatch, uninferred generic associated parameters, ambiguous trait associated functions, generic enum constructor errors, generic enum `??` fallback type mismatch, generic enum payload pattern type mismatch, generic enum constant mismatch | monomorphized function calls, alias expansion before layout and codegen, specialized generic function pointer refs, specialized generic impl method and associated calls, aggregate surface validation, generic enum LLVM aggregate construction, try/coalesce extraction, compact payload pattern extraction, and inline generic enum constants | good first pass |
| Minimum trait subset | trait declarations, concrete and generic impls, impl conformance, duplicate/overlapping impl rejection, concrete and bound static dispatch, trait-qualified calls, generic function and impl bounds, supertrait requirements, trait-backed equality and ordering operators, user-defined compiler-shaped Eq/Hash/Debug/Ord-like fixtures, stdlib-like `cmp::Eq`, `cmp::Ord`, `hash::Hash`, `fmt::Debug`, and iterator-shaped fixtures | unknown traits, wrong trait arity, duplicate traits/methods/impls, missing/extra/mismatched methods, receiver mismatches, missing bound impls, ambiguous method and trait-qualified calls, missing supertrait impls, non-object-safe generic trait methods under dyn dispatch | runtime trait fixtures, LLVM greps for specialized static impl symbols, typed IR static-dispatch artifact, diagnostic artifacts for missing-bound and ambiguous-call failures, and capability inventory status | complete for current static subset |
| Structural capability parameters | Free-function, inherent `impl` method, trait method, and trait impl method `has method(...) -> Type` plus grouped `has { method(...) -> Type, other(...) -> Type }` parameter requirements lower to hidden generics; duplicate method names in one capability set are rejected; field/property-style requirements are rejected as method-only; explicit function/method/aggregate generic bounds such as `T: has method(...) -> Type` name the structural type; reusable capability aliases such as `type Serializable = has serialize() -> i64;` and generic aliases such as `type Mapper[Input, Output] = has map(Input) -> Output;` can be reused as supported function/method/struct/enum bounds; trait impl conformance compares structural method signatures exactly; executable call forms check the concrete call-site type and concrete aggregate type applications, including forwarded aggregate fields and generic impl bodies over forwarded fields, check selected type arguments for matching static methods before ordinary method dispatch or aggregate lowering | unsupported `has` positions, rejected `interface` spelling, duplicate capability method requirements, field/property requirement attempts, missing method, wrong non-receiver parameter type/count, wrong return type, trait/impl structural-bound mismatch, capability aliases used as storage types or unsupported generic contexts, alias arity mistakes, forwarded aggregate bounds with missing methods, accidental dynamic dispatch expectations, ambiguous structural method lookup, and diagnostics that hide the whole grouped or aliased contract | parser diagnostic fixture covers unsupported type-position `has`; positive trait fixtures cover single, grouped, explicit generic-bound, inherent method, associated-function dispatch, alias-bound dispatch, generic alias-bound dispatch, struct and enum aggregate bounds, forwarded aggregate bounds, String-returning capability methods, forwarded impl body usage, and trait method contracts; negative fixtures cover duplicate method requirements, field/property requirement attempts, missing method with full grouped-set notes, unsupported generic-bound positions, alias-as-type, alias arity, unsupported alias-bound positions, aggregate-bound missing methods, forwarded aggregate-bound missing methods, trait method structural-bound mismatch, return mismatch, and alias-expanded failure context; future language design can add requirement kinds beyond methods | complete for current method-only subset |
| Discriminant-linked union fields | `union by` fields whose active payload type follows a stable discriminant field; parser/AST preserve selector and arm types for tooling, sema validates earlier-field selector roots, nested struct-field selector segments, unique arm names, arm payload type refs, exact enum-case arm coverage, exact bool `false`/`true` arm coverage, targeted rejection of non-enum/non-bool selectors, enum/bool selector struct literals with natural `field: arm(payload)` syntax or compatibility `field: arm => payload`, struct-payload shorthand `field: arm { field: value }`, direct enum/bool selector fields and selector-only nested paths inferred from omitted selectors, direct field matches with subject-type case resolution, direct selector-path matches that narrow linked payload-slot projections inside each arm, wrapper selector-path matches that narrow nested payload slots, local alias matches that narrow alias payload slots, direct selector-path, selector-ancestor, payload-field assignment, unproven direct payload-slot projection rejection, local-alias payload-slot projection rejection, pattern-alias payload-slot projection rejection, local-alias reassignment rejection, pattern-alias reassignment rejection, direct owner-word arm payload move/drop, whole-value active owner payload cleanup, and live-owner return diagnostics | unstable selector expressions, unknown constructor arms, constructor calls with zero or multiple payload expressions, missing bool arms, bool arms other than `false`/`true`, constructor arms that disagree with statically visible selector values, dynamic selector expressions that cannot be checked locally, non-struct payload shorthand, conflicting omitted direct or nested selector inference, direct selector or payload mutation after construction, owner-bearing aggregate arm payloads, active-arm drop mistakes, and confusion with ordinary enum ADTs or unchecked C unions | syntax artifact plus targeted diagnostics cover missing selector roots, missing nested selector fields, duplicate arms, missing enum-case arms, unknown enum-case arms, unsupported selector types, unknown constructor arms, constructor payload-count mistakes, same-literal selector mismatches, dynamic selector rejection, bool arm coverage/name diagnostics, shorthand non-struct payload diagnostics, direct and nested selector inference conflicts, selector/payload-field assignment rejection, unproven direct payload-slot projection rejection, local/pattern alias projection rejection, local/pattern alias reassignment rejection, owner-bearing aggregate payload diagnostics, active owner payload undropped diagnostics, and live owner-carrying binding return diagnostics; `make check-structs` covers executable enum-selector construction, bool-selector construction, bool selector-match payload-slot projection, bool call-style construction, struct-payload shorthand construction, direct selector inference, nested selector inference, direct field match reading, enum selector-match payload-slot projection, wrapper selector-match payload-slot projection, local alias-match payload-slot projection, active owner payload cleanup, and selector/payload safety diagnostics; future work is language design beyond enum/bool selectors or public ABI promises | complete for current executable subset |
...
| Prelude IO/format/input/assert | `print`, `println`, `eprintln`, `std::print`, `std::println`, `std::eprintln`, `std`-alias qualified print names, `{}`, `{name}`, `{name.field}`, `{name.0}`, `{:?}`, `{name:?}`, `{:.N}`, and `{name:.N}` format placeholders, escaped braces, formatted string literals/source `String`/`char`/`u64`/bool/`f32`/`f64` output on LLVM, stderr formatting through `eprintln!`, LLVM static string-literal storage with raw-pointer byte reads and `write_byte`, source `io::write_bytes`/root `write_bytes`, source `io::Reader`/`Writer`/`Seek`, `ReadByte`, `Cursor`, `stdin`, `stdout`, `stderr`, Result-returning `read_one`, `read`, `read_exact`, `copy`, `write_all`, `flush`, compatibility `read_exact_unchecked`, `try_copy`, `copy_unchecked`, `write_all_unchecked`, `flush_unchecked`, `print!`, `println!`, `eprintln!`, `format_in!` explicit-zone source `String` construction for `{}` and named captures over local fields/tuple indexes with string literals/byte-character/signed and unsigned integer/bool/`f32`/`f64`/Display values, `{:?}` and `{name:?}` Debug values, and `{:.N}`/`{name:.N}` float precision with once-evaluated function-call/computed arguments plus named local capture, `matches!`, IO aliases, `write_i64`, `write_u64`, `read_byte`, source `input::try_read_byte`, host `read_line`/`input`, explicit-zone owned line helpers returning source `std::string::String`, root `String` / `std::String` aliases for the source string handle, source string append helpers for string literals, `char`, `i64`, `u64`, bool, `f32`, and `f64`, source `Option[T]` / `Result[T, E]` / `Slice[T]` and `slice(data, len)`, Option/Result borrowed view handles, borrowed and consuming payload predicates, `unwrap`/`expect`/`unwrap_or`/`unwrap_or_else`, Option `as_ref`/`as_mut`/`take`/`replace`/`map`/`or`/`or_else`/`xor`/`and_then`/`filter`/`flatten`/`transpose`/`ok_or`/`ok_or_else`, and Result `as_ref`/`as_mut`/`ok`/`err`/`map`/`map_err`/`and_then`/`or`/`or_else`/`transpose`/`unwrap_err`/`expect_err` methods, Slice `len`/`is_empty`/indexing/indexed assignment/range slicing/read/mutable access/search/predicate search/affix stripping/comparison/copy-to-Vec methods, mutable local array/Vec `as_slice()` view creation, `std::mem::replace` / `std::mem::swap` plus root aliases for scalar/plain aggregate values, assertion function and macro helpers | bad placeholder count, nonliteral format, bad precision placeholder syntax/type, bad named-capture syntax, `format!` without an implicit allocation source, unsupported `format_in! f128`/non-Display/non-Debug values, line input, owned line/root String use after zone reset, mismatched string append/growth zone, bad assert type, bad assert macro type, bad `matches!` arity/patterns, ownership-valued `std::mem::replace`/`swap`, removed `Maybe[T]` alias | stdio, getchar/fgets, LLVM string constants, decimal integer, named capture lowering, byte-character display/debug formatting, debug string quoting, source String span formatting through stdio streams, float formatting, and stderr routing, source byte-slice output, source IO trait/cursor/buffered/stderr read-one/partial-read/exact-read/write-all helper monomorphization, source input EOF-to-Option helper, source string copying/appending for owned line and explicit text construction, statement-position and expression-arm stop helpers lowering as noreturn/bottom-like control flow, Option/Result predicate, borrowed-view, take/replace, unwrap/expect, conversion, borrowed-predicate filtering, flattening, bidirectional transposition, eager/lazy fallback, and combinator method lowering on source generic enums, `std::mem` value helper lowering through pointer materialization, assertion exits, pattern-engine macro lowering | partial |
...
| Standard hashing | `std::hash::Hasher`, `Hash[T]`, `new`, `reset`, `finish`, generic `write<T>`/`value<T>`, byte-slice `bytes`, primitive `write_byte`/`write_bytes`/`write_i64`/`write_u64`/`write_bool`, the `collections::hash_i64` compatibility wrapper, and trait-driven hash collection `with_capacity` constructors | future aggregate impls, seed/collision-policy tests, and encoding-module split tests | pure source helper monomorphization today; collection defaults use `std::hash::value<T>` while custom hash policy remains explicit | partial |
| Standard random | Result-returning `std::random::entropy` and direct OS byte `fill`, strict `entropy_unchecked`/`fill_unchecked`, raw `entropy_raw`/`fill_raw` compatibility forms, deterministic non-cryptographic `Prng`, `seed`, Result-returning `from_entropy`/`seed_from_os`, strict `from_entropy_unchecked`/`seed_from_os_unchecked`, raw `next`, `boolean`, unbiased bounded integer `below`/`range`, unit `float`, PRNG byte `fill_from`, and generic `shuffle` over `Slice[T]` views | future larger byte-fill tests, statistical bounded integer tests, CSPRNG stream policy, and distribution helpers if they belong in core std | `entropy` and `fill` are hosted runtime hooks using Linux `getrandom` with `/dev/urandom` fallback; natural helpers bridge hosted errno failures through `std::error`, while `_unchecked` helpers hard-fail on entropy errors; PRNG helpers, deterministic booleans, unbiased bounded integer sampling, and shuffle are source Ari | partial |
| Standard parsing | `std::parse::Parse`, `parse<T>`, `parse_or<T>`, `is_parse<T>`, Result-returning signed `integer`/`integer_radix`, unsigned `unsigned`/`unsigned_radix`, integer and float `ParseError` diagnostics with offsets, names, messages, and `is_*` predicates, hex/binary/octal signed wrappers, `boolean`, `_optional` compatibility forms, `is_float`, Result-returning `float`, `float_optional`, `float_or`, and `float_unchecked` over ASCII-trimmed `Slice[u8]` values | future exact IEEE boundary rounding and locale-rejection tests | pure source helper monomorphization today; generic calls now accept string-literal-to-slice coercion, natural integer/bool/float parser names return `std::error::Error` categories, `*_error` helpers provide syntax/separator/range offsets and stable human-readable labels, and enum payload lowering supports `f64` inside `Option`/`Result` | partial |
| Standard encoding | `std::encoding::is_ascii`, detailed Result-returning `validate_utf8`, `decode_utf8`, `utf8_count`, `utf16_count`, `encode_utf8`, `encode_utf8_in`, UTF-8 diagnostic `name`/`message`, natural hex/base64 decoder helpers plus `_in` aliases, structured `CodecError` diagnostics with `name`/`message` for hex, standard base64, MIME base64, and URL-safe base64, hex/base64 decoded length helpers, standard, MIME line-wrapped, and URL-safe base64 encode/decode helpers, `_optional` compatibility validation/count/decode helpers, `_unchecked` asserting scalar/codec helpers, `is_utf8`, `utf8_width`, `utf8_at`, `utf8_next_index`, `utf8_encoded_len`, `is_utf16`, and hex/base64 encode guards | future normalization/transcoding, codec-specific rich Result payload migration if chosen, and optional compression tests | pure source helper monomorphization today; natural UTF-8 failure APIs return detailed `Utf8Error`, byte-codec fallible APIs return shared `std::error::Error` categories, and `*_error` helpers provide precise codec diagnosis when callers need it | partial |
| Standard paths | `std::path::is_separator`, `is_absolute`, `is_relative`, explicit Windows `is_windows_*`/`windows_*` drive and UNC classifiers, `trim_trailing_separators`, `components`, `file_name`, `parent`, `extension`, `stem`, component-aware `starts_with`/`ends_with` and `strip_prefix`/`strip_suffix`, `join_in`, `join`/`join_many`, owned `PathBuf`, and `normalize_in` over byte paths | future platform-specific path storage, verbatim Windows path policy, and richer component kinds | pure source helper monomorphization today; filesystem-backed existing-path canonicalization is covered by `std::fs` | partial |
| Context, environment, process, thread, sync, and filesystem runtime | `context::argc`, `context::arg`, `context::thread_id`, startup `context::cwd`, startup `context::executable_path`, source `context::has_args`, `context::has_arg`, `context::user_arg_count`, `context::has_user_args`, `context::is_main_thread`, startup cwd/executable optional and OS/path view helpers, user-facing `env::arg_count`, `env::arg`, `env::has_arg`, `env::try_arg`, `env::program_name`, Result-returning `env::get`, `env::has`, `env::try_get`, `env::get_or_default`, Result-returning `env::set`/`env::remove`, `env::set_unchecked`, `env::remove_unchecked`, Result-returning `env::current_dir`, `env::current_dir_optional`, `env::set_current_dir`, `env::executable_path`, `env::executable_path_optional`, `process::id`, `process::uid`, `process::gid`, `process::exit`, `process::abort`, `process::fork`, `process::wait_status`, `process::wait`, raw `process::fork_raw`, raw `process::wait_raw`, source process command builders, module-level `process::spawn`, `process::status`, `process::exit_status`, `process::output_in`, `process::exec`, source process status/root/branch/error helpers, `thread::spawn`, `thread::join`, `thread::yield_now`, `thread::id`, `thread::is_main`, `thread::is_join_error`, `thread::builder`, `thread::is_finished`, `thread::thread_local`, `thread::thread_local_with_capacity`, `Thread` method wrappers, `Builder`, `ThreadLocal`, `sync::load`, `sync::store`, `sync::swap`, `sync::fetch_add`, `sync::compare_exchange`, `sync::try_lock`, `sync::lock`, `sync::unlock`, `sync::call_once`, `Ordering`, `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr`, `Mutex`, `RwLock`, `Once`, `OnceLock`, `Condvar`, `Barrier`, `Channel`, `Sender`, and `Receiver` method wrappers, `fs::exists`, `fs::can_read`, `fs::can_write`, `fs::can_execute`, `fs::permissions`, `fs::metadata`, `fs::try_metadata`, `fs::symlink_metadata`, `fs::try_symlink_metadata`, `fs::mode`, `fs::try_mode`, `fs::set_mode`, `fs::set_permissions`, `fs::canonicalize`, `fs::try_canonicalize`, `Metadata`, `Metadata::accessed`, `Metadata::modified`, `Metadata::changed`, `Metadata::owner`, `Metadata::group`, `Metadata::created`, `Metadata::created_or_changed`, `FileKind`, `fs::remove`, `fs::remove_dir_all`, `fs::rename`, `fs::hard_link`, `fs::symbolic_link`, `fs::read_link`, `fs::try_read_link`, `fs::create_dir`, `fs::remove_dir`, `fs::open_dir`, `fs::try_open_dir`, `fs::read_dir`, `fs::try_read_dir`, `fs::read_dir_entries`, `fs::try_read_dir_entries`, `fs::read_dir_info`, `fs::try_read_dir_info`, `fs::metadata_detailed`, `fs::rename_detailed`, `PathError`, `TwoPathError`, `Operation`, `DirEntryInfo`, `fs::read_dir_next`, `fs::close_dir`, `Dir`, `DirEntry`, `DirEntry::metadata`, `DirEntry::try_metadata`, `DirEntry::symlink_metadata`, `DirEntry::try_symlink_metadata`, `DirEntry::try_file_type`, `DirEntry::is_file`, `DirEntry::is_dir`, `DirEntry::is_symlink`, `DirEntry::is_other`, mode-string `fs::open`/`fs::open_optional`/`fs::try_open`/`fs::open_unchecked` with `"r"`, `"w"`, `"a"`, `"rw"`, `"r+"`, `"w+"`, and `"a+"`, `OpenOptions`, `fs::open_options`, `OpenOptions::read`, `OpenOptions::write`, `OpenOptions::append`, `OpenOptions::truncate`, `OpenOptions::create`, `OpenOptions::create_new`, `OpenOptions::open`, `OpenOptions::open_optional`, `OpenOptions::open_unchecked`, `OpenOptions::try_open`, `fs::create`, `fs::create_optional`, `fs::create_unchecked`, `fs::try_create`, compatibility `fs::open_*`/`fs::try_open_*` wrappers, `File` byte read/write/position/seek/close helpers, source `fs::read`/`fs::try_read`/`fs::write`/`fs::try_write`/`fs::append`/`fs::try_append`/`fs::truncate`/`fs::copy`/`fs::try_copy`/`fs::read_to_string`/`fs::try_read_to_string`, aliases, `@ari_entry`/`ari::main` host wrapper | shared-library context use, process interactive stdin/stdout/stderr handles, signal mask/action handling, memory mapping, future path normalization, future richer thread statuses, futex-backed blocking policy, true OS condvars/channels, compiler-level thread-local declaration sugar, future owned filesystem resource policy, future ACL policy, exact mid-read filesystem errors, distinct owned resource policy, and platform symlink policy | host runtime init, thread-local `@ari_thread_id`, `getenv`, `setenv`, `unsetenv`, `getcwd`, `chdir`, `readlink`, `getpid`, `getuid`, `getgid`, `exit`, `abort`, `fork`, `waitpid`, `pthread_create`, `pthread_join`, `pthread_kill`, `sched_yield`, LLVM atomic load/store/RMW/CAS, source thread-local/mutex/once/atomic/condvar/barrier/channel helpers, standard thread/sync zone-pointer provenance helpers, `access`, `stat`, `lstat`, `chmod`, `realpath`, `readlink`, `unlink`, `rename`, `link`, `symlink`, `mkdir`, `rmdir`, `opendir`, `readdir`, `closedir`, `open`, `read`, `write`, `lseek`, `close`, plus source helper monomorphization, stack-local byte-path adapters for `DirEntry` metadata methods, metadata timestamps, metadata owner/group ids, detailed filesystem errors, per-entry directory metadata snapshots, recursive tree removal, direct `OpenOptions` flag lowering, process command wrappers, readiness-drained process output, descriptor poll helpers, and compact `std::error` bridges for open/read failures | partial |
| Environment OS/path views | `env::arg_os`, `env::try_arg_os`, `env::program_name_os`, Result-returning `env::get_os`, `env::try_get_os`, `env::get_os_or_default`, Result-returning `env::current_dir_os`, `env::current_dir_path`, and `env::executable_path_os`, plus `_optional` and `try_*` compatibility path views | future owned `OsString`/`PathBuf`, Windows OS-string semantics, and canonicalization/error policy | source wrappers over existing runtime C-string hooks plus `std::string::OsStr` and `std::path::PathBytes` boundary helpers | partial |
...
`std-process-output.ari` covers readiness-drained `Command::output_in`
stdout/stderr capture, `Output` accessors, missing-command status behavior,
`pipe`/`dup2` lowering, and compiler zone provenance for the zone-backed
`Output` handle.
...
interactive stdin/stdout/stderr handles, richer platform status fields, and
non-POSIX mapping.
...
- [x] `std::string::from(ref mut Zone, Slice[u8])` copies borrowed bytes
      or string literals into independent source `std::string::String` storage
...
- [x] `std::string::from_slice_in(ref mut Zone, Slice[u8])` copies a borrowed
      ...
      the target zone
- [x] `std::string::from(ref mut Zone, Slice[u8])`, `copy(ref mut Zone, Slice[u8])`,
...
- [x] byte character literals such as `'c'`, `'\n'`, and `'\x63'` lower as
      `u8`, and `std::string::bytes("literal")` exposes borrowed literal bytes as
...
- [x] source `std::parse` exposes ASCII-trimmed whole-input `integer`,
      ...
      helpers with natural Result names, `_optional` compatibility forms, and
      focused invalid-input coverage
- [x] source `std::parse` exposes integer and float `ParseError` diagnostics
      with `ParseErrorKind`, trimmed-input byte offsets, names, messages,
      `is_*` predicates, and decimal float
      overflow/underflow range reporting
- [x] source `std::encoding` exposes ASCII, detailed UTF-8, and UTF-16
      validation plus UTF-8 byte-string decoding, Result-returning hex/base64
      length/decode helpers, `_optional` compatibility helpers, `_unchecked`
      asserting helpers, and invalid-input guard helpers
- [x] source `std::path` exposes POSIX-style lexical path separator,
      component, component-iterator, component-aware prefix/suffix, join,
      owned `PathBuf`, lightweight normalization helpers, and explicit Windows
      drive/UNC byte classifiers
...
- [x] source `std::allocator::Allocator` provides the user-facing allocation
      capability view over zone-backed storage through `allocator::from_zone`,
      `allocator::from_data`, `allocator::of`, typed allocation, counters, and
      preflight helpers
...
- [x] add source IO contracts and cursor helpers: `io::Reader`,
      ...
      Result-returning `io::read_exact`, `io::copy`, `io::write_all`, and
      `io::flush`, plus compatibility `io::read_exact_unchecked`,
      `io::copy_unchecked`, `io::write_all_unchecked`, and
      `io::flush_unchecked`
...
- [x] source `std::fmt::Debug` has built-in impl coverage for `i64`, `u64`,
      `char`, `bool`, `f32`, `f64`, string literals, and owned
...
- [x] expose root `String` as the source `std::string::String` handle while
      keeping string literals as unnameable static bytes that coerce into
      `String`, `Slice[u8]`, typed text views, and `ptr c_char`
