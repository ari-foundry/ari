...
- [x] lower refutable scalar literal/range, or-pattern, fieldless enum-case,
      ...
      nested aggregate-enum multi-payload item patterns for `Iterator[T]`
      loops with stop-on-first-mismatch semantics
...
- [x] lower iterator item literal/range/or, fieldless enum-case, compact enum
      ...
      aggregate-enum multi-payload patterns through the
      `while let Some(pattern) = next()` path
