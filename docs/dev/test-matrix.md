...
| Standard encoding | `std::encoding::is_ascii`, Result-returning `validate_utf8`, `utf8_count`, `utf16_count`, `encode_utf8_in`, hex/base64 decoded length and decoder helpers, `_optional` compatibility validation/count/decode helpers, `_unchecked` asserting scalar/codec helpers, `is_utf8`, `utf8_width`, `utf8_at`, `utf8_next_index`, `utf8_encoded_len`, `is_utf16`, and hex/base64 encode guards | future URL-safe base64, MIME line wrapping, richer decode error values, normalization/transcoding, and optional compression tests | pure source helper monomorphization today; natural fallible names return `std::error::Error`, while optional codec helpers use compiler-tracked zone-backed enum payloads | partial |
...
- [x] source `std::encoding` exposes ASCII, UTF-8, and UTF-16 validation plus
      Result-returning hex/base64 length/decode helpers, `_optional`
      compatibility helpers, `_unchecked` asserting helpers, and
      invalid-input guard helpers
