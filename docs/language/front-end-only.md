...
it starts with a specific token-text prefix, or whether a fixed token position
has a specific text spelling. The condition can use bool literals, `!`, `&&`,
`||`, `tokens_empty(input)`, `input.is_empty()`, integer comparisons over
`tokens_count(input)` or `input.len()`, prefix checks with
`tokens_starts_with(input, "...", ...)` or `input.starts_with("...", ...)`,
and indexed checks with `tokens_nth_is(input, index, "...")` or
`input.nth_is(index, "...")`.
Each branch must return token output:
...
bodies inside returned control-flow expressions, structured token extraction
beyond empty/count/prefix/indexed-text branching, and attribute rewrites remain
reserved until Ari has a broader compile-time evaluator. Item-position `ast -> ast`
macros can return
