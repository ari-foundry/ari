...
the invocation token payload is empty. The condition can use bool literals,
`!`, `&&`, `||`, `tokens_empty(input)`, or `input.is_empty()`, and each branch
must return token output:
...
bodies inside returned control-flow expressions, richer token inspection beyond
empty-input branching, and attribute rewrites remain reserved until Ari has a
broader compile-time evaluator. Item-position `ast -> ast`
macros can return
...
is parsed in the site domain after raw token substitution. General token
matching, counting, and content inspection are still reserved.
