...
it starts with a specific token-text prefix. The condition can use bool
literals, `!`, `&&`, `||`, `tokens_empty(input)`, `input.is_empty()`, integer
comparisons over `tokens_count(input)` or `input.len()`, and prefix checks with
...
empty/count/prefix branching, and attribute rewrites remain reserved until
