...
additional derives are still planned. Outside a `zone { ... }` current-zone
block, `format!` still has no implicit allocation target; use executable
`format_in!` for explicit-zone owned string formatting in that case.
