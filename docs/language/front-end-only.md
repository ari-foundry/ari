...
by one delimiter pair. The condition can use bool literals, `!`, `&&`, `||`,
`tokens_empty(input)`, `input.is_empty()`, integer comparisons over
`tokens_count(input)` or `input.len()`, boundary checks with
...
beyond empty/count/boundary/wrapper/indexed-text branching, and attribute
rewrites remain reserved until Ari has a broader compile-time evaluator. Item-position `ast -> ast`
macros can return
