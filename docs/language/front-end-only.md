...
the first token has a specific text spelling. The condition can use bool
literals, `!`, `&&`, `||`, `tokens_empty(input)`, `input.is_empty()`, integer
comparisons over `tokens_count(input)` or `input.len()`, and first-token checks
with `tokens_starts_with(input, "...")` or `input.starts_with("...")`. Each
branch must return token output:
...
empty/count/first-token branching, and attribute rewrites remain reserved until
