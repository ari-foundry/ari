...
position has a specific text spelling. The condition can use bool literals,
`!`, `&&`, `||`, `tokens_empty(input)`, `input.is_empty()`, integer comparisons
over `tokens_count(input)` or `input.len()`, boundary checks with
`tokens_starts_with(input, "...", ...)`, `input.starts_with("...", ...)`,
`tokens_ends_with(input, "...", ...)`, or `input.ends_with("...", ...)`, and
indexed checks with `tokens_nth_is(input, index, "...")` or
`input.nth_is(index, "...")`.
...
beyond empty/count/boundary/indexed-text branching, and attribute rewrites
remain reserved until Ari has a broader compile-time evaluator. Item-position `ast -> ast`
macros can return
