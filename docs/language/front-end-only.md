...
bodies inside returned control-flow expressions, non-identity token
construction, and attribute rewrites remain reserved until Ari has a broader
compile-time evaluator. Item-position `ast -> ast`
macros can return
