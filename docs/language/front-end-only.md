...
the invocation token payload is empty or how many tokens it contains. The
condition can use bool literals, `!`, `&&`, `||`, `tokens_empty(input)`,
`input.is_empty()`, and integer comparisons over `tokens_count(input)` or
`input.len()`. Each branch must return token output:
...
empty/count branching, and attribute rewrites remain reserved until Ari has a
