...
Each branch must return token output, the input stream, or a slice:
...
bodies inside returned control-flow expressions, named token captures beyond
empty/count/boundary/wrapper/indexed-text/slice/pattern branching, and
attribute rewrites remain reserved until Ari has a broader compile-time evaluator. Item-position `ast -> ast`
macros can return
