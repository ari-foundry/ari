...
`tokens_nth_is(input, index, "...")` or `input.nth_is(index, "...")`. Exact
token patterns use `tokens_match(input, "...", "_", ...)` or
`input.matches("...", "_", ...)`, where `"_"` matches one token. Pattern
strings beginning with `$`, such as `"$value"`, bind a named single-token
capture. Branches can return that capture with
...
bodies inside returned control-flow expressions, multi-token capture spans
beyond empty/count/boundary/wrapper/indexed-text/slice/pattern/single-capture
branching, and attribute rewrites remain reserved until Ari has a broader compile-time evaluator. Item-position `ast -> ast`
macros can return
