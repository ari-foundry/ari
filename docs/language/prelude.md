...
`as_mut_ptr()` methods for copyable, zone-placeable values. `set(value)` drops
...
instead of copying it; `as_ref()` returns a shared borrow of the stored value
and `as_mut()` returns a mutable borrow of that value. `as_mut_ptr()` and the
mutating methods borrow the handle mutably.
...
checked byte `first`/`last`/`get`, byte search, exact/prefix/suffix checks,
...
`*_text` wrappers keep the borrowed builtin-string compatibility view.
...
provenance, and `as_mut_ptr()` returns the same raw element pointer through a
mutable receiver; both are rejected after that zone is reset or destroyed.
`iter()`
...
same view ABI for ordinary non-return parameter slots, and `impl Vec[T]` /
`impl Trait for Vec[T]` receivers lower as borrowed views too. Generic
functions whose source parameter is `Vec[T]` use that same view ABI, while
by-value generic `T` parameters still carry concrete local Vec capacity when
`T` resolves to local Vec storage. Bare root `Vec[T]` returns, extern
parameters/returns, trait method return types, and struct fields still reject it
with a dedicated local/view-only diagnostic. Use `std::Vec<T>` /
...
let view = boxed.as_ref()
let unique = boxed.as_mut()
...
`copy_to(ref mut Zone)`, `as_ref()`, and `as_ptr()` are read-only borrows of
the handle; `as_mut()`, `as_mut_ptr()`, `set`, `replace`, and `swap` take
mutable borrows. `take()`
...
matches. `equals(view)`, `starts_with(view)`, and `ends_with(view)` compare the
string bytes against a borrowed `Slice[u8]` view.
...
`or[F](Result[T, F])` / `or_else[F](fn(E) -> Result[T, F])`, plus
`transpose()` on `Result[Option[T], E]`. Bind
