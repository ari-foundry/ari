...
integer constants, and immutable local integer capacities initialized from
literals can raise that local capacity, while runtime `reserve(n)` checks that
the requested capacity already fits the fixed local storage. A typed `Vec[T]`
