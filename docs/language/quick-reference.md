...
| unique linear set | `collections::new<T>(ref mut zone, capacity)`, `Set::new<T>(ref mut zone, capacity)`, `.insert(ref mut zone, value)`, `.try_get(index)`, `.try_pop()`, `.reserve(ref mut zone, capacity)`, `.iter()`, `.contains(value)` |
