...
| environment/path state | `env::get(name)`, `env::try_get(name)`, `env::get_os(name)`, `env::try_get_os(name)`, `env::set(name, value)`, `env::current_dir_path()`, `env::set_current_dir(path)`, `env::executable_path_os()` |
...
| thread | `thread::spawn(entry)`, `thread::builder().name("worker").spawn(entry)`, `thread::join(handle)`, `handle.join()`, `handle.is_finished()`, `thread::id()`, `thread::yield_now()`, `thread::sleep(duration)`, `thread::available_parallelism()`, `thread::thread_local<T>(ref mut zone)` |
...
| locks/channels | `Mutex::new()`, `.lock()`, `.try_lock()`, `guard.unlock()`, `.lock_raw()`, `RwLock::new()`, `.read()`, `.try_read()`, `.write()`, `.try_write()`, `rw_guard.unlock()`, `.read_lock()`, `.read_unlock()`, `.write_lock()`, `.write_unlock()`, `Once::new()`, `.call_once(init)`, `OnceLock::new<T>()`, `Condvar::new()`, `Barrier::new(parties)`, `sync::channel<T>(ref mut zone)` |
