...
| environment/path state | `env::get(name)`, `env::try_get(name)`, `env::get_os(name)`, `env::try_get_os(name)`, `env::set(name, value)`, `env::try_current_dir_path()`, `env::set_current_dir(path)`, `env::try_executable_path_os()` |
