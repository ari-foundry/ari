...
| process command | `process::arg(value)`, `process::env_var(name, value)`, `process::command(program)`, `process::command_with_args(program, args)`, `cmd.spawn()`, `cmd.status()`, `cmd.output_in(ref mut zone)`, `cmd.exec()`, `child.wait()`, `child.kill(signal)`, `output.stdout()`, `output.stderr()`, `process::kill(pid, signal)` |
