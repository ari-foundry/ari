...
| process command | `process::arg(value)`, `process::env_var(name, value)`, `process::command(program)`, `process::command_with_args(program, args)`, `cmd.spawn()`, `cmd.status()`, `cmd.exit_status()`, `cmd.output_in(ref mut zone)`, `cmd.exec()`, `child.wait()`, `child.wait_status()`, `child.kill(signal)`, `status.code()`, `status.signal()`, `output.stdout()`, `output.stderr()`, `process::kill(pid, signal)` |
