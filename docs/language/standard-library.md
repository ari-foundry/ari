...
| `std::fs` | Byte-oriented filesystem handles. | `File`, `OpenOptions`, `FileKind`, `Metadata`, `Permissions`, `exists`, `can_read`, `can_write`, `can_execute`, `permissions`, `metadata`, `metadata`, `try_metadata`, `symlink_metadata`, `symlink_metadata`, `try_symlink_metadata`, `file_type`, `try_file_type`, `is_file`, `is_dir`, `is_symlink`, `is_other`, metadata access/modification/status-change methods, `mode`, `mode`, `try_mode`, `set_mode`, `set_permissions`, `canonicalize`, `canonicalize`, `try_canonicalize`, `remove`, `remove`, `remove_raw_result`, `rename`, `rename`, `rename_raw_result`, `hard_link`, `symbolic_link`, `read_link`, `read_link`, `try_read_link`, `create_dir`, `create_dir`, `create_dir_raw_result`, `ensure_dir`, `create_dir_all`, `ensure_dir_all`, `remove_dir`, `remove_dir`, `remove_dir_raw_result`, `remove_dir_all`, `open_dir`, `read_dir`, `read_dir_entries`, `open`, `open_optional`, `try_open`, `open_unchecked`, `open`, `open_raw_result`, `open_options`, `OpenOptions::open`, `OpenOptions::open_optional`, `OpenOptions::open_unchecked`, `OpenOptions::open`, `OpenOptions::open_raw_result`, `create`, `create_optional`, `try_create`, `create_unchecked`, `create`, `create_raw_result`, compatibility `open_*`/`try_open_*`, `read_byte`, `write_byte`, `write_bytes`, `position`, `seek`, whole-file `read`, `read`, `try_read`, `write`, `write`, `write_raw_result`, `try_write`, `append`, `append`, `append_raw_result`, `try_append`, `truncate`, `copy`, `copy`, `copy_raw_result`, `try_copy`, `read_to_string`, `read_to_string`, `try_read_to_string`, temp file/dir helpers, advisory file locks, `File` as `std::io::Reader`/`Writer`/`Seek`, `descriptor`, `close`. | First runtime-backed file slice over host file descriptor/stat/lstat/chmod/realpath/readlink/mkdir/lseek/flock hooks plus natural `Result[..., Error]` open/create/read/query/mutation/write/append/copy results, `Option[File]` compatibility helpers, raw compatibility result bridges, `OpenOptions`, `Option[String]`, `Option[i64]` byte-count helpers, target-following and no-follow metadata, direct path-kind predicates, access/modification/status-change timestamps, `Permissions`, canonical path copying, symbolic-link target reads, whole-file byte helpers, generic IO file adapters, cursor positioning, directory creation and removal, hosted temporary paths, advisory locking, and non-owning `std::os::Fd` descriptor views. Owned path values, portable creation/birth time, richer owner/group/ACLs, Windows locking policy, and structured filesystem errors remain roadmap work. |
