...
| `std::source` | Source-coordinate values and borrowed text lookup for tools and diagnostics. | `FileId`, half-open byte `Span`, one-based `LineCol`, `Location`, `SourceFile`, `file_id`, `file`, `span`, `empty_span`, `line_col`, `location`, `full_span`, `line_count`, `line_start`, `line_end`, `line_span`, `locate`, `contains`, `touches`, `same_file`, `merge`, and method wrappers. | Source-only value and borrowed text layer. Owned filename/text maps, cached line-start tables, diagnostic labels, and stable rendering are future work. |
...
| Represent source positions in tools. | `let id = source::file_id(1); let input = source::file(id, text); let span = input.line_span(2); let loc = input.locate(span.start())` | Spans are half-open byte ranges. `touches(offset)` is inclusive for cursor/caret logic. Line and column values are one-based. `SourceFile` borrows text and computes line/column data without allocation; owned source maps and diagnostic labels are the next bootstrap layer. |
