...
| `std::diag` | Lightweight diagnostic values for libraries and small tools. | `Severity`, `Diagnostic`, `error`, `warning`, `note`, `help`, `with_note`, `write`, and method wrappers. | Source-only value layer over borrowed byte code/message text. One borrowed note and stable summary output are current; source spans, labels, fix-its, owned messages, and stable source renderers belong in future compiler/tooling packages. |
...
| Build a small diagnostic summary. | `let diagnostic = diag::error("E0001", "expected name").with_note("identifiers start with a letter"); diagnostic.write()` | Use `std::diag` for shared severity/code/message/note values. `write()` emits a simple stderr summary and one optional note line today. Source spans, labels, multi-file renderers, multiple notes, and fix-its belong in future compiler/tooling packages. |
...
| Testing And Diagnostics | Library work needs source-level tests and stable failure reporting. | current `std::test::Report`, `std::log` line helpers, and lightweight `std::diag` summaries; future runner discovery, richer panic/assert messages, compiler-tooling source renderers, structured logging, stack/backtrace, optional benchmarks, and optional fuzz hooks. |
