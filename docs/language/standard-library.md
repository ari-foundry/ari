...
| `std::source` | Source-coordinate values for tools and diagnostics. | `FileId`, half-open byte `Span`, one-based `LineCol`, `Location`, `file_id`, `span`, `empty_span`, `line_col`, `location`, `contains`, `touches`, `same_file`, `merge`, and method wrappers. | Source-only value layer. Filename/text storage, line-start tables, span-to-line/column conversion, and diagnostic labels are future work. |
...
| Represent source positions in tools. | `source::file_id(id)`, `source::span(file, start, end)`, `span.contains(offset)`, `span.merge(ref other)`, `source::line_col(line, column)`, `source::location(file, line, column)` | Spans are half-open byte ranges. `touches(offset)` is inclusive for cursor/caret logic. Line and column values are one-based. Source maps that convert spans to line/column output are the next bootstrap layer. |
