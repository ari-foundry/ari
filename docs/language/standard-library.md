...
| `std::c` | C ABI boundary helpers. | `CStr`, `CString`, `Library`, `Symbol`, C string constructors, POSIX `errno`/`error`, null checks, and `dlopen`/`dlsym` dynamic loading wrappers. | Source helpers over compiler-known C ABI aliases and hosted loader symbols. Typed symbol casts and explicit owned-buffer FFI escape policy are future work. |
...
| `std::string` | Zone-backed owned byte string seed plus typed borrowed text-boundary views. | `String`, `RawString`, `Utf8`, `OsStr`, `CStr`, `utf8`, `os_str`, `c_str`, `c_len`, `c_bytes`, capacity constructors, `join_in`, copy helpers, byte get/set/search, byte-slice `find`, borrowed split/chunk/window views, `try_get`, `try_pop`, growth, append helpers, UTF-8 validation/scalar helpers, ASCII case compare/search, trim views, trim copies, whole and prefix parse helpers, `as_slice`, `as_ptr`. | Implemented as a byte string with UTF-8 scalar helpers and boundary views. Owned `Utf8String`/`OsString`, normalization, grapheme clusters, and locale policy remain future work. C ABI owned strings live in `std::c::CString`. |
...
| Share code with C. | `extern "C"`, C ABI aliases such as `c_int`/`c_char`/`c_void`, `std::c::CStr`, `std::c::CString`, `@repr(C)`, `@export`, `--shared`, `--emit-c-header` | Use `c::from_string("name").as_ptr()` for borrowed C strings, `c::from_slice_in(ref mut zone, bytes)` for owned NUL-terminated storage, and `c::errno()`/`c::error()` for sentinel-plus-errno C APIs. Do not pass Ari ownership across C directly. Zone-backed `CString` direct handoff to arbitrary externs is still behind an explicit FFI escape policy. |
...
| C Interop | Ari should call C libraries without making the standard library depend on a C++ ABI. | `extern "C"` declarations, `std::c` C string/errno/dynamic-loader helpers, and future typed symbol wrappers plus platform-specific low-level OS modules. |
