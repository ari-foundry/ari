...
| Store ordered membership. | `TreeSet::with_capacity<T: std::cmp::Ord[T]>(ref mut zone, capacity)`, `TreeSet::new<T>(ref mut zone, capacity, less)`, or `collections::tree_set<T>(...)` | Uses a red-black tree. `with_capacity` uses `<` through `Ord`. `insert` rejects equal values, `replace` returns the previous equal value, and `iter`/direct `for value in set` walk ascending comparator order. |
