...
| `std::net` | Network address values, DNS lookup, and socket handles. | `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, `TcpListener`, `TcpStream`, `UdpSocket`, `UnixListener`, `UnixStream`, `Shutdown`, address constructors/accessors, family, loopback, unspecified, port helpers, IPv4 DNS lookup, IPv4 TCP bind/connect/accept/local-port/local-address/peer-address helpers, IPv4 UDP bind/local-address/send-byte/receive-byte helpers, Unix stream bind/connect/accept helpers, descriptor views, nonblocking flags, `std::time::Duration` timeouts with raw millisecond compatibility helpers, stream shutdown, explicit close, TCP/Unix `std::io` byte adapters, and TCP/Unix `read_exact`/`write_all` stream buffer helpers. | Hosted IPv4 TCP/UDP/DNS and Unix stream slices plus source address values. IPv6 socket handles, buffer datagrams, UDP source address helpers, broad socket options, timeout-specific errors, and richer error payloads are roadmap work. |
...
| Write generic byte-output helpers. | `io::Writer`, `io::BufWriter`, `io::write_all[W: io::Writer](writer: ref mut W, bytes)`, `io::flush[W: io::Writer](writer: ref mut W)` | `io::Stdout`, `io::Stderr`, `io::PipeWriter`, buffered writers, and `std::fs::File` implement `Writer` now. `BufWriter` uses a caller-provided `Slice[u8]` buffer and flushes explicitly. File close ownership is still manual. |
...
`stream.read_exact(output, len)` for the usual method-call style. IPv6 socket
handles, buffer datagrams, UDP source addresses, richer options, and direct `Result[..., Error]`
payloads still need owned OS-resource and error policy.
