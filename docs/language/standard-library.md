...
| `std::collections` | Zone-backed collection handles beyond sequences. | `Set[T]`, `Iter[T]`, constructors, `from_slice_in`, metadata, insertion-order access, `try_*` access, `contains`, `index_of`, `insert`, `remove`, `take`, `pop`, `try_pop`, `reserve`, `clear`, `as_slice`, `iter`, `copy_to`. | Implemented as a linear insertion-order set seed with direct iteration; hash maps and hash sets are future work. |
...
| Store unique values in insertion order. | `collections::new<T>(ref mut zone, capacity)` or `Set::new<T>(ref mut zone, capacity)` | `insert(ref mut zone, value)` returns whether a value was newly added. Use `try_get`, `try_pop`, `contains`, `remove`, `take`, `reserve`, `iter`, `as_slice`, and `copy_to` for the current linear set. |
...
accessors, `index_of`, `as_slice`, and `iter()` preserve insertion order, and
the handle implements `IntoIterator[T]` for direct `for value in set` loops.
