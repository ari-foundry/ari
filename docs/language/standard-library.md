...
| Build a compiler diagnostic. | `let diagnostic = diag::error("E0001", "expected name", span).with_label(diag::primary(span, "name goes here")).with_note("identifiers start with a letter"); diagnostic.location(ref input); diagnostic.write()` | Use `std::diag` for shared severity/code/message/span values. `write()` emits a simple stderr summary and one optional note line today; stable source rendering with labels, multiple notes, and fix-its is roadmap work. |
