...
| `std::os` | Low-level OS value boundaries. | `Fd`, `fd`, `invalid`, `stdin`, `stdout`, `stderr`, descriptor predicates, raw accessor, equality, and `std::fs::File.descriptor()`. | Source non-owning descriptor view. Owned descriptors, close-on-exec/nonblocking mode, readiness APIs, raw syscalls, signals, and memory mapping are future work. |
