...
| `std::fmt` | Formatting traits and explicit formatting helpers. Root `Display`/`Debug` are aliases for these traits. | `Debug`, `Display::format_in`, `FormatSpec`, `decimal`, `hex`, `binary`, `octal`, width/precision/alignment modifiers, allocator-backed `*_in` helpers including `float_in`, `write_value`, `print_value`, `println_value`, and type-specific `write_*` helpers for `io::Writer`. | Formatting macros still use compiler lowering, while `FormatSpec` covers source hex/binary/octal, width, precision, and alignment for unsigned integers. |
