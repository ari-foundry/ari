...
| IO And Process Context | Programs need arguments, stdin/stdout, process status, and eventually files, full environment access, child processes, and threads. | `std::io`, `std::input`, `std::context`, `std::env`, `std::process`, future `std::fs`, `std::thread`, `std::sync`. |
