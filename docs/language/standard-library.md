...
| `std::source` | Source-coordinate values, borrowed text lookup, and cached line maps for tools and diagnostics. | `FileId`, half-open byte `Span`, one-based `LineCol`, `Location`, `SourceFile`, `LineMap`, `file_id`, `file`, `span`, `empty_span`, `line_col`, `location`, `full_span`, `line_count`, `line_start`, `line_end`, `line_span`, `locate`, `line_map`, `contains`, `touches`, `same_file`, `merge`, and method wrappers. | Source-only value and borrowed text layer. Owned filename/text maps, diagnostic labels, and stable rendering are future work. |
...
| Represent source positions in tools. | `let id = source::file_id(1); let input = source::file(id, text); let map = input.line_map(ref mut zone); let span = map.line_span(2); let loc = map.locate(span.start())` | Spans are half-open byte ranges. `touches(offset)` is inclusive for cursor/caret logic. Line and column values are one-based. `SourceFile` borrows text; `LineMap` caches line starts in an explicit `Zone` for repeated lexer/parser/diagnostic lookup. Owned source maps and diagnostic labels are the next bootstrap layer. |
