...
| Split or join lexical paths. | `path::file_name(bytes)`, `path::parent(bytes)`, `path::stem(bytes)`, `path::extension(bytes)`, `path::join_in(ref mut zone, base, child)`, `path::normalize_in(ref mut zone, bytes)` | These helpers do not query the filesystem. Borrowed component helpers return `Option[Slice[u8]]` views into the original bytes. Current separator policy is POSIX-style `/` only. |
