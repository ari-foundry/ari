...
creation and recursive removal are still roadmap work. `DirEntry` metadata is
available on collected entry values; path-level metadata helpers remain the
right choice when you already have a path string.
