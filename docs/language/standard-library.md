...
| Distinguish text and path boundary values. | `std::string::utf8(bytes)`, `std::string::utf8_string(ref mut zone, bytes)`, `std::string::os_str(bytes)`, `std::string::os_string(ref mut zone, bytes)`, `std::string::c_str(text)`, `std::path::bytes(bytes)`, `std::path::from_os(os)` | `String` is owned bytes, `Utf8` is validated borrowed UTF-8, `Utf8String` is owned validated UTF-8, `OsStr`/`OsString` are raw OS bytes, `std::c::CStr` wraps NUL-terminated C ABI text, and `PathBytes` means path policy. String literals coerce directly to expected `Utf8`, `OsStr`, `PathBytes`, and `CStr` boundary views. |
