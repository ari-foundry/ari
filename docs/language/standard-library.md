...
| --- | --- | --- | --- |
| `std` root | Common prelude surface and shared ADTs. | `Option[T]`, `Result[T, E]`, `Slice[T]` with `try_*` accessors, `Range[T]`, `RangeInclusive[T]`, `move`, `take`, assertion helpers, panic helpers, root `Box`, `String`, `Vec`, `AtomicI64`, `Mutex`, and `Once` aliases. | Implemented source surface with compiler-known hooks for selected helpers. |
...
| `std::sync` | Small explicit synchronization primitives. | `AtomicI64`, `Mutex`, `Once`, atomic `load`/`store`/`swap`/`fetch_add`/`compare_exchange`, `try_lock`, `lock`, `unlock`, `call_once`. | LLVM atomic slice plus source spin/yield `Mutex` and `Once`. Generic atomics, value-protecting locks, shared ownership, `RwLock`, `Condvar`, `OnceLock`, `LazyLock`, channels, and futex-backed blocking are future work. |
...
| Use a small explicit lock or one-time initializer. | `Mutex::new()`, `mutex.try_lock()`, `mutex.lock()`, `mutex.unlock()`, `Once::new()`, `once.call_once(init)` | Current `Mutex` is a source primitive lock without a protected payload or guard type. Current `Once` runs a plain `fn() -> void` at most once. `Mutex[T]`, `RwLock`, `Condvar`, `OnceLock`, `LazyLock`, channels, and futex-backed blocking are future work. |
