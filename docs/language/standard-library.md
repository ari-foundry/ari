...
| `std::env` | User-facing process argument and environment-variable helpers. | `arg_count`, `arg`, `has_arg`, `try_arg`, `program_name`, `get`, `has`, `try_get`, `set`, `remove`. | Argument wrappers over `std::context` plus runtime-backed current-process environment hooks. |
