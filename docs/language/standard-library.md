...
| `std::net` | Network address values, DNS lookup, and socket handles. | `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, `TcpListener`, `TcpStream`, `UdpSocket`, `UnixListener`, `UnixStream`, `Shutdown`, address constructors/accessors, family, loopback, unspecified, port helpers, IPv4 DNS lookup, IPv4 TCP bind/connect/accept/local-port helpers, IPv4 UDP bind/send-byte/receive-byte helpers, Unix stream bind/connect/accept helpers, descriptor views, nonblocking flags, millisecond timeouts, stream shutdown, explicit close, TCP/Unix `std::io` byte adapters, and TCP/Unix `read_exact`/`write_all` stream buffer helpers. | Hosted IPv4 TCP/UDP/DNS and Unix stream slices plus source address values. IPv6 socket handles, buffer datagrams, peer/local address helpers, broad socket options, and richer error payloads are roadmap work. |
...
| Represent network addresses or use sockets. | `net::Ipv4Addr::localhost()`, `net::Ipv6Addr::any()`, `addr.as_ip()`, `net::socket_addr(ip, port)`, `net::lookup_v4(host, port)`, `net::TcpListener::bind(addr)`, `net::TcpStream::connect(addr)`, `listener.accept()`, `listener.local_port()`, `stream.write_all(bytes)`, `stream.read_exact(ptr, len)`, `net::UdpSocket::bind(addr)`, `socket.send_byte_to(value, addr)`, `net::UnixListener::bind(path)`, `net::UnixStream::connect(path)`, `stream.shutdown(net::Write)` | Address values are source structs. TCP/UDP handles are hosted IPv4 descriptors; Unix stream sockets are hosted path-based descriptors. Socket creation may return `PermissionDenied` or `Unsupported` on restricted hosts. IPv6 handles, buffer datagrams, broad options, peer/local addresses, and richer errors are future runtime slices. |
...
hosted path-based stream sockets. TCP and Unix streams implement
...
handles, buffer datagrams, richer options, and direct `Result[..., Error]`
