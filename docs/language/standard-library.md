...
| `std::fs` | Byte-oriented filesystem handles. | `File`, `FileKind`, `Metadata`, `Permissions`, `exists`, `can_read`, `can_write`, `can_execute`, `permissions`, `metadata`, `try_metadata`, `try_file_type`, `is_file`, `is_dir`, `is_symlink`, `is_other`, `mode`, `try_mode`, `set_mode`, `set_permissions`, `canonicalize`, `try_canonicalize`, `remove`, `rename`, `hard_link`, `symbolic_link`, `create_dir`, `ensure_dir`, `create_dir_all`, `ensure_dir_all`, `remove_dir`, `open`, `try_open`, `create`, `try_create`, compatibility `open_*`/`try_open_*`, `read_byte`, `write_byte`, `write_bytes`, `position`, `seek`, whole-file `read`, `try_read`, `write`, `try_write`, `append`, `try_append`, `truncate`, `copy`, `try_copy`, `read_to_string`, `try_read_to_string`, `File` as `std::io::Reader`/`Writer`/`Seek`, `descriptor`, `close`. | First runtime-backed file slice over host file descriptor/stat/chmod/realpath/mkdir/lseek hooks plus source `Option[File]`, `Option[String]`, `Option[i64]` byte-count helpers, `Option[Metadata]`, direct path-kind predicates, `Permissions`, canonical path copying, whole-file byte helpers, generic IO file adapters, cursor positioning, directory creation, and non-owning `std::os::Fd` descriptor views. Directory iteration, owned path values, temp files, richer metadata timestamps/owner/group/ACLs, recursive directory removal, and optional locking remain roadmap work. |
...
| Create, remove, or read directories. | `fs::create_dir(path)`, `fs::ensure_dir(path)`, `fs::create_dir_all(path)`, `fs::ensure_dir_all(path)`, `fs::remove_dir(path)`, `fs::try_read_dir(ref mut zone, path)`, `fs::read_dir(ref mut zone, path)`, `fs::try_read_dir_entries(ref mut zone, path)`, `fs::read_dir_entries(ref mut zone, path)`, `fs::try_open_dir(path)`, `dir.next(ref mut zone)`, `dir.close()`, `fs::exists(path)` | `create_dir`/`ensure_dir` are single-directory helpers. `create_dir_all`/`ensure_dir_all` create missing parents and treat existing directories as success. `remove_dir` removes one empty directory only. `try_read_dir` is the convenient name-list helper; `try_read_dir_entries` returns lightweight entries with names and joined paths; `try_open_dir` plus `Dir::next` is the streaming shape. Directory reads skip `"."`/`".."`; metadata-bearing entries and recursive removal are future work. |
...
| Plan OS-facing code. | `std::target`, `std::env`, `std::process`, `std::thread`, `std::sync`, `std::time`, `std::fs`, `std::os`, `std::io`, `std::mem`, `std::random`, and source `std::net` values today. | Args, current-process environment variables, current directory/executable path, process id/uid/gid/exit/abort, POSIX fork/wait, function-pointer thread spawn/join/sleep/yield, available parallelism, concrete atomic i64 operations, source `Mutex`/`RwLock`/`Once`, monotonic/wall-clock reads, sleep, byte-oriented file handles, file cursor seek/position, descriptor views/owned descriptor close, duplication, pipe creation, pipe `Reader`/`Writer` adapters, close-on-exec, and nonblocking policy, access permissions, basic metadata, POSIX permission mode lookup/mutation, existing-path canonicalization, path rename, single-directory create/remove, directory-name reads, runtime page size, OS entropy through `getrandom` with `/dev/urandom` fallback, network IP/socket-address values, and target facts are implemented. Portable process spawn/exec/kill, richer thread statuses, user thread-local storage, custom thread stack sizes, shared ownership, generic synchronization, Condvar/OnceLock/LazyLock/channels/futex-backed blocking, richer directory-entry metadata, recursive directories, owned path values, temporary files, richer metadata timestamps/owner/group/ACLs/no-follow symlink metadata, network sockets/DNS/options, optional locking, raw syscall wrappers, broader fcntl/ioctl/poll/select, epoll/inotify/eventfd/timerfd/signalfd/pidfd/memfd/io_uring, signal mask/sigaction/alt-stack, mmap/munmap/mprotect/msync/mlock/madvise, fallible entropy, and build hardening profiles need runtime and ownership policy work. |
...
`fs::try_open(path, mode)` to obtain an `Option[File]` before calling them, or
`fs::try_create(path)` for the common create/truncate case. `File` also
...
unstatable paths return `false`. Use `fs::try_file_type(path)` or
`fs::try_metadata(path)` when absence should be explicit.
