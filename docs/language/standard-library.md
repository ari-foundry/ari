...
| Randomness | Tests, simulations, randomized algorithms, and shuffling need repeatable streams, while seeds and tokens need host entropy. | `std::random`, current OS entropy, deterministic non-crypto `Prng`, unbiased bounded integers, unit floats, byte filling, and shuffling. |
