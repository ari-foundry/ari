...
| `std::process` | Current-process helpers and POSIX child-process control. | `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, `is_success`, `is_failure`, `is_root`, `fork_result`, `wait_result`, raw compatibility `fork`, `wait`, `is_child`, `is_parent`, `is_fork_error`, `is_wait_error`, `Command`, `Child`, `Output`, `arg`, `env_var`, `command`, `command_with_args`, `kill`, `terminate`. | Runtime-backed id/uid/gid/exit/abort hooks plus POSIX fork/wait Result helpers and a command builder for `spawn`, `status`, `output_in`, `exec`, environment setup, working-directory setup, child kill/wait, and small stdout/stderr capture. Large-output draining, stdin redirection, richer statuses, and Windows process mapping are future work. |
...
| Run a child command on POSIX. | `process::arg(value)`, `process::env_var(name, value)`, `process::command(program)`, `process::command_with_args(program, args)`, `Command::spawn()`, `Command::status()`, `Command::output_in(ref mut zone)`, `Command::exec()`, `Child::pid()`, `Child::wait()`, `Child::kill(signal)`, `process::kill(pid, signal)`, `Output::stdout()`, `Output::stderr()` | Use `process::arg` for argv slices and `process::env_var` for child environment setup. `status` spawns and waits; `spawn` returns a handle; `output_in` captures small stdout/stderr output into a zone-backed `Output`; `exec` replaces the current process on success. Large-output draining, stdin redirection, and richer status values are future work. |
