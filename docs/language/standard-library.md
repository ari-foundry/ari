...
| `std::test` | Executable unit-test helpers. | `Report`, `report`, `scratch`, `check`, generic `equal`/`not_equal`, pass/fail accessors, `ok`, `finish`, `require`, and method wrappers. | Source helpers for aggregated checks and temporary test zones. Test discovery, logging, source locations, stack/backtrace, benchmark, and fuzzing hooks are roadmap work. |
...
| Testing And Diagnostics | Library work needs source-level tests and stable failure reporting. | current `std::test::Report` helpers; future runner discovery, richer panic/assert messages, source locations, logging, stack/backtrace, optional benchmarks, and optional fuzz hooks. |
