...
| `std::thread` | Function-pointer thread spawn/join, runtime ids, sleep/yield hints, and hosted parallelism. | `Thread`, `Builder`, `spawn`, `join`, `is_finished`, `yield_now`, `sleep`, `id`, `is_main`, `available_parallelism`, `is_join_error`. | First pthread-backed LLVM/Linux slice. Capturing entries, user-facing thread-local storage, applied custom stack sizes/names, and richer statuses are future work. |
...
| Start and join a thread. | `thread::spawn(worker)`, `thread::builder().name("worker").spawn(worker)`, `handle.join()`, `thread::join(handle)`, `handle.is_finished()`, `thread::id()`, `thread::yield_now()`, `thread::sleep(duration)`, `thread::available_parallelism()` | Thread entries are plain `fn() -> i64` today. The main thread id is `0`; spawned thread ids are positive. Join returns the entry result or `-1` for the current failure sentinel. `is_finished` is advisory. Builder stack/name options are recorded but not applied to pthread attributes yet; user TLS remains roadmap work. |
