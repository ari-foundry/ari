...
| `std::net` | Network address values and first TCP socket handles. | `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, `TcpListener`, `TcpStream`, address constructors/accessors, family, loopback, unspecified, port helpers, IPv4 TCP bind/connect/accept/local-port helpers, descriptor views, explicit close, and `std::io` byte adapters. | Hosted IPv4 TCP slice plus source address values. DNS, UDP, Unix sockets, IPv6 socket handles, socket options, nonblocking mode, timeouts, and shutdown are runtime roadmap work. |
...
| Represent network addresses or open first TCP sockets. | `net::Ipv4Addr::localhost()`, `net::Ipv6Addr::any()`, `addr.as_ip()`, `net::socket_addr(ip, port)`, `net::SocketAddr::localhost(port)`, `net::TcpListener::bind(addr)`, `net::TcpStream::connect(addr)`, `listener.accept()`, `listener.local_port()` | Address values are source structs. TCP handles are hosted IPv4 descriptors and may return `PermissionDenied` on restricted hosts. DNS lookup, UDP/Unix sockets, IPv6 handles, options, timeouts, nonblocking mode, and shutdown are future runtime slices. |
...
`TcpStream` implements `std::io::Reader` and `std::io::Writer`, so generic byte
helpers work with streams. DNS, UDP, Unix sockets, IPv6 socket handles,
timeouts, options, and shutdown still need owned OS-resource and error policy.
