...
| `std::io` | Byte-oriented process IO hooks and contracts. | `Reader`, `Writer`, `Seek`, `Stdin`, `Stdout`, `Cursor`, `stdin`, `stdout`, `cursor`, `read_exact`, `write_all`, `flush`, raw byte/scalar/line hooks. | Runtime-backed hooks plus source trait/cursor/exact-read/write-all helpers. `stderr`, `pipe`, `BufReader`, and `BufWriter` are roadmap items. |
...
| Write generic byte-output helpers. | `io::Writer`, `io::write_all[W: io::Writer](writer: ref mut W, bytes)`, `io::flush[W: io::Writer](writer: ref mut W)` | `io::Stdout` implements `Writer` now. File and buffered writer adapters are planned after owned handle and flush/drop behavior are specified. |
| Test parser or binary read logic without host stdin. | `io::cursor(bytes)`, `io::Reader`, `io::Seek`, `io::read_exact[R: io::Reader](reader: ref mut R, output, len)` | `Cursor` reads from a borrowed `Slice[u8]`, supports `position()` and `seek(position)`, and returns `-1` at EOF through `Reader.read_byte()`. |
...
`Option[File]` before calling them. Supported modes are `"r"`, `"w"`, `"a"`,
