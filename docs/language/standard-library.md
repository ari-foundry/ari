...
| `std::thread` | Function-pointer thread spawn/join, runtime ids, sleep/yield hints, hosted parallelism, and explicit thread-local handles. | `ThreadId`, `Thread`, `JoinHandle`, `JoinError`, `ThreadResult`, `ThreadLocalSetError`, `Builder`, `ThreadLocal`, `spawn`, `join`, `join_value`, `join_thread`, `join_thread_value`, `join_compat`, `detach`, `is_finished`, `yield_now`, `sleep`, `id`, `id_raw`, `current`, `is_main`, Result-returning `available_parallelism`, `available_parallelism_or`, raw compatibility helpers, `thread_local`, `thread_local_with_capacity`, `ThreadLocal::try_set`, `ThreadLocal::get_or_try_init`. | First pthread-backed LLVM/Linux slice. Capturing entries, generic `JoinHandle[T]`, scoped threads, compiler-level `thread_local` declaration sugar, and generic thread return payloads are future work. |
