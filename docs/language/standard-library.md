...
| `std::test` | Executable unit-test helpers. | `Report`, `report`, `scratch`, `check`, generic `equal`/`not_equal`, pass/fail accessors, `ok`, `finish`, `require`, and method wrappers. | Source helpers for aggregated checks and temporary test zones. Test discovery, log capture, richer source-map diagnostics, stack/backtrace, benchmark, and fuzzing hooks are roadmap work. |
| `std::log` | Level-prefixed stderr diagnostics. | `Level`, `rank`, `name`, `enabled`, `write`, `message`, `trace`, `debug`, `info`, `warn`, `error`. | Source-only helper over `std::io::Stderr`. Structured records, source-map labels, stack/backtrace, and capture policy are future work. |
| `std::diag` | Diagnostic values for compiler tools. | `Severity`, `LabelStyle`, `Label`, `Diagnostic`, `error`, `warning`, `note`, `help`, `primary`, `secondary`, `with_label`, `with_note`, `location`, `label_location`, `source_location`, `label_source_location`, `write`, and method wrappers. | Source-only value layer over borrowed byte messages and `std::source::Span`. One borrowed note and SourceMap lookup are current; multi-label builders, multiple notes, fix-its, owned messages, and stable source renderers are future work. |
| `std::source` | Source-coordinate values, borrowed text lookup, cached line maps, and bounded source maps for tools and diagnostics. | `FileId`, half-open byte `Span`, one-based `LineCol`, `Location`, `SourceFile`, `LineMap`, `SourceMap`, `file_id`, `file`, `span`, `empty_span`, `line_col`, `location`, `full_span`, `line_count`, `line_start`, `line_end`, `line_span`, `locate`, `line_map`, `source_map`, `source_file`, `source_lines`, `source_locate`, `contains`, `touches`, `same_file`, `merge`, and method wrappers. | Source-only value and borrowed text layer. Owned filename/text maps, diagnostic labels, and stable rendering are future work. |
...
| `std::fs` | Byte-oriented filesystem handles. | `File`, `FileKind`, `Metadata`, `Permissions`, `exists`, `can_read`, `can_write`, `can_execute`, `permissions`, `metadata`, `try_metadata`, `mode`, `try_mode`, `set_mode`, `set_permissions`, `canonicalize`, `try_canonicalize`, `remove`, `rename`, `hard_link`, `symbolic_link`, `create_dir`, `remove_dir`, `open`, `try_open`, `create`, `try_create`, compatibility `open_*`/`try_open_*`, `read_byte`, `write_byte`, `write_bytes`, whole-file `read`, `try_read`, `write`, `try_write`, `append`, `try_append`, `truncate`, `copy`, `try_copy`, `read_to_string`, `try_read_to_string`, `close`. | First runtime-backed file slice over host file descriptor/stat/chmod/realpath hooks plus source `Option[File]`, `Option[String]`, `Option[i64]` byte-count helpers, `Option[Metadata]`, `Permissions`, canonical path copying, and whole-file byte helpers. Directory iteration, recursive directory helpers, owned path values, temp files, richer metadata timestamps/owner/group/ACLs, and optional locking remain roadmap work. |
| `std::path` | Source-only lexical path manipulation. | `PathBytes`, `bytes`, `from_os`, method-style path-byte helpers, `is_separator`, `is_absolute`, `is_relative`, `trim_trailing_separators`, `components`, `file_name`, `parent`, `extension`, `stem`, `join_in`, `normalize_in`. | POSIX-style `/` separator policy implemented in Ari source. Existing-path runtime canonicalization lives in `std::fs`; platform-specific paths and owned path buffers are future work. |
...
| Write a small executable unit test. | `var report = test::report(); report.equal(left, right); report.check(condition); var zone = test::scratch(n); report.finish()` | `std::test` aggregates pass/fail counts and returns a final status. Destroy scratch zones explicitly. Use root `assert`, `assert_equal`, `assert_not_equal`, `panic`, `todo`, or `unreachable` when the first failure should stop immediately. Source-map rendering, logging capture, stack/backtrace, benchmark, and fuzzing integration are future work. |
| Represent source positions in tools. | `var sources = source::source_map(ref mut zone, 8); let id = sources.add(ref mut zone, text); let span = sources.lines(id).line_span(2); let loc = sources.locate(span)` | Spans are half-open byte ranges. `touches(offset)` is inclusive for cursor/caret logic. Line and column values are one-based. `SourceFile` borrows text; `LineMap` caches line starts in an explicit `Zone`; `SourceMap` registers several borrowed files and cached maps. Owned source text, filenames, and diagnostic labels are the next bootstrap layer. |
| Build a compiler diagnostic. | `let diagnostic = diag::error("E0001", "expected name", span).with_label(diag::primary(span, "name goes here")).with_note("identifiers start with a letter"); let loc = diagnostic.source_location(ref sources); diagnostic.write()` | Use `std::diag` for shared severity/code/message/span values. Use `location(ref file)` for one borrowed file and `source_location(ref sources)` when a `SourceMap` is available. `write()` emits a simple stderr summary and one optional note line today; stable source rendering with labels, multiple notes, and fix-its is roadmap work. |
...
| Testing And Diagnostics | Library work needs source-level tests and stable failure reporting. | current `std::test::Report`, `std::log` line helpers, and `std::source` coordinate/borrowed text lookup values; future runner discovery, richer panic/assert messages, owned source-map rendering, structured logging, stack/backtrace, optional benchmarks, and optional fuzz hooks. |
