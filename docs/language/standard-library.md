...
| `std::thread` | Function-pointer thread spawn/join and runtime ids. | `Thread`, `spawn`, `join`, `yield_now`, `id`, `is_main`, `is_join_error`. | First pthread-backed LLVM/Linux slice. Capturing entries, shared state, atomics, and richer statuses are future work. |
...
| Plan OS-facing code. | `std::env`, `std::process`, `std::thread`, `std::time`, and `std::fs` today; future `std::sync`, `std::os` | Args, current-process environment variables, current directory/executable path, current process id/exit, POSIX fork/wait, function-pointer thread spawn/join/yield, monotonic/wall-clock reads, sleep, and byte-oriented file handles are implemented. Portable process spawn, richer thread statuses, shared ownership/atomics, directories, metadata, and raw syscall wrappers need runtime and ownership policy work. |
...
| IO And Process Context | Programs need arguments, environment variables, stdin/stdout, process status, files, child processes, and threads. | `std::io`, `std::input`, `std::context`, `std::env`, `std::process`, `std::thread`, `std::fs`, future `std::sync`. |
