...
| `std::time` | Monotonic instants, wall-clock timestamps, durations, sleep, and deadlines. | `Duration`, `Instant`, `SystemTime`, `Deadline`, `nanoseconds`, `microseconds`, `milliseconds`, `seconds`, `now`, `system_now`, `elapsed`, `sleep`, `timeout`, `timeout_after`, `deadline_at`. | Runtime-backed clock/sleep hooks plus source value wrappers. Calendar conversion and timezone data remain roadmap or external-package work. |
...
| `std::string` | Zone-backed owned byte string seed plus typed borrowed text-boundary views. | `String`, `RawString`, `Utf8`, `OsStr`, `CStr`, `utf8`, `os_str`, `c_str`, `c_len`, `c_bytes`, capacity constructors, `join_in`, copy helpers, byte get/set/search, byte-slice `find`, borrowed split/chunk/window views, `try_get`, `try_pop`, growth, append helpers, UTF-8 validation/scalar helpers, ASCII case compare/search, trim views, trim copies, whole and prefix parse helpers, `as_slice`, `as_ptr`. | Implemented as a byte string with UTF-8 scalar helpers and boundary views. Owned `Utf8String`/`OsString`/`CString`, normalization, grapheme clusters, and locale policy remain future work. |
