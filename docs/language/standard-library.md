...
| `std::cmp` | Comparison traits and helpers. | `Eq`, `PartialEq`, `Ord`, `PartialOrd`, primitive scalar impls, `Ordering`, `compare`, `then_compare`, `min`, `max`, `clamp`, `is_between`. | Implemented for source-level trait-bound static dispatch. |
