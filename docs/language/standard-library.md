...
| `std::process` | Current-process helpers and POSIX child-process control. | `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, `is_success`, `is_failure`, `is_root`, `fork_result`, `wait_result`, raw compatibility `fork`, `wait`, `is_child`, `is_parent`, `is_fork_error`, `is_wait_error`, `Command`, `Child`, `arg`, `env_var`, `command`, `command_with_args`, `kill`, `terminate`. | Runtime-backed id/uid/gid/exit/abort hooks plus POSIX fork/wait Result helpers and a command builder for `spawn`, `status`, `exec`, environment setup, working-directory setup, and child kill/wait. Captured output, richer statuses, and Windows process mapping are future work. |
...
| `std::fs` | Byte-oriented filesystem handles. | `File`, `OpenOptions`, `FileKind`, `Metadata`, `Permissions`, `exists`, `can_read`, `can_write`, `can_execute`, `permissions`, `metadata`, `try_metadata`, `symlink_metadata`, `try_symlink_metadata`, `try_file_type`, `is_file`, `is_dir`, `is_symlink`, `is_other`, metadata access/modification/status-change methods, `mode`, `try_mode`, `set_mode`, `set_permissions`, `canonicalize`, `try_canonicalize`, `remove`, `remove_result`, `remove_raw_result`, `rename`, `rename_result`, `rename_raw_result`, `hard_link`, `symbolic_link`, `read_link`, `try_read_link`, `create_dir`, `create_dir_result`, `create_dir_raw_result`, `ensure_dir`, `create_dir_all`, `ensure_dir_all`, `remove_dir`, `remove_dir_result`, `remove_dir_raw_result`, `remove_dir_all`, `open`, `try_open`, `open_result`, `open_raw_result`, `open_options`, `OpenOptions::open_result`, `OpenOptions::open_raw_result`, `create`, `try_create`, `create_result`, `create_raw_result`, compatibility `open_*`/`try_open_*`, `read_byte`, `write_byte`, `write_bytes`, `position`, `seek`, whole-file `read`, `try_read`, `write`, `write_result`, `write_raw_result`, `try_write`, `append`, `append_result`, `append_raw_result`, `try_append`, `truncate`, `copy`, `copy_result`, `copy_raw_result`, `try_copy`, `read_to_string`, `try_read_to_string`, `File` as `std::io::Reader`/`Writer`/`Seek`, `descriptor`, `close`. | First runtime-backed file slice over host file descriptor/stat/lstat/chmod/realpath/readlink/mkdir/lseek hooks plus source `Option[File]`, direct `Result[..., Error]` open/mutation/write/append/copy results, raw compatibility result bridges, `OpenOptions`, `Option[String]`, `Option[i64]` byte-count helpers, target-following and no-follow `Option[Metadata]`, direct path-kind predicates, access/modification/status-change timestamps, `Permissions`, canonical path copying, symbolic-link target reads, whole-file byte helpers, generic IO file adapters, cursor positioning, directory creation and removal, and non-owning `std::os::Fd` descriptor views. Owned path values, temp files, portable creation/birth time, richer owner/group/ACLs, and optional locking remain roadmap work. |
| `std::path` | Source-only lexical path manipulation. | `PathBytes`, `bytes`, `from_os`, method-style path-byte helpers, `is_separator`, `is_absolute`, `is_relative`, `trim_trailing_separators`, `components`, `file_name`, `parent`, `extension`, `stem`, `has_file_name`, `has_extension`, `has_stem`, `join_in`, `normalize_in`. | POSIX-style `/` separator policy implemented in Ari source. Existing-path runtime canonicalization lives in `std::fs`; platform-specific paths and owned path buffers are future work. |
| `std::net` | Network address values, DNS lookup, and socket handles. | `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, `TcpListener`, `TcpStream`, `UdpSocket`, `UnixListener`, `UnixStream`, `Shutdown`, address constructors/accessors, family, loopback, unspecified, port helpers, IPv4 DNS lookup, direct `Error` result and raw compatibility lookup/socket construction helpers, IPv4 TCP bind/connect/accept/local-port/local-address/peer-address helpers, IPv4 UDP bind/local-address/send-byte/receive-byte helpers, Unix stream bind/connect/accept helpers, descriptor views, nonblocking flags, `std::time::Duration` timeouts with raw millisecond compatibility helpers, stream shutdown, explicit close, TCP/Unix `std::io` byte adapters, and TCP/Unix `read_exact`/`write_all` stream buffer helpers. | Hosted IPv4 TCP/UDP/DNS and Unix stream slices plus source address values. IPv6 socket handles, buffer datagrams, UDP source address helpers, broad socket options, timeout-specific errors, and richer structured error payloads are roadmap work. |
...
| `std::algo` | Slice algorithms. | `sort`, `sort_by`, `stable_sort`, `stable_sort_by`, `binary_search`, `binary_search_by`, `lower_bound`, `lower_bound_by`, `upper_bound`, `upper_bound_by`, `equal_range`, `equal_range_by`, `partition_point`, `is_sorted`, `is_sorted_by`, `reverse`, `rotate_left`, `rotate_right`, `partition`, `min`, `min_by`, `max`, `max_by`, `clamp`, `clamp_by`, `swap`, `fill`, `copy`, `dedup`. | Source-only first slice over borrowed `Slice[T]` views, with comparator variants for temporary ordering policies. Faster sorts, move-aware contracts, hex/base64 encoding, and compression policy remain roadmap work. |
...
| Fork and wait for a child on POSIX. | `process::fork_result()`, `process::is_child(pid)`, `process::is_parent(pid)`, `process::wait_result(pid)`, compatibility `process::fork()`, `process::wait(pid)` | Prefer Result helpers when failure matters. `fork_result` returns the child branch/pid or `Error`; `wait_result` returns a normal child exit code or `Error`. Raw helpers keep the older sentinel conventions. Portable spawn, process handles, and richer status values are future work. |
| Run a child command on POSIX. | `process::arg(value)`, `process::env_var(name, value)`, `process::command(program)`, `process::command_with_args(program, args)`, `Command::spawn()`, `Command::status()`, `Command::exec()`, `Child::pid()`, `Child::wait()`, `Child::kill(signal)`, `process::kill(pid, signal)` | Use `process::arg` for argv slices and `process::env_var` for child environment setup. `status` spawns and waits; `spawn` returns a handle; `exec` replaces the current process on success. Captured `output` and stdout/stderr redirection are future work. |
...
| Represent network addresses or use sockets. | `net::Ipv4Addr::localhost()`, `net::Ipv6Addr::any()`, `addr.as_ip()`, `net::socket_addr(ip, port)`, `net::lookup_v4(host, port)`, `net::TcpListener::bind(addr)`, `net::TcpStream::connect(addr)`, `listener.accept()`, `listener.local_port()`, `listener.local_addr()`, `listener.set_accept_timeout(time::milliseconds(n))`, `stream.local_addr()`, `stream.peer_addr()`, `stream.set_read_timeout(timeout)`, `stream.set_write_timeout(timeout)`, `stream.write_all(bytes)`, `stream.read_exact(ptr, len)`, `net::UdpSocket::bind(addr)`, `socket.local_addr()`, `socket.send_byte_to(value, addr)`, `net::UnixListener::bind(path)`, `net::UnixStream::connect(path)`, `stream.shutdown(net::Write)` | Address values are source structs. TCP/UDP handles are hosted IPv4 descriptors; Unix stream sockets are hosted path-based descriptors. Prefer `std::time::Duration` timeout setters and reserve `_millis` variants for raw boundary work. Socket creation may return `PermissionDenied` or `Unsupported` on restricted hosts. IPv6 handles, buffer datagrams, broad options, UDP source addresses, timeout-specific errors, and richer errors are future runtime slices. |
...
| Work with borrowed contiguous data. | `Slice[T]`, `slice(data, len)`, `.as_slice()`, `.slice(start, end)`, `.split_at(index)`, `.find(needle)`, `.chunks(size)`, `.windows(size)`, `.split(delimiter)`, `.sort()`, `.sort_by(less)`, `.binary_search(value)`, `.binary_search_by(value, less)`, `.equal_range(value)`, `.partition_point(predicate)` | Slice methods borrow the view; use `try_get` when absence is expected, `find`/`contains_slice` for subslice search, lazy chunks/windows/split for allocation-free views, `copy_to(ref mut zone)` when an owned collection is needed, and direct algorithm wrappers for in-place reordering, fill/copy, partition/dedup, sorting, search, equal-range lookup, partition-point lookup, and min/max. The non-`_by` ordering wrappers require `Ord`; the `*_by` wrappers take an explicit comparator. |
...
| Store a growable source collection. | `std::vec::new<T>(ref mut zone, capacity)` | Common tracked locals can call `push`, `insert`, `reserve`, and related methods without spelling the zone again. Use `vec.slice`, `vec.split_at`, `vec.find`, `vec.contains_slice`, `vec.compare`, `vec.chunks`, `vec.windows`, and `vec.split` for allocation-free borrowed sequence views, `vec.reverse`/`vec.rotate_left`/`vec.rotate_right` for in-place reordering, `vec.sort`/`vec.binary_search`/`vec.equal_range` when the element type implements `Ord`, `vec.partition_point` for predicate-partitioned storage, and `vec.sort_by`/`vec.binary_search_by`/`vec.equal_range_by` for explicit comparator orderings. |
...
| Plan OS-facing code. | `std::target`, `std::env`, `std::process`, `std::thread`, `std::sync`, `std::time`, `std::fs`, `std::os`, `std::io`, `std::mem`, `std::random`, and `std::net` socket handles today. | Args, current-process environment variables, current directory/executable path, process id/uid/gid/exit/abort, POSIX fork/wait with direct `Error` helpers plus raw compatibility helpers, POSIX `Command` spawn/status/exec/kill with child environment and working-directory setup, function-pointer thread spawn/join/sleep/yield, available parallelism, concrete atomic i64 operations, source `Mutex`/`RwLock`/`Once`, monotonic/wall-clock reads, sleep, byte-oriented file handles, `OpenOptions`, direct `Error` result helpers for filesystem open/mutation/byte-count operations, raw filesystem result compatibility bridges, file cursor seek/position, descriptor views/owned descriptor close, duplication, pipe creation, pipe `Reader`/`Writer` adapters, close-on-exec, and nonblocking policy, access permissions, target-following and no-follow metadata with access/modification/status-change timestamps, POSIX permission mode lookup/mutation, existing-path canonicalization, path rename, single-directory create/remove, directory-name reads, runtime page size, OS entropy through `getrandom` with `/dev/urandom` fallback, network IP/socket-address values, IPv4 DNS lookup, IPv4 TCP bind/connect/accept/local-port/local-address/peer-address helpers, IPv4 UDP local-address/single-byte datagrams, Unix stream sockets, TCP/Unix stream buffer helpers, socket nonblocking/timeout/shutdown helpers, and target facts are implemented. Captured process output, stdout/stderr redirection, Windows process mapping, richer thread statuses, user thread-local storage, custom thread stack sizes, shared ownership, generic synchronization, Condvar/OnceLock/LazyLock/channels/futex-backed blocking, richer directory-entry metadata, recursive directories, owned path values, temporary files, portable metadata creation/birth time, richer owner/group/ACLs, IPv6 socket handles/buffer datagrams/broad options, optional locking, raw syscall wrappers, broader fcntl/ioctl/poll/select, epoll/inotify/eventfd/timerfd/signalfd/pidfd/memfd/io_uring, signal mask/sigaction/alt-stack, mmap/munmap/mprotect/msync/mlock/madvise, fallible entropy, and build hardening profiles need runtime and ownership policy work. |
...
`fs::open_result(path, mode)` when a `std::error::Error` value matters,
