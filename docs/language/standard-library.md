...
| Work with small byte files. | `fs::write(path, bytes)`, `fs::append(path, bytes)`, `fs::read(ref mut zone, path)`, `fs::read_to_string(ref mut zone, path)`, `fs::read_bytes(ref mut zone, path)`, `fs::copy(source, target)`, compatibility `*_result`/`try_*`/`*_optional`/`*_unchecked` helpers, `fs::rename(source, target)`, `fs::truncate(path)`, `fs::create(path)`, `fs::create_optional(path)`, `fs::open(path, "r")`, `fs::open_optional(path, "r")`, `file.read_byte()`, `file.write_bytes(slice)`, `file.close()`, `fs::exists(path)`, `fs::permissions(path)`, `fs::remove(path)` | Natural whole-file helpers return `Result[..., Error]`; use `try_*` or `_optional` only when absence/error detail should be collapsed, and `_unchecked` only for compatibility assertions. `*_result` names are migration aliases for older callers. `"w"` creates or truncates, `"a"` creates or appends, and `"rw"` opens an existing file for reading/writing. `permissions` is a preflight access snapshot, not a replacement for handling later file errors. The current `File` is a value handle, so close successful handles once and do not reuse copies after closing. |
...
| Write generic byte-output helpers. | `io::Writer`, `io::BufWriter`, `io::write[W: io::Writer](writer: ref mut W, bytes)`, `io::write_all[W: io::Writer](writer: ref mut W, bytes)`, `writer.write(bytes)`, `writer.write_all(bytes)`, `io::flush[W: io::Writer](writer: ref mut W)`, `io::print_text(text)`, `io::println_text(text)`, `io::eprint_text(text)`, `io::eprintln_text(text)`, `_unchecked` compatibility wrappers | `io::Stdout`, `io::Stderr`, `io::PipeWriter`, buffered writers, and `std::fs::File` implement `Writer` now. Prefer natural Result helpers when failure matters; `_unchecked` wrappers discard error detail intentionally. `BufWriter` uses a caller-provided `Slice[u8]` buffer and flushes explicitly. File close ownership is still manual. |
