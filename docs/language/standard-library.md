...
| `std::os` | Low-level OS value boundaries. | `Fd`, `OwnedFd`, `fd`, `invalid`, `stdin`, `stdout`, `stderr`, descriptor predicates, raw accessor, equality, `OwnedFd::from_raw`, `as_fd`, `take`, `try_clone`, `close_on_exec`, `set_close_on_exec`, `is_nonblocking`, `set_nonblocking`, `close`, and `std::fs::File.descriptor()`. | Source descriptor views plus runtime-backed close/dup/fcntl hooks. Duplicate-with-flags policy, readiness APIs, raw syscalls, signals, and memory mapping are future work. |
