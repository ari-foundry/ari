...
| Format into owned text. | `format_in!(ref mut zone, "...", values...)`, `fmt::format<T>(ref mut zone, "value={}", value)`, `fmt::format2<A, B>(ref mut zone, "{}={}", key, value)`, `fmt::format3<A, B, C>(ref mut zone, "{}{}{}", a, b, c)`, `fmt::format4<A, B, C, D>(ref mut zone, "{}{}{}{}", a, b, c, d)`, `fmt::concat2(ref mut zone, a, b)`, `fmt::concat3(ref mut zone, a, b, c)`, `fmt::concat_all(ref mut zone, parts)`, `fmt::unsigned_in(ref mut zone, value, spec)`, `fmt::integer_in(ref mut zone, value)`, `fmt::debug_value(ref mut zone, value)` | Default-zone `format!` is intentionally not executable in the current surface. Macro formatting is literal and compiler-checked; runtime template helpers support fixed one/two/three/four-value `{}` placeholders with `{{`/`}}` escapes and return `InvalidInput` on malformed templates. |
