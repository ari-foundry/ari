...
| Classify, trim, or parse ASCII bytes. | `ascii::is_digit`, `ascii::to_lower`, `ascii::trim`, `ascii::parse_decimal`, `ascii::parse_hex` | Scalar helpers take `u8`; slice helpers take `Slice[u8]`. Parsers return `Option[i64]` and require the entire slice to be valid. |
