...
| `std::zone` | Explicit region/zone lifecycle capability. | `create`, byte `alloc`, typed `alloc[T]`, `alloc_array[T]`, `new[T]`, `promote[T]`, `reset`, `destroy`, `allocation_zone`. | Runtime-backed with ownership/provenance checks in sema plus source raw array allocation. `ZoneMetadata` remains a compatibility bridge for allocator recovery. |
