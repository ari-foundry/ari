...
| `std::allocator` | Public allocation capability over region-backed storage. | `Allocator`, `from_zone`, `from_data`, `of`, typed allocation, logical counters, capacity preflight. | Source wrapper over current zone-backed metadata. New library growth APIs should prefer this vocabulary over direct `ZoneMetadata`. |
| `std::zone` | Low-level region runtime and compatibility layer. | `create`, byte `alloc`, typed `alloc[T]`, `alloc_array[T]`, `new[T]`, `promote[T]`, `reset`, `destroy`, `allocation_zone`. | Runtime-backed with ownership/provenance checks in sema plus source raw array allocation. `ZoneMetadata` remains a compatibility bridge for allocator recovery. |
...
`std::allocator::Allocator`; `ZoneMetadata` remains the compatibility bridge
under the current implementation. The `_in` suffix means "use this explicit
zone for growth or copying". `std::vec::Vec[T]` and `std::string::String`
