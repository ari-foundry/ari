...
take an explicit allocation lifetime, now documented as `Region`; older source
still uses the low-level `Zone` compatibility name because that is the current
runtime implementation. Ari does not provide a magical global heap. Some
...
| `std::process` | Current-process helpers and POSIX child-process control. | `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, `is_success`, `is_failure`, `is_root`, `fork`, `wait_status`, `wait`, raw compatibility `fork_raw`, `wait_raw`, `is_child`, `is_parent`, `is_fork_error`, `is_wait_error`, `Command`, `Child`, `ExitStatus`, `Output`, `arg`, `arg_bytes`, `env_var`, `env_var_bytes`, `command`, `command_with_args`, module-level `spawn`, `status`, `status_with_stdin`, `status_with_stdin_string`, `exit_status`, `output`, `output_in`, `exec`, `kill`, `terminate`. | Runtime-backed id/uid/gid/exit/abort hooks plus POSIX fork/wait Result helpers and a command builder for `spawn`, `status`, `status_with_stdin`, `exit_status`, `output`, `output_in`, `exec`, by-value `with_arg`/`with_env`/`with_clear_env`/`with_inherit_env`/`with_current_dir` chaining, byte-boundary argv/env/cwd helpers, inherited-or-cleared environment setup, working-directory setup, parent-visible setup/exec errors, bounded pipe-backed stdin plus file-backed or `/dev/null` stdin redirection, child kill/wait, typed normal/signal status inspection, and readiness-drained stdout/stderr capture. Module-level wrappers provide the same `Result[..., Error]` shape for function-style call sites. Interactive streaming stdin/stdout/stderr handles, richer platform status fields, and Windows process mapping are future work. |
...
| `std::thread` | Function-pointer thread spawn/join, explicit raw-data thread entries, runtime ids, sleep/yield hints, hosted parallelism, fixed-capacity scoped join owners, and explicit thread-local handles. | `ThreadId`, `Thread`, `JoinHandle`, `JoinError`, `ThreadResult`, `ThreadGroupResult`, `ThreadScope`, `ThreadLocalSetError`, `Builder`, `ThreadLocal`, `spawn`, `spawn_raw`, `spawn_configured`, `spawn_raw_configured`, `join`, `join_value`, `try_join`, `try_join_value`, `join_thread`, `join_thread_value`, `join_compat`, `detach`, `is_finished`, `yield_now`, `sleep`, `id`, `id_raw`, `current`, `is_main`, Result-returning `available_parallelism`, `available_parallelism_or`, raw compatibility helpers, `scope`, `thread_local`, `thread_local_with_capacity`, `ThreadLocal::try_set`, `ThreadLocal::get_or_try_init`. | First pthread-backed LLVM/Linux slice. Raw-data entries require caller-owned payload lifetimes. `ThreadScope` owns a fixed group of join rights, but borrowed scoped threads, capturing entries, generic `JoinHandle[T]`, compiler-level `thread_local` declaration sugar, and generic thread return payloads are future work. |
| `std::sync` | Small explicit synchronization primitives. | `Ordering`, `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr`, `RawMutex`, `RawRwLock`, `Mutex[T]`, `RwLock[T]`, `MutexGuard[T]`, `RwLockReadGuard[T]`, `RwLockWriteGuard[T]`, `Once`, `OnceLock`, `Condvar`, `WaitTimeoutResult`, `Barrier`, `Channel`, `Sender`, `Receiver`, atomic helpers, raw mutex/rwlock helpers, value lock helpers, `call_once`, `channel`. | LLVM atomic slice plus source spin/yield no-poison raw/manual locks, value-protecting payload locks, one-time slots, generation condition variables with timeout waits, barriers, and capacity-1 MPSC channels with Result errors. Futex-backed blocking waits, `LazyLock`, semaphores, and full send/share checks are future work. |
...
| `std::path` | Source-only lexical path manipulation. | `Path`, `PathBytes`, distinct owned POSIX `PathBuf`, `bytes`, `from_os`, `from_bytes`, `from`, `to_string`, `is_empty`, `contains_nul`, `as_bytes`, method-style path-byte and owned-path helpers, `is_separator`, `is_absolute`, `is_relative`, explicit `windows_*` drive/UNC classifiers, `trim_trailing_separators`, `components`, `file_name`, `parent`, `extension`, `stem`, `file_stem`, `has_file_name`, `has_extension`, `has_stem`, `has_file_stem`, `starts_with`, `strip_prefix`, `ends_with`, `strip_suffix`, `join_in`, `join`, `join_many`, `current_dir_join`, `normalize_in`. | Hosted Linux/POSIX-style `/` separator policy remains the default. Paths are bytes, not validated UTF-8. `PathBuf` wraps a zone-backed byte string and exposes path-specific methods. Existing-path runtime canonicalization lives in `std::fs`; Windows drive/UNC helpers classify bytes lexically, while platform-specific path storage and normalization remain future work. |
| `std::net` | Network address values, DNS lookup, and socket handles. | `Ipv4Addr`, `Ipv6Addr`, `IpAddr`, `SocketAddr`, `TcpListener`, `TcpStream`, `UdpSocket`, `UnixListener`, `UnixStream`, `Shutdown`, `ToSocketAddrs`, address constructors/accessors, family, loopback, unspecified, port helpers, IPv4/IPv6 DNS lookup, common service-name port lookup, `"host:port"` and `"[host]:port"` endpoint resolution, zone-backed `resolve_all`/`resolve_service`/`to_socket_addrs`/`to_socket_addrs_service` and `ToSocketAddrs` list conversion, module-level `listen`, `connect`, `connect_host`, `tcp_listen`, `tcp_connect`, `tcp_listen_v6`, `tcp_connect_v6`, `tcp_connect_host`, `udp_bind`, `udp_bind_v6`, `unix_listen`, `unix_connect`, direct `Error` result and raw compatibility lookup/socket construction helpers, IPv4/IPv6 TCP bind/connect/accept/local-port/local-address/peer-address helpers, IPv4/IPv6 UDP bind/local-address/send-byte/receive-byte helpers plus `send_to`/`recv_from`/`peek_from` and connected `send`/`recv`, Unix stream bind/connect/accept helpers, descriptor views, close-on-exec, nonblocking flags, TCP listener/UDP reuse-address and reuse-port helpers, TCP nodelay/keepalive/linger helpers, UDP broadcast helpers, TCP/UDP send/receive buffer-size helpers, IPv4 TTL, IPv6 hop-limit, `std::time::Duration` timeouts with raw millisecond compatibility helpers, single-descriptor readiness probes, stream shutdown, explicit close, TCP/Unix `std::io` byte adapters, and TCP/Unix `read`/`write`/`read_exact`/`write_all`/`read_to_end`/`read_to_string` stream buffer helpers. | Hosted IPv4/IPv6 TCP/UDP/DNS and Unix stream slices plus source address values. Natural socket constructors and methods use `Result[..., Error]` for production-style error handling while `_optional`/`try_*` wrappers remain compatibility conveniences. Fuller DNS iteration, host service-database lookup, timeout-specific errors, multi-descriptor poll/event loops, multicast, Unix datagram/credential helpers, and richer structured error payloads are roadmap work. TLS is intentionally outside `std::net` for now and should live in a future package-layer library. |
...
| `std::region` | Preferred public bulk allocation lifetime facade. | `Region`, `create`, `default_capacity`, `allocator`, raw and typed allocation, placement `new`, `promote`, logical capacity/used/remaining counters, preflight helpers, `reset`, and `destroy`. | Source facade over the current zone runtime. Use this name in new user-facing docs and examples. |
| `std::allocator` | Public allocation capability over region-backed storage. | `Allocator`, `from_region`, `from_zone`, `from_data`, `of`, `of_mut`, typed allocation, logical counters, capacity preflight. | Source wrapper over current zone-backed metadata. New library growth APIs should prefer this vocabulary over direct `ZoneMetadata`. |
| `std::zone` | Low-level region runtime and compatibility layer. | `create`, byte `alloc`, typed `alloc[T]`, `alloc_array[T]`, `new[T]`, `promote[T]`, `reset`, `destroy`, `allocation_zone`. | Runtime-backed with ownership/provenance checks in sema plus source raw array allocation. `ZoneMetadata` remains a module-scoped compatibility bridge for allocator recovery. |
...
| `std::string` | Zone-backed owned byte string seed plus typed text-boundary views. | `String`, `RawString`, borrowed `Utf8`, owned `Utf8String`, borrowed `OsStr`, owned `OsString`, `SplitOnce`, `utf8`, `utf8_string`, `utf8_string_optional`, `utf8_string_unchecked`, `os_str`, `os_string`, `os_string_from_slice`, `c_str`, `c_len`, `c_bytes`, `bytes`, capacity constructors, `join_in`, copy helpers, borrowed parser helpers `lines`, `trim`, `split_once`, `starts_with`, `ends_with`, `contains`, `find`, `strip_prefix`, `strip_suffix`, `substring`, allocating `replace`, byte get/set/search, byte-slice `find`, borrowed split/chunk/window views, `try_get`, `try_pop`, growth, `push_str`, append helpers, UTF-8 validation/scalar helpers, ASCII case compare/search, trim views, trim copies, whole and prefix parse helpers, `as_slice`, `as_ptr`. | Implemented as byte-oriented storage with typed UTF-8 and OS-boundary wrappers. `Utf8String` validates before owning bytes and does not expose mutable byte access; `OsString` owns raw POSIX-style OS bytes and can be converted to UTF-8 only after validation. Normalization, grapheme clusters, locale policy, and platform-specific OS-string storage remain future work. C ABI owned strings live in `std::c::CString`. |
...
| `std::parse` | Whole-input parsers over ASCII-trimmed byte slices. | `Parse`, `ParseError`, `ParseErrorKind`, `parse<T>`, `parse_or<T>`, `is_parse<T>`, Result-returning signed `integer`/`integer_radix`, unsigned `unsigned`/`unsigned_radix`, hex/binary/octal signed wrappers, `boolean`, `_optional` compatibility forms, `is_float`, Result-returning `float`, `float_error`, `float_optional`, `float_or`, `float_unchecked`. | Implemented in Ari source. Natural integer, boolean, and float parser names return `Result[..., Error]`; `_optional`, `_or`, and `_unchecked` forms discard error details or assert trusted input intentionally. `ParseError` reports syntax, separator, overflow, and underflow offsets; `parse[f64]`/`parse_or[f64]` provide strict/fallback typed float parsing. |
...
| `std::hash` | Deterministic non-cryptographic hashing. | `Hasher`, `Hash[T]`, `new`, `reset`, `finish`, `write`, `value`, `pair`, `combine`, `bytes`, primitive write helpers. | Source-only helpers for values, byte slices, two-value composition, and precomputed hash composition. Hash-table `with_capacity` constructors use this trait-driven default hash policy; explicit hash functions remain available for custom policies. |
...
| `std::fmt` | Formatting traits and explicit formatting helpers. Root `Display`/`Debug` are aliases for these traits. | `Debug::debug_in`, `Display::format_in`, `FormatSpec`, `decimal`, `hex`, `binary`, `octal`, width/precision/alignment modifiers, allocator-backed `*_in` helpers including `float_in`, direct scalar/text `*_stream` writer helpers, `debug_value`, `format_value`, fixed-arity runtime-template `format`/`format2`/`format3`/`format4`, `concat2`, `concat3`, variable-count `concat_all`/`concat_strings`, Result-returning `write_value`, `write_format`, `write_format2`, `write_format3`, `write_format4`, explicit `write_format_stream*` spellings, `write_concat2`, `write_concat3`, variable-count writer concat helpers, `write_debug`, type-specific `write_*` helpers for `io::Writer`, `_bool` compatibility writer wrappers, `print_value`, `println_value`, `print_debug`, and `println_debug`. | Formatting macros still use compiler lowering. Runtime template helpers use `{}` placeholders plus `{{`/`}}` escapes and return `InvalidInput` on template or arity mismatch. `FormatSpec` covers source hex/binary/octal, width, precision, and alignment for unsigned integers. |
...
Allocation APIs create regions through `Zone` and grow existing handles through
an allocation capability. The preferred public name for that capability is
`std::allocator::Allocator`; `ZoneMetadata` remains the module-scoped
compatibility bridge under the current implementation. The `_in` suffix means
"use this explicit zone for growth or copying". `std::vec::Vec[T]` and
`std::string::String`
recover an allocator from their backing storage, so common growth methods such
as `push`, `insert`, `reserve`, `reserve_extra`, `extend_from_slice`, byte
appends, and `resize` can recover the runtime allocation capability without a
zone argument after construction.
...
| --- | --- | --- |
| Print debug or user-facing output. | `print`, `println`, `eprintln`, `print!`, `println!`, `eprintln!`, `log::debug("...")`, `log::info("...")`, `log::write(log::Info, bytes)`, `var out = io::stdout(); io::write_all(ref mut out, slice)`, `var err = io::stderr(); io::write_all(ref mut err, slice)`, `io::write_bytes(slice)` | Format strings must be string literals. Use `{}` or `{name}` for text literals, owned `String`, byte `char` values, integers, bools, and `f32`/`f64`; named captures can read fields like `{point.x}` and tuple slots like `{pair.0}`. Use `{:.N}` or `{name:.N}` for float precision; use `{:?}` or `{name:?}` for debug text. `eprintln`/`eprintln!` use the same formatting rules and write to stderr. Use `std::log` for level-prefixed stderr diagnostics and `write_all`/`write_bytes` for raw `Slice[u8]` output. |
...
| Read process arguments. | `env::args(ref mut zone)`, `env::args_os(ref mut zone)`, `env::try_arg(ref mut zone, index)`, `env::try_arg_os(index)`, `env::program_name(ref mut zone)`, `env::program_name_os()`, `env::arg_count()`, `env::has_arg(index)`, root `arg_count()`, `has_arg(index)` | Use `args` for CLI dispatch such as `arix build --release`; use `args_os`/`try_arg_os` when an argument should stay as `std::string::OsStr` until validated or treated as bytes. Natural text helpers allocate owned `String` values in the caller's zone; raw startup strings stay in `std::context` and boundary code. |
| Read or edit current-process environment variables. | `env::var(ref mut zone, ref name)`, `env::var_os(ref name)`, `env::get(ref mut zone, ref name)`, `env::get_os(ref name)`, `env::try_get(ref mut zone, ref name)`, `env::try_get_os(ref name)`, `env::get_or_default(ref mut zone, ref name)`, `env::get_os_or_default(ref name)`, `env::has(ref name)`, `env::set_var(ref name, ref value)`, `env::remove_var(ref name)`, `env::set(ref name, ref value)`, `env::remove(ref name)`, `env::set_unchecked(ref name, ref value)`, `env::remove_unchecked(ref name)` | `name` and `value` are `std::string::String` values passed by reference. `var`/`var_os` return `Option` because missing environment variables are normal configuration absence. `get`/`get_os` and `set_var`/`remove_var` preserve `Error` detail; `_or_default` and `_unchecked` helpers intentionally discard it. |
| Inspect current process paths. | `env::current_dir(ref mut zone)`, `env::current_dir_path()`, `env::set_current_dir(ref path)`, `env::executable_path(ref mut zone)`, `env::executable_path_path()`, `env::executable_path_os()`, `env::home_dir()`, `_optional`/`try_*` compatibility wrappers, `std::path::from_os(value)` | Natural path text names return `Result[String, Error]`; `set_current_dir` takes a `String` path by reference. `home_dir` returns `Option[PathBytes]` because a user profile may be absent. Use `PathBytes` for lexical path work and `OsStr` when the path is still OS-boundary data. |
...
| Run a child command on POSIX. | `process::arg(value)`, `process::arg_bytes(ref mut zone, bytes)`, `process::env_var(name, value)`, `process::env_var_bytes(ref mut zone, name, value)`, `process::command(program)`, `process::command_with_args(program, args)`, module-level `process::spawn`, `process::status`, `process::status_with_stdin`, `process::status_with_stdin_string`, `process::status_code`, `process::exit_status`, `process::output`, `process::output_in`, `process::exec`, `Command::with_arg(ref mut zone, value)`, `Command::with_env(ref mut zone, name, value)`, `Command::with_clear_env()`, `Command::with_inherit_env()`, `Command::with_current_dir(path)`, `Command::clear_env()`, `Command::inherit_env()`, `Command::arg_bytes(ref mut zone, bytes)`, `Command::env_bytes(ref mut zone, name, value)`, `Command::current_dir_path(ref mut zone, path)`, `Command::spawn()`, `Command::spawn_with_stdin_file(path)`, `Command::spawn_with_stdin_null()`, `Command::status()`, `Command::status_with_stdin(bytes)`, `Command::status_with_stdin_string(text)`, `Command::status_with_stdin_file(path)`, `Command::status_with_stdin_null()`, `Command::status_code()`, `Command::exit_status()`, `Command::output(ref mut zone)`, `Command::output_in(ref mut zone)`, `Command::exec()`, `Child::pid()`, `Child::wait()`, `Child::wait_status()`, `Child::kill(signal)`, `process::kill(pid, signal)`, `ExitStatus::code()`, `ExitStatus::success()`, `ExitStatus::signal()`, `Output::stdout()`, `Output::stdout_string(ref mut zone)`, `Output::stderr()`, `Output::stderr_string(ref mut zone)` | Use `process::arg`/`env_var` for literal C-string-shaped values and `_bytes` helpers for owned `String`, `PathBuf`, or byte-slice values that need NUL validation. `with_arg`/`with_env`/`with_clear_env`/`with_inherit_env`/`with_current_dir` are by-value chainable helpers; mutating `arg`/`env`/environment-policy methods still take explicit receiver state and return `void`. `status` spawns and waits for typed `ExitStatus`; `status_code` keeps the explicit normal-code compatibility shape; `spawn` returns a handle; `output`/`output_in` capture stdout/stderr into a zone-backed `Output` while readiness-draining both pipes to avoid one stream blocking the other; `exec` replaces the current process on success. Method and module-level forms both return `Result[..., Error]`. `status`/`spawn` inherit stdio and environment by default; child setup and `execvp` failures are reported to the parent through `Err(Error)`; call `clear_env` before `env` to start from an empty child environment; use `status_with_stdin` for bounded byte input and `status_with_stdin_file` or `status_with_stdin_null` when child stdin policy should be explicit. Interactive streaming stdin/stdout/stderr handles are future work. |
| Start and join a thread. | `thread::spawn(worker)`, `thread::builder().name("worker").stack_size(bytes).spawn(worker)`, `handle.join()`, `handle.join_value()`, `thread::join(ref mut handle)`, `thread::join_value(ref mut handle)`, `handle.detach()`, `handle.is_finished()`, `thread::id()`, `thread::id_raw()`, `thread::current()`, `thread::yield_now()`, `thread::sleep(duration)`, `thread::available_parallelism()`, `thread::available_parallelism_or(default)`, `thread::thread_local<T>(ref mut zone)` | Thread entries are plain `fn() -> i64` today. The main thread id is `0`; spawned thread ids are positive `ThreadId` values. `spawn` and `available_parallelism` return `Result`; `JoinHandle::join` returns `Result[i64, JoinError]`, while `join_value` wraps that value in `ThreadResult`. Join at most once unless the handle is detached. `Thread` is raw thread info for inspection/compatibility. `is_finished` is advisory. Builder stack/name options lower through pthread attributes on the LLVM/Linux backend. `ThreadLocal[T]` is an explicit zone-backed fixed-capacity handle with `try_set`, `get_or_try_init`, and capacity introspection; compiler-level `thread_local` statics remain roadmap work. |
...
| Use a small explicit lock or one-time initializer. | `Mutex::new<T>(value)`, `mutex.try_lock()`, `mutex.lock()`, `guard.value()`, `guard.set(value)`, `guard.unlock()`, `RwLock::new<T>(value)`, `rwlock.read()`, `rwlock.write()`, `rw_guard.value()`, `rw_guard.unlock()`, `RawMutex::new()`, `raw_mutex.lock()`, `raw_mutex.lock_raw()`, `RawRwLock::new()`, `raw_rwlock.read_lock()`, `raw_rwlock.write_lock()`, `Once::new()`, `once.call_once(init)`, `OnceLock::new<T>()`, `once_lock.set(value)`, `once_lock.get_or_try_init(init)`, `Condvar::new()`, `condvar.wait_timeout(ref mut raw_mutex, duration)`, `Barrier::new(parties)`, `sync::channel<T>(ref mut zone)`, `sender.send(value)`, `sender.try_send(value)`, `receiver.recv()`, `receiver.try_recv()`, `OnceCell::new<T>(ref mut zone)`, `Lazy::new<T>(ref mut zone, make)` | Current `Mutex[T]` and `RwLock[T]` are value-protecting no-poison locks that own a payload and expose it through guards. Use `guard.unlock()` or explicit `drop guard`; automatic scope/early-return RAII cleanup is not promised yet. `RawMutex` and `RawRwLock` remain for low-level manual lock code such as `Condvar`. `Condvar`, `Barrier`, and channels use spin/yield internals until Ari grows blocking wait/wake runtime support; the channel is capacity 1 and uses Result errors, with `_optional`/`_bool` compatibility helpers when detail should be discarded. `OnceCell`/`Lazy` are local value initializers; `OnceLock` is the sync-facing slot. |
...
| Split or join lexical paths. | `path::bytes(bytes)`, `path::from_os(os)`, `path::from(ref mut zone, text)`, `path::to_string(ref mut zone, path)`, `path::components(bytes)`, `path::file_name(bytes)`, `path::parent(bytes)`, `path::stem(bytes)`, `path::extension(bytes)`, `path::starts_with(bytes, prefix)`, `path::strip_prefix(bytes, prefix)`, `path::ends_with(bytes, suffix)`, `path::strip_suffix(bytes, suffix)`, `path::join(ref mut zone, base, child)`, `path::join_many(ref mut zone, parts)`, `path::current_dir_join(ref mut zone, child)`, `path::normalize_in(ref mut zone, bytes)`, `path::windows_drive(bytes)`, `path::windows_unc_prefix(bytes)` | Prefer `PathBytes` when a value should be treated as a path rather than a generic byte string. `PathBuf` is an owned POSIX path byte buffer backed by `String`. These helpers do not query the filesystem except `current_dir_join`, which reads cwd and returns `Result[PathBuf, Error]`. Borrowed component and strip helpers return views into the original bytes. Prefix/suffix helpers are component-aware. Default separator policy is hosted Linux/POSIX-style `/`; explicit `windows_*` helpers classify drive/UNC-shaped bytes without changing default path behavior. |
...
| Represent network addresses or use sockets. | `net::Ipv4Addr::localhost()`, `net::Ipv6Addr::any()`, `addr.as_ip()`, `net::socket_addr(ip, port)`, `net::lookup_v4(host, port)`, `net::lookup_v6(host, port)`, `net::service_port("https")`, `net::resolve("127.0.0.1:8080")`, `net::resolve("[::1]:8080")`, `net::resolve_all(ref mut zone, host, port)`, `net::resolve_service(ref mut zone, host, service)`, `net::to_socket_addrs(ref mut zone, endpoint)`, `net::to_socket_addrs_service(ref mut zone, host, service)`, `net::listen(addr)`, `net::connect(addr)`, `net::connect_host(endpoint)`, `net::tcp_listen(addr)`, `net::tcp_listen_v6(addr)`, `net::tcp_connect(addr)`, `net::tcp_connect_v6(addr)`, `net::tcp_connect_host(endpoint)`, `net::TcpListener::bind(addr)`, `net::TcpStream::connect(addr)`, `listener.accept()`, `listener.accept_ready(timeout)`, `listener.local_port()`, `listener.local_addr()`, `listener.local_addr_v6()`, `listener.set_accept_timeout(time::milliseconds(n))`, `listener.reuse_port()`, `listener.set_reuse_port(enabled)`, `stream.local_addr()`, `stream.local_addr_v6()`, `stream.peer_addr()`, `stream.peer_addr_v6()`, `stream.set_read_timeout(timeout)`, `stream.set_write_timeout(timeout)`, `stream.read_ready(timeout)`, `stream.write_ready(timeout)`, `stream.keepalive()`, `stream.set_keepalive(enabled)`, `stream.linger_seconds()`, `stream.set_linger_seconds(1)`, `stream.disable_linger()`, `stream.ttl()`, `stream.set_ttl(64)`, `stream.hop_limit()`, `stream.set_hop_limit(64)`, `stream.send_buffer_size()`, `stream.set_send_buffer_size(size)`, `stream.write(bytes)`, `stream.write_all(bytes)`, `stream.read(output)`, `stream.read_exact(ptr, len)`, `stream.read_to_end(ref mut zone)`, `stream.read_to_string(ref mut zone)`, `net::udp_bind(addr)`, `net::udp_bind_v6(addr)`, `net::UdpSocket::bind(addr)`, `socket.local_addr()`, `socket.local_addr_v6()`, `socket.reuse_port()`, `socket.set_reuse_port(enabled)`, `socket.broadcast()`, `socket.set_broadcast(enabled)`, `socket.ttl()`, `socket.set_ttl(64)`, `socket.hop_limit()`, `socket.set_hop_limit(64)`, `socket.send_buffer_size()`, `socket.recv_buffer_size()`, `socket.recv_ready(timeout)`, `socket.send_ready(timeout)`, `socket.send_to(bytes, addr)`, `socket.recv_from(output)`, `socket.peek_from(output)`, `socket.connect(addr)`, `socket.send(bytes)`, `socket.recv(output)`, `socket.send_byte_to(value, addr)`, `net::unix_listen(path)`, `net::unix_connect(path)`, `net::UnixListener::bind(path)`, `net::UnixStream::connect(path)`, `stream.shutdown(net::Write)` | Address values are source structs. TCP/UDP handles are hosted IPv4/IPv6 descriptors; Unix stream sockets are hosted path-based descriptors. Prefer natural `Result[..., Error]` socket constructors and methods for new code; `_optional`, `try_*`, `_unchecked`, and `_raw` forms intentionally discard detail or expose compatibility boundaries. `resolve_all`/`to_socket_addrs` return zone-backed vectors and currently collect the first IPv4 and first IPv6 answers when available. `service_port` uses a deterministic stdlib table for common names like `http`, `https`, `ssh`, and `domain`; it is not host `/etc/services` lookup. UDP `recv_from`/`peek_from` report the sender address. `linger_seconds` reads TCP `SO_LINGER` as `Option[i64]`, where `None` means disabled; `set_linger_seconds` enables it and `disable_linger` clears it. `ttl` controls IPv4 `IP_TTL`; `hop_limit` controls IPv6 `IPV6_UNICAST_HOPS`; setters accept `1..=255`. Prefer `std::time::Duration` timeout setters and reserve `_millis` variants for raw boundary work. Readiness probes are advisory and do not replace the later operation's `Result`. Socket creation may return `PermissionDenied` or `Unsupported` on restricted hosts. Fuller DNS iteration, host service-database lookup, multi-descriptor poll/event loops, multicast, timeout-specific errors, and richer errors are future runtime slices; TLS belongs in a future package-layer library rather than `std::net` today. |
| Inspect the target/platform contract. | `target::triple(ref mut zone)`, `target::triple_text()`, `target::arch()`, `target::is_linux()`, `target::uses_glibc()`, `target::uses_elf()`, `target::syscall_abi()`, `target::has_epoll()` | Natural text helpers return owned `String`; `_text` exposes borrowed compiler-owned strings for raw boundaries. These are compile-target facts, not live kernel probes. Static/dynamic linking, PIE, RELRO, stack protector, and raw descriptor APIs are platform roadmap items. |
| Read stdin. | `input::try_read_byte()`, `input(ref mut zone)`, `read_line(ref mut zone)`, `input_text()` | `try_read_byte` returns `Option[u8]` instead of the raw `-1` EOF sentinel. Natural line input copies into `std::string::String`; `_text` names expose the borrowed internal buffer. |
...
| Test parser or binary read logic without host stdin. | `io::cursor(bytes)`, `io::BufReader`, `io::Reader`, `io::Seek`, `io::read_one[R: io::Reader](reader: ref mut R)`, `io::read_exact[R: io::Reader](reader: ref mut R, output, len)`, `io::read_line_from[R: io::Reader](ref mut zone, ref mut reader)`, `io::read_to_string[R: io::Reader](ref mut zone, ref mut reader)`, `io::read_exact_unchecked[R: io::Reader](reader: ref mut R, output, len)` | `Cursor` reads from a borrowed `Slice[u8]`, supports `position()` and `seek(position)`, and returns `-1` at EOF through `Reader.read_byte()`. `read_one` converts the low-level byte hook into `ReadByteValue`, `ReadByteEof`, or `ReadByteError`; values below `-1` from custom readers mean adapter failure. `read_exact` returns `Error(UnexpectedEof)` for short input or the adapter error for a real read failure; `read_line_from` and `read_to_string` return zone-backed byte strings in `Result`; `_unchecked` wrappers keep compatibility shapes. `BufReader` uses a caller-provided `Slice[u8]` buffer. |
...
| Pop highest-priority values. | `BinaryHeap::new<T: std::cmp::Ord[T]>(ref mut zone)`, `PriorityQueue::new<T: std::cmp::Ord[T]>(ref mut zone)`, `with_capacity(...)`, or explicit `with_less(..., less)` constructors | Default constructors use `Ord` and make larger values higher priority. Explicit `less(a, b)` means `a` has lower priority than `b`; with `collections::less_i64`, larger integers pop first. Use `push`, `peek`, `try_peek`, `pop`, and `try_pop`. |
| Store values by hash lookup. | `HashMap::new<K: std::hash::Hash[K],V>(ref mut zone)`, `HashMap::with_capacity<K,V>(ref mut zone, capacity)`, `HashMap::with_hash<K,V>(ref mut zone, capacity, hash)`, `collections::hash_map<K,V>(...)`, or `collections::string_hash_map<V>(...)` | Hash functions have shape `fn(K) -> u64`. `new` and `with_capacity` use `std::hash::value<K>` and normal equality; `collections::hash_i64` is available for explicit i64 policies and delegates to `hash::value<i64>`. `string_hash_map` uses content hashing/equality for `HashMap[String,V]`; its `_bytes` helpers such as `contains_key_bytes`, `try_get_bytes`, `get_bytes`, `get_or_bytes`, `try_get_mut_bytes`, `get_mut_bytes`, `remove_bytes`, and `remove_entry_bytes` look up borrowed `Slice[u8]` keys without allocating temporary `String` values. `insert`/`replace` return the replaced `Option[V]`, `entry(key).or_insert(value)` updates missing values in place, `entry(key).or_default()` uses `Default` for missing values, `entry(key).insert_entry(value)` stores a value and returns the entry handle for chaining, `try_get` handles absence, `get_mut` returns `ref mut V`, `try_get_mut` returns an optional mutable value handle, `remove` leaves a tombstone for later probing, `remove_entry` keeps the removed key and value together, `keys()`/`values()` iterate live buckets, `values_mut()` updates stored values in place, `iter()`/direct `for entry in map` walk copied entries, `iter_mut()` walks copied-key mutable-value handles, and `drain()` empties the map while yielding entries. |
| Store hash-based membership. | `HashSet::new<T: std::hash::Hash[T]>(ref mut zone)`, `HashSet::with_capacity<T>(ref mut zone, capacity)`, `HashSet::with_hash<T>(ref mut zone, capacity, hash)`, `collections::hash_set<T>(...)`, or `collections::string_hash_set(...)` | `new` and `with_capacity` use `std::hash::value<T>` and normal equality. `insert` returns whether the value was new. Use `replace`, `take`, `remove`, `contains`, `reserve`, `clear`, and `iter`. `HashSet[String]` values created with `string_hash_set` also support borrowed byte-slice helpers such as `contains_bytes`, `try_get_bytes`, `get_bytes`, `take_bytes`, and `remove_bytes`. `intersection`, `difference`, `union`, and `symmetric_difference` are lazy live-bucket cursors; direct `for value in set` uses the same live-bucket cursor. |
| Store values by ordered lookup. | `TreeMap::new<K: std::cmp::Ord[K],V>(ref mut zone)`, `TreeMap::with_capacity<K,V>(ref mut zone, capacity)`, `TreeMap::with_less<K,V>(ref mut zone, capacity, less)`, or `collections::tree_map<K,V>(...)` | The default constructor uses `<` through `Ord`. An explicit comparator has shape `fn(K, K) -> bool` and must be a strict less-than relation. `collections::less_i64` is available for i64 keys. `entry(key)` supports in-place `or_insert`/`or_insert_with`/`or_default`/`and_modify` and `insert_entry` chaining, `replace` is the named insert-or-replace form, `get_mut` returns `ref mut V`, `try_get_mut` returns an optional mutable value handle, `remove_entry` returns copied key-value data, `keys()`/`values()` iterate in ascending key order, `values_mut()` updates values in sorted key order, `iter()`/direct `for entry in map` walk entries, `iter_mut()` walks copied-key mutable-value handles in sorted order, and `drain()` empties the map while yielding sorted entries. |
| Store ordered membership. | `TreeSet::new<T: std::cmp::Ord[T]>(ref mut zone)`, `TreeSet::with_capacity<T>(ref mut zone, capacity)`, `TreeSet::with_less<T>(ref mut zone, capacity, less)`, or `collections::tree_set<T>(...)` | Uses a red-black tree. Default constructors use `<` through `Ord`. `insert` rejects equal values, `replace` returns the previous equal value, `intersection`, `difference`, `union`, and `symmetric_difference` are lazy comparator-order cursors, and `iter`/direct `for value in set` walk ascending comparator order. |
...
| Store owned byte text. | `let text: String = "text"` inside a current/named region or zone, `std::string::from(ref mut zone, "text")`, `std::string::copy(ref mut zone, bytes)`, `std::string::empty(ref mut zone)`, `std::string::join_in(ref mut zone, parts, separator)`, `std::string::replace(ref mut zone, bytes, needle, replacement)` | The handle stores bytes, not a full Unicode text abstraction yet. Use `append`, `append_byte`, `append_bytes`, `push_str`, `contains_text`, `starts_with_text`, and `equals_text` for ordinary string-literal workflows. String literals copy into expected `String` values when a `region { ... }`, `zone { ... }`, mutable local named `region`/`zone`, or one unambiguous mutable region/zone source is visible. They also coerce to `Slice[u8]`, local `Vec[u8]`, or fixed `[u8, N]` when those byte types are expected; use `std::string::bytes("literal")` when the boundary should be explicit. |
| Distinguish text and path boundary values. | `std::string::utf8(bytes)`, `std::string::utf8_string(ref mut zone, bytes)`, `std::string::os_str(bytes)`, `std::string::os_string(ref mut zone, bytes)`, `std::string::c_str(cstr)`, `std::path::bytes(bytes)`, `std::path::from_os(os)` | `String` is owned bytes, `Utf8` is validated borrowed UTF-8, `Utf8String` is owned validated UTF-8, `OsStr`/`OsString` are raw OS bytes, `std::c::CStr` wraps NUL-terminated C ABI text, and `PathBytes` means path policy. String literals coerce directly to expected `Utf8`, `OsStr`, `PathBytes`, and `CStr` boundary views. |
...
| Allocate raw memory. | `region::alloc`, `region::alloc_array<T>`, `region::new<T>`, compatibility `zone::alloc`, `zone::alloc<T>`, `zone::alloc_array<T>`, `zone::new<T>` | Raw allocation does not run destructors or make memory safe by itself. `alloc_array<T>` returns uninitialized contiguous storage for `count` values. Prefer owning handles unless raw storage is the point. |
...
| Plan OS-facing code. | `std::target`, `std::env`, `std::process`, `std::thread`, `std::sync`, `std::rc`, `std::cell`, `std::time`, `std::fs`, `std::path`, `std::os`, `std::io`, `std::mem`, `std::random`, and `std::net` socket handles today. | Args, current-process environment variables, Result-returning current directory/executable path helpers, POSIX byte-oriented `Path`/`PathBuf` lexical helpers, process id/uid/gid/exit/abort, POSIX fork/wait with direct `Error` helpers plus raw compatibility helpers, POSIX `Command` spawn/status/typed-status/output/exec/kill with child environment, parent-visible setup/exec errors, readiness-drained stdout/stderr capture, and bounded pipe/file/null stdin redirection, module-level command wrappers, working-directory setup, normal/signal status inspection, function-pointer thread spawn/join/sleep/yield, typed `ThreadResult`, advisory completion, builder option values, explicit `ThreadLocal[T]` handles with recoverable capacity helpers, available parallelism, concrete atomic wrappers, explicit ordering names, source `Mutex[T]`/`RwLock[T]`/`RawMutex`/`RawRwLock`/`Once`/`OnceLock`/`Condvar`/`Barrier`, single-slot MPSC channels, `Rc`/`Arc`/`Weak`, `Cell`/`RefCell`/`OnceCell`/`Lazy`, monotonic/wall-clock reads, sleep, byte-oriented file handles, `OpenOptions`, direct `Error` result helpers for filesystem open/read/query/mutation/byte-count operations, raw filesystem result compatibility bridges, file cursor seek/position, descriptor views/owned descriptor close, duplication, pipe creation, pipe `Reader`/`Writer` adapters, close-on-exec, nonblocking policy, descriptor readiness probes, access permissions, target-following and no-follow metadata with access/modification/status-change timestamps, POSIX permission mode lookup/mutation, existing-path canonicalization, path rename, single-directory create/remove, recursive directories, directory-name and entry reads, hosted temporary paths, advisory file locks, runtime page size, OS entropy through `getrandom` with `/dev/urandom` fallback plus `Error`-returning entropy/fill forms, network IP/socket-address values, IPv4/IPv6 DNS lookup, `"host:port"` and `"[host]:port"` endpoint resolution, zone-backed resolver lists, module-level TCP/UDP/Unix socket constructors returning `Error`, IPv4/IPv6 TCP bind/connect/accept/local-port/local-address/peer-address helpers, IPv4/IPv6 UDP local-address helpers, buffer datagrams with source-address reporting, connected UDP send/recv, Unix stream sockets, TCP/Unix stream buffer helpers, common socket options including TCP linger, IPv4 TTL, and IPv6 hop-limit, socket nonblocking/timeout/shutdown helpers, and target facts are implemented. Interactive streaming stdin/stdout/stderr handles, Windows process mapping, generic thread return payloads, compiler-level `thread_local` statics, applied custom thread stack sizes beyond the pthread path, `LazyLock`/semaphores/futex-backed blocking, platform-specific path values and rules, ACL policy, exact mid-read filesystem errors, multi-descriptor poll/event loops, multicast socket options, raw syscall wrappers, broader fcntl/ioctl/select, epoll/inotify/eventfd/timerfd/signalfd/pidfd/memfd/io_uring, signal mask/sigaction/alt-stack, mmap/munmap/mprotect/msync/mlock/madvise, TLS package-layer policy, and build hardening profiles need runtime and ownership policy work. |
| Choose between collection families. | `Set`, `Deque`, `RingBuffer`, `LinkedList`, `BinaryHeap`/`PriorityQueue`, `HashMap`/`HashSet`, `TreeMap`/`TreeSet` | Use `Set` for small insertion-order unique lists, `Deque` for both-end queues, `RingBuffer` for bounded FIFO storage, `LinkedList` for linked front/back operations, heaps for priority removal, hash containers for average-case lookup, and tree containers for ordered lookup. Hash/tree/heap containers have trait-driven `new`/`with_capacity` constructors for ordinary `Hash + Eq` or `Ord` values, while `with_hash`, `with_less`, and the lower-level module constructors keep custom policy explicit. |
...
| Format into owned text. | `format_in!(ref mut zone, "...", values...)`, `region { format!("...", values...) }`, compatibility `zone { format!("...", values...) }`, `fmt::format<T>(ref mut zone, "value={}", value)`, `fmt::format2<A, B>(ref mut zone, "{}={}", key, value)`, `fmt::format3<A, B, C>(ref mut zone, "{}{}{}", a, b, c)`, `fmt::format4<A, B, C, D>(ref mut zone, "{}{}{}{}", a, b, c, d)`, `fmt::concat2(ref mut zone, a, b)`, `fmt::concat3(ref mut zone, a, b, c)`, `fmt::concat_all(ref mut zone, parts)`, `fmt::unsigned_in(ref mut zone, value, spec)`, `fmt::integer_in(ref mut zone, value)`, `fmt::debug_value(ref mut zone, value)` | Macro formatting is literal and compiler-checked. `format!` uses the active current allocation block; outside `region { ... }` or compatibility `zone { ... }`, use `format_in!` or explicit `fmt::*` helpers. Runtime template helpers support fixed one/two/three/four-value `{}` placeholders with `{{`/`}}` escapes and return `InvalidInput` on malformed templates. |
| Format into a writer. | `fmt::write_unsigned<W>(ref mut writer, ref mut zone, value, spec)`, `fmt::write_value<W, T>(ref mut writer, ref mut zone, value)`, `fmt::write_format<W, T>(ref mut writer, ref mut zone, "value={}", value)`, `fmt::write_format2<W, A, B>(ref mut writer, ref mut zone, "{}={}", key, value)`, `fmt::write_format3<W, A, B, C>(ref mut writer, ref mut zone, "{}{}{}", a, b, c)`, `fmt::write_format4<W, A, B, C, D>(ref mut writer, ref mut zone, "{}{}{}{}", a, b, c, d)`, `fmt::write_concat2<W, A, B>(ref mut writer, ref mut zone, first, second)`, `fmt::write_concat3<W, A, B, C>(ref mut writer, ref mut zone, first, second, third)`, `fmt::write_concat_all<W>(ref mut writer, parts)`, `fmt::write_debug<W, T>(ref mut writer, ref mut zone, value)` | `write_*` helpers take a temporary zone explicitly, work with any `std::io::Writer`, and return `Result[(), Error]`; use `_bool` wrappers only when discarding write failure details is intentional. Runtime-template writer helpers stream literal bytes and placeholder values directly, preserve invalid-template errors and writer errors, and do not roll back bytes written before a later error. |
...
| Share code with C. | `extern "C"`, C ABI aliases such as `c_int`/`c_char`/`c_void`, `std::c::CStr`, `std::c::CString`, `std::c::Library`, `std::c::Symbol`, `@repr(C)`, `@export`, `--shared`, `--emit-c-header` | Use `c::from("name").as_ptr()` for borrowed C strings, `c::from_slice_in(ref mut zone, bytes)` for owned NUL-terminated storage, `c::errno()`/`c::error()` for sentinel-plus-errno C APIs, and `symbol.function<fn(...) -> ...>()` for loaded dynamic functions. Do not pass Ari ownership across C directly. Zone-backed `CString` direct handoff to arbitrary externs is still behind an explicit FFI escape policy. |
...
| Collections | Most programs need growable storage, membership checks, ordered lookup, queues, priority queues, and borrowed views. | `std::vec`, `std::boxed`, `std::collections::Set`, `Deque`, `RingBuffer`, `LinkedList`, `BinaryHeap`, `PriorityQueue`, `HashMap`, `HashSet`, `TreeMap`, `TreeSet`, entry update handles, key-value removal, direct red-black tree deletion, natural trait-driven constructors, String-key borrowed lookup, mutable value cursors, and lazy set algebra. |
