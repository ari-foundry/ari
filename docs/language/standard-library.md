...
| `std::iter` | Iteration traits, range constructors, lazy adapters, and eager consumers. | `range`, `range_inclusive`, `repeat_with`, `Iterator[T]`, `IntoIterator[T]`, `Iterable[T]`, `map`, `filter`, `take`, `skip`, `enumerate`, `zip`, `fold`, `reduce`, `collect`, plus Slice/Vec iterator sources. | Adapter callbacks are plain function pointers today; non-capturing lambdas can fill those slots when an expected `fn(...) -> ...` type is present. `repeat_with` is an infinite generator source, so pair it with `take`, `zip`, or another terminating consumer. Mutable Slice/Vec iteration yields value handles until reference-valued iterator items land. Capturing closure-aware iterator adapter traits remain future work. |
...
| Transform iterators lazily. | `iter::repeat_with`, `iter::map`, `iter::filter`, `iter::take`, `iter::skip`, `iter::enumerate`, `iter::zip` | Store the adapter in a mutable local and iterate with `for value in ref mut adapter` when it tracks progress. `repeat_with(make)` calls `make()` for every yielded value and has no natural end, so bound it with `take`, `zip`, or another terminating consumer. `skip` is the drop-count adapter name because `drop` is a language operation. |
