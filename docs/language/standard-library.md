...
| Format into a writer. | `fmt::write_unsigned<W>(ref mut writer, ref mut zone, value, spec)`, `fmt::write_value<W, T>(ref mut writer, ref mut zone, value)`, `fmt::write_debug<W, T>(ref mut writer, ref mut zone, value)` | `write_*` helpers take a temporary zone explicitly, work with any `std::io::Writer`, and return `Result[(), Error]`; use `_bool` wrappers only when discarding write failure details is intentional. |
