...
| `std::math` | Source-only numeric helpers. | `abs`, `sign`, sign/parity predicates, checked add/sub/mul/neg/abs, wrapping/overflowing add/sub, saturating add/sub/mul/neg/abs, `pow`, floor/ceil division, `gcd`, `lcm`. | Current i64-signature helper slices with natural names; wrapping and overflowing multiplication remain future intrinsic work. |
...
| Use integer helper routines. | `math::abs`, `math::is_positive`, `math::checked_add`, `math::checked_mul`, `math::wrapping_add`, `math::overflowing_add`, `math::saturating_add`, `math::saturating_mul`, `math::pow`, `math::div_floor`, `math::mod_floor`, `math::gcd`, `math::lcm` | Current helpers have i64 signatures and natural names so they can grow into generic APIs later. Checked helpers return `Option[i64]`; overflowing add/sub return the tuple `(value, overflowed)`; saturating helpers clamp to i64 bounds. `checked_mul` guards before multiplying, `pow` asserts that the exponent is non-negative, division rounding helpers assert a non-zero denominator, and `lcm` returns `0` if either input is `0`. |
...
| Numerics | Systems programs need reliable arithmetic and bit helpers beyond operators. | `std::math`, `std::bits`, current checked add/sub/mul/neg/abs, wrapping/overflowing add/sub, saturating add/sub/mul/neg/abs, byte-swap, population count, and future wrapping/overflowing multiplication helpers. |
