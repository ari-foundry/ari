...
| `std::sync` | Small explicit synchronization primitives. | `Ordering`, `AtomicI64`, `AtomicBool`, `AtomicUsize`, `AtomicPtr`, `Mutex`, `RwLock`, `MutexValue[T]`, `RwLockValue[T]`, `Once`, `OnceLock`, `Condvar`, `WaitTimeoutResult`, `Barrier`, `Channel`, `Sender`, `Receiver`, atomic helpers, mutex/rwlock helpers, value lock helpers, `call_once`, `channel`. | LLVM atomic slice plus source spin/yield no-poison locks, value-protecting payload locks, one-time slots, generation condition variables with timeout waits, barriers, and capacity-1 MPSC channels with Result errors. Futex-backed blocking waits, final shorter `Mutex[T]`/`RwLock[T]` spelling, `LazyLock`, semaphores, and full send/share checks are future work. |
