...
| `std::parse` | Whole-input parsers over ASCII-trimmed byte slices. | `Parse`, `parse<T>`, `parse_or<T>`, `is_parse<T>`, signed `integer`/`integer_radix`, unsigned `unsigned`/`unsigned_radix`, hex/binary/octal signed wrappers, `boolean`, `is_float`, `float_or`, `float`. | Implemented in Ari source. `parse[f64]`/`parse_or[f64]` provide strict/fallback typed float parsing; `Option[f64]` and richer parse errors wait on float enum payload support. |
...
| Parse whole byte-slice values. | `parse::parse[i64](bytes)`, `parse::parse[u64](bytes)`, `parse::parse[bool](bytes)`, `parse::parse[f64](bytes)`, `parse::parse_or[i64](bytes, fallback)`, `parse::is_parse[u64](bytes)`, `parse::integer(bytes)`, `parse::unsigned(bytes)`, `parse::integer_radix(bytes, radix)`, `parse::unsigned_radix(bytes, radix)`, `parse::boolean(bytes)`, `parse::float(bytes)` | These helpers trim ASCII whitespace and reject trailing garbage. Signed and unsigned integer parsers are overflow checked. `boolean` accepts lowercase `true`/`false`. Strict `parse<T>` and `float` panic on invalid input; use `is_parse<T>`, `parse_or<T>`, or type-specific `*_or` helpers when invalid input is ordinary. |
