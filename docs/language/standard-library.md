...
| `std::os` | Low-level OS value boundaries. | `Fd`, `OwnedFd`, `fd`, `invalid`, `stdin`, `stdout`, `stderr`, descriptor predicates, raw accessor, equality, `OwnedFd::from_raw`, `as_fd`, `take`, `close`, and `std::fs::File.descriptor()`. | Source descriptor views plus runtime-backed close hook. Descriptor duplication, close-on-exec/nonblocking mode, readiness APIs, raw syscalls, signals, and memory mapping are future work. |
