...
| Work with borrowed contiguous data. | `Slice[T]`, `slice(data, len)`, `.as_slice()`, `.slice(start, end)`, `.split_at(index)`, `.find(needle)`, `.chunks(size)`, `.windows(size)`, `.split(delimiter)`, `.sort()`, `.sort_by(less)`, `.binary_search(value)`, `.binary_search_by(value, less)` | Slice methods borrow the view; use `try_get` when absence is expected, `find`/`contains_slice` for subslice search, lazy chunks/windows/split for allocation-free views, `copy_to(ref mut zone)` when an owned collection is needed, and direct algorithm wrappers for in-place reordering, fill/copy, partition/dedup, sorting, search, and min/max. The non-`_by` ordering wrappers require `Ord`; the `*_by` wrappers take an explicit comparator. |
