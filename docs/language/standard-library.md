...
| `std::process` | Current-process helpers and the first POSIX child-process slice. | `id`, `uid`, `gid`, `exit`, `abort`, `success`, `failure`, `is_success`, `is_failure`, `is_root`, `fork_result`, `wait_result`, raw compatibility `fork`, `wait`, `is_child`, `is_parent`, `is_fork_error`, `is_wait_error`. | Runtime-backed id/uid/gid/exit/abort hooks plus source POSIX fork/wait Result helpers and raw compatibility predicates. Portable spawn, exec, kill, richer statuses, and process handles are future work. |
