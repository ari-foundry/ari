...
function surface and body shape for each source. They are validated as part of
cache parsing: the payload header, hash, and function count must agree with the
cache record, and the source hash must still match the embedded metadata. Today
the module loader still uses AST summaries to decide whether a dependency can
skip parsing; the IR sidecar is the reserved V0 bridge for future executable
bodies that no longer fit in the compact source-level AST summary.
