...
`starts_with(Slice[u8])`, `ends_with(Slice[u8])`, method-style
`copy_to(ref mut Zone)`, and top-level
...
Metadata, checked byte `first`/`last`/`get`, byte search, prefix/suffix checks,
`as_ptr`, and target-zone copy borrow the string handle instead of copying it.
`String` is still an
